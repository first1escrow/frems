...
$realCertifiedMoney  = round(($rs->fields['cTotalMoney'] - $rs->fields['cFirstMoney']) * 0.0006);
...
//代扣二代健保總額
$NHITax = 0;
$NHITax = $detail['cNHITax'] + $detail['bNHITax']; //賣方 + 買方
##

//代扣稅款總額
$InterestTax = 0;
$InterestTax = $detail['cTax'] + $detail['bTax']; //賣方 + 買方
##
...
$balance = ($detail['cCertifiedMoney'] + $detail['bCertifiedMoney']) - $detail['cInterest'] - $feedBackMoney + $NHITax + $InterestTax + $feedBackNHITax + $feedBackTax;
##
//會計是否出款二代健保
if ($pay_by_case['fNHIpay'] == 'N') {
    $NHITax = 0;
}

//確認是否已確認過案件
$sql = "SELECT * FROM tFeedBackMoney WHERE fCertifiedId ='" . $cid . "' AND fDelete = 0";
...
<head>
    ...
    $(document).ready(function() {
        ...
        alert("<?=$certify_money_notice?>");
        <?php endif?>

        $('#int_total', window.parent.document).html(
            'NT$<?=number_format(($detail['bInterest'] + $detail['cInterest'])) . 元?>');
    ...
    function checkSellerFee() {
        let check = <?=$checkSellerServiceFee?>;
    ...
    function preview_pdf() {
        window.open("list_pdf.php?cCertifiedId=<?=$detail['cCertifiedId']?>&iden=b", "buyer",
            "width=800px,height=1200px,status=yes,scrollbars=yes,location=no,menubar=no,location=no");
        sleep(1000);

        window.open("list_pdf.php?cCertifiedId=<?=$detail['cCertifiedId']?>&iden=o", "owner",
            "width=800px,height=1200px,status=yes,scrollbars=yes,location=no,menubar=no,location=no");
    }

    function preview_pdf_new(target) {
        window.open("list_pdf_new.php?cCertifiedId=<?=$detail['cCertifiedId']?>&target=buyer", "buyer_new",
            "width=800px,height=1200px,status=yes,scrollbars=yes,location=no,menubar=no,location=no");

        setTimeout(function() {
            window.open("list_pdf_new.php?cCertifiedId=<?=$detail['cCertifiedId']?>&target=owner", "owner_new",
                "width=800px,height=1200px,status=yes,scrollbars=yes,location=no,menubar=no,location=no");
        }, 1000);
        ...
        <form name="chkform" method="post">
            ...
            <div style="padding-bottom: 20px;">
                <span style="float:left;">
                    ...
                    履保費(<?=$detail['cCertifiedMoney'] + $detail['bCertifiedMoney']?>)
                    - 利息(<?=$detail['cInterest']?>) + 代扣利息二代健保(<?=$NHITax?>) + 代扣利息稅額(<?=$InterestTax?>)
                    - 地政士回饋金(<?=$feedBackMoney?>) + 代扣地政士回饋金二代健保(<?=$feedBackNHITax?>) + 代扣地政士回饋金稅額(<?=$feedBackTax?>)
                    =
                    <?php
                ...
                <table cellspacing="0" cellspadding="2" style="width:800px;">
                    ...
                    <tr>
                        ...
                        <td style="width:200px;"><input type="text" name="bBuyerId"
                                value="<?=strtoupper($detail['bBuyerId'])?>"></td>
                    </tr>
                    <tr>
                        ...
                        <td style="width:200px;"><input type="text" name="bOwnerId"
                                value="<?=strtoupper($detail['bOwnerId'])?>"></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><input type="text" name="bMoreStore" value="<?=$detail['bMoreStore']?>"
                                style="width:100%"></td>
                    </tr>
                    <?php endif?>
...
while (!$rs->EOF) {?>
                    <tr>
                        ...
                        <td colspan="3" style="width:600px;">
                            ...
                            <!-- <input type="text" name="bProperty" style="width:545px;" value="<?=$detail['bProperty']?>"> -->
                        </td>
                    </tr>
                ...
                <table cellspacing="0" cellspadding="2" style="width:800px;">
                    ...
                    <tr>
                        ...
                        <td style="width:200px;"><input type="text" class="dollars" name="bRealestateBalance"
                                value="<?=$detail['bRealestateBalance']?>"></td>
                        <td style="width:300px;">買方應付仲介服務費 </td>
                        <td style="width:100px;"><input type="button"
                                value="<?=($detail['bRealestateBalanceHide'] == 0) ? "隱藏" : "顯示";?>"
...
while (!$rs->EOF) {?>
                    <tr>
                        <td style="width:200px;">*<input type="text" name="TaxTitle[]"
                                value="<?=$rs->fields['cTaxTitle']?>"><input type="hidden" name="tax_id[]"
                                value="<?=$rs->fields['cId']?>"></td>
                        <td style="width:200px;"><input type="text" class="dollars" name="Tax[]"
                                value="<?=$rs->fields['cTax']?>"></td>
                        <td style="width:400px;" colspan="2"><input type="text" name="TaxRemark[]" maxlength="255"
                                style="width:300px;" value="<?=$rs->fields['cTaxRemark']?>"><input type="button"
                                onclick="del_tax(<?=$rs->fields['cId']?>)" value="刪除"></td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="4" style="width:800px;">&nbsp;&nbsp;<input type="text" name="other_remark_buyer"
                                style="width:700px;" value="<?=$detail['other_remark_buyer']?>"></td>
                    </tr>
                    <?php
...
while (!$rs->EOF) {?>
                    <tr>
                        <td colspan="3" style="width:700px;"><input type="text" name="data_other_remark[]"
                                style="width:700px;" value="<?=$rs->fields['cRemark']?>"><input type="hidden"
                                name="data_other_remark_id[]" value="<?=$rs->fields['cId']?>" />
                        </td>
                        <td><input type="button" onclick="del_remark(<?=$rs->fields['cId']?>)" value="刪除"></td>
                    </tr>
...
while (!$rs->EOF) {
    ...
    ?>
                        ...
                        <div class="gap" style="float:left;width:80px;">
                            <input type="text" value="<?=$rs->fields['cMoney']?>"
                                name='bankMoney1<?=$rs->fields['cId']?>' style="width:70px" />
                        </div>
                        <div class="gap">
...
?>
                    <div style="height:30px;width:800px;"><label><input type="checkbox" name="bNote" value="1"
                                <?=$bchecked?> />加入預售屋換約備註事項</label></div>
                </div>
            </div>
                ...
                <table cellspacing="0" cellspadding="2" style="width:800px;">
                    ...
                    <tr>
                        ...
                        <td style="width:200px;"><input type="text" name="cBuyerId"
                                value="<?=strtoupper($detail['cBuyerId'])?>"></td>
                    </tr>
                    <tr>
                        <td style="width:200px;">賣方姓名：</td>
                        ...
                        <td style="width:200px;"><input type="text" name="cOwnerId"
                                value="<?=strtoupper($detail['cOwnerId'])?>"></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><input type="text" name="cMoreStore" value="<?=$detail['cMoreStore']?>"
                                style="width:100%"></td>
                    </tr>
                    <?php endif?>
...
?>
                    ...
                    <tr>
                        ...
                        <td><input type="text" class="dollars" style="width:100px;" name="cInterest"
                                value="<?=$detail['cInterest']?>"></td>
                        <td><input type="text" class="dollars" style="width:100px;" disabled value="0"></td>
                        <?php if ($owner > 0): ?>
                        <td colspan="2">
                            <input type="text" style="width:108px;" class="" name="cAvgQue" value="<?=$cAvgQue?>">
                ...
                <table cellspacing="0" cellspadding="2" style="width:800px;">
                    ...
                    <tr style="display:;">
                        ...
                        <td style="width:400px;" colspan="2"><input type="text" name="balance_remark"
                                style="width:300px;" value="<?=$detail['balance_remark']?>"></td>
                    </tr>
                    <tr>
                        <td style="width:200px;">*應付仲介服務費餘額</td>
                        <td style="width:200px;"><input type="text" class="dollars" name="cRealestateBalance"
                                value="<?=$detail['cRealestateBalance']?>"></td>
                        <td style="width:300px;"><input type="text" name="realty_remark" style="width:300px;"
                                value="<?=$detail['realty_remark']?>"></td>
                        <td style="width:100px;"><input type="button"
                                value="<?=($detail['cRealestateBalanceHide'] == 0) ? "隱藏" : "顯示";?>"
...
while (!$rs->EOF) {?>
                    <tr>
                        ...
                        <td style="width:300px;"><input type="text" name="TaxRemark[]" maxlength="255"
                                style="width:300px;" value="<?=$rs->fields['cTaxRemark']?>"></td>
                        <td><input type="button" onclick="del_tax(<?=$rs->fields['cId']?>)" value="刪除"></td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="4" style="width:800px;">&nbsp;&nbsp;<input type="text" name="other_remark"
                                style="width:700px;" value="<?=$detail['other_remark']?>"></td>
                    </tr>
...
while (!$rs->EOF) {
    ...
    ?>
                        ...
                        <div class="gap" style="float:left;width:80px;">
                            <input type="text" value="<?=$rs->fields['cMoney']?>"
                                name='bankMoney2<?=$rs->fields['cId']?>' style="width:70px;">
                        </div>
                        <div class="gap" style="">
...
?>
                    <div style="height:30px;width:800px;"><label><input type="checkbox" name="cNote" id="" value="1"
                                <?=$cchecked?> />加入預售屋換約備註事項</label></div>
                </div>
            </div>

            <div style="height:30px;width:800px;">　連絡電話：<?=$company['tel']?>
                Ext.<?=$undertaker['Ext']?>　傳真電話：<?=$undertaker['FaxNum']?></div>
...
<!--                    <button id="view_btn" onclick="view()">--><?php //=$btn_str?><!--並預覽(合併)</button>-->
                    <button id="new_view_btn" onclick="view_new()">新版<?=$btn_str?>並預覽(合併)</button>
                    <?php endif?>
                    <?php if ($front_ok == 1): ?>
                    <button id="delete_uploadfile"
                        onclick="deleteUploadFileRecord(<?=$detail['cCertifiedId']?>)">收回發布</button>
                    <?php endif?>
        ...
        <form action="list_pdf.php?cCertifiedId=<?=$detail['cCertifiedId']?>&iden=o" method="GET" name="pdf_o"
            target="_blank">
            <input type="hidden" name="cCertifiedId" value="<?=$detail['cCertifiedId']?>"><input type="hidden"
                name="iden" value="o">
        </form>
    </div>
</body>
