...
$pdf->SetFontSize(12);
$title_txt = ($detail['bNote'] != 1) ? '專戶收支明細表暨點交確認單(賣方)' : '履約專戶收支明細表暨換約確認單(賣方)';
...
if ($detail['cInterestHidden'] == 0) {
    ...
    //
    if($owner > 0 and $detail['cAvgQue'] != '') {
        $ans1 = '□';
        $ans2 = '□';
        if($detail['cAvgAns'] == '1') $ans1 = '■';
        if($detail['cAvgAns'] == '0') $ans2 = '■';
        $pdf->Cell(68.75, $cell_y1, '', 0, 0);
        $pdf->SetFont('','B',12) ;
        $pdf->Cell(121.25, $cell_y1, $detail['cAvgQue'].':'.$ans1 . $detail['cAvgOption1'] . ' '.$ans2. $detail['cAvgOption2'] .' 說明:' . $detail['cAvgDesc'] , 0, 1);
        $pdf->SetFont('','',12) ;
    }
} else {
    if ($max_owner > 0) {
...
$pdf->SetFontSize(12);
$pdf->Cell(47.5, $cell_y4, '待扣款項明細'); // 結清付款項 Title
...
if($owner > 0 and $detail['cCertifyQue'] != '') {
    $ans1 = '□';
    $ans2 = '□';
    if($detail['cCertifyAns'] == '1') $ans1 = '■';
    if($detail['cCertifyAns'] == '0') $ans2 = '■';
    $pdf->SetFont('','B',12) ;
    $pdf->SetX(105);
    $pdf->Cell(30, $cell_y2, $detail['cCertifyQue'].':'.$ans1 . $detail['cCertifyOption1'] . ' '.$ans2. $detail['cCertifyOption2'] .' 說明:' . $detail['cCertifyDesc'] , 0, 1);
    $pdf->SetFont('','',12) ;
} else {
    $pdf->SetX(105);
...
for ($i = 0; $i < count($tax_owner); $i++) {
    ...
    $rows1 = ceil(mb_strlen($tax_owner[$i]['cTaxTitle']) / 30); //待扣款項明細 算字數&所需行數
    $rows2 = ceil(mb_strlen($tax_owner[$i]['cTaxRemark']) / 66); //備註 算字數&所需行數
...
while (!$rs->EOF) {
    $tmp = $rs->fields;
    //戶名是否需要換行
    if (strlen($tmp['cBankAccountName']) > 51) {
        $cell_y5 = 10;
    } else {
        $cell_y5 = 8;
    }
    ...
    if (strlen($tmp['cBankAccountName']) > 51) {
        $_y_ = $pdf->GetY();
        $_x_ = $pdf->GetX();
        $pdf->MultiCell(46.25, 5, $tmp['cBankAccountName'], 1); // 戶名
        $pdf->SetY($_y_);
        $pdf->SetX($_x_ + 46.25);
    } else {
    ...
    // $pdf->MultiCell(46.25,$cell_y5,$tmp['cBankAccountName'],1,1) ;
...
$pdf->Cell(185, $cell_y6, '第一建築經理股份有限公司以點交確認單為專戶結算之依據，請確認上述內容後，於下方簽章處簽名蓋章：', 0, 1);
$itemNo++;
...
$pdf->Cell(5, $cell_y6, $itemNo . '.');
$pdf->Cell(185, $cell_y6, '專戶結算後之保留款及未收取履約保證手續費案件，專戶價金不計息。', 0, 1);
