...
if ($_POST) {
	...
	if ($identity == 1) {
		...
		foreach ($list as $k => $v) {
			...
			$sql = "SELECT * FROM tFeedBackData WHERE fType = 1 AND fStoreId = '".$v['sId']."'";
			$rs = $conn->Execute($sql);
			$max = $rs->RecordCount();
			$check = feedbackMoney2($conn,$v['sId'],$year,$season);
			
			if ($check > 0 && $max == 0){ // 有回饋但沒有回饋資料
				$notfind[] = $v;
				echo 'AAAA';
				die;
			}
			
			unset($check);unset($max);
		}
	}
	...
	foreach ($list as $k => $v) {
		$sql = "SELECT * FROM tFeedBackData WHERE fType = 2 AND fStoreId = '".$v['bId']."'";
		$rs = $conn->Execute($sql);
		$max = $rs->RecordCount();		
...
$smarty->display('searchStoreFeedAcc.inc.tpl', '', 'report');
?>