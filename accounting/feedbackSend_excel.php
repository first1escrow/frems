...
$sql = '
	...
	WHERE
		tra.tObjKind IN ("點交(結案)","解除契約","建經發函終止")
...
for ($i=0; $i < count($list); $i++) { 
	...
	$data = array_merge($data,(array)$tmp);
	//撈取其他回饋對象
	$tmp2 = getFeedBackMoney($list[$i]['cCertifiedId']);
	if (is_array($tmp2)) {
		$data2 = array_merge($data2,(array)$tmp2);
	}
	##
...
for ($i = 0 ; $i < count($data) ; $i ++) {
	...
	if ($invert_result=='1') {		//顯示剔除資料
		...
		if ($data[$i]['cCaseFeedback']=='0') {			//要回饋	
			if ($data[$i]['cCaseFeedBackMoney'] > 0) {
				$list[$index] = $data[$i];	
				$list[$index]['cat'] = '回饋';
...
for ($i=0; $i < count($list); $i++) { 
	...
	if ($list[$i]['zip'] && $list[$i]['addr']) {
		...
		$check = checkRepeat($data[$list[$i]['zip'].$list[$i]['addr'].$list[$i]['fRtitle']],$list[$i]['code'],$list[$i]['type']); //1未建立資料 2 重複 3店編++
		// echo "<bR>".$check."<bR>";
		//有仲介跟地政同個地址，所以拆開來算
		if ($list[$i]['type'] == 'B') { //
			if ($check == 1) {
				$data[$list[$i]['zip'].$list[$i]['addr'].$list[$i]['fRtitle']]['data'.$list[$i]['type']] = $list[$i];
				$data[$list[$i]['zip'].$list[$i]['addr'].$list[$i]['fRtitle']]['data'.$list[$i]['type']]['realcode'][(int)substr($list[$i]['code'], 2)] = $list[$i]['code'];
		...
		if ($arr['cBranchNum'] > 0) {
			...
			if (is_array($tmp)) { //有N筆情況
				for ($j=0; $j < count($tmp); $j++) { 
					...
					$tmp_arr[$i]['cCaseFeedBackMoney'] = $arr['cCaseFeedBackMoney'];
					$tmp_arr[$i]['type'] = 'B';
					$i ++ ;
	...
	if ($arr['cSpCaseFeedBackMoney'] > 0 && (($arr['cBrand']!=2 && $arr['cBrand']!=49 && $arr['cBrand']!=1)||($arr['cBrand1']!=2 && $arr['cBrand1']!=49 && $arr['cBrand1']!=1)||($arr['cBrand2']!=2 && $arr['cBrand2']!=49 && $arr['cBrand2']!=1))) {
			...
			if (is_array($tmp)) { //有N筆情況
				for ($j=0; $j < count($tmp); $j++) { 
					...
					$tmp_arr[$i]['cCertifiedId'] = $arr['cCertifiedId'] ;
					$tmp_arr[$i]['cCaseFeedBackMoney'] = $arr['cSpCaseFeedBackMoney'];
		...
		if ($arr['cBranchNum1'] > 0) {	
			...
			if (is_array($tmp)) { //有N筆情況
				for ($j=0; $j < count($tmp); $j++) { 
					...
					$tmp_arr[$i]['cCaseFeedBackMoney'] = $arr['cCaseFeedBackMoney1'];
					$tmp_arr[$i]['type'] = 'B';
					$i ++ ;
		...
		if ($arr['cBranchNum2'] > 0) {
			...
			if (is_array($tmp)) {
				for ($j=0; $j < count($tmp); $j++) { 
					...
					$tmp_arr[$i]['cCaseFeedBackMoney'] = $arr['cCaseFeedBackMoney2'];
					$tmp_arr[$i]['type'] = 'B';
					$i ++ ;
