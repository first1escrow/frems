...
if ($_POST['search']) {
	...
	include_once 'feedbackSend_excel.php';
...
// 店名選單
$sql = 'SELECT bId,bStore FROM tBranch WHERE bId <> "0" ORDER BY bId ASC;' ;
$rs = $conn->Execute($sql);

while (!$rs->EOF) {
	
	$menu_branch .= '<option value="'.$rs->fields['bId'].'">'.$rs->fields['bStore']."</option>\n" ;
...
$smarty->display('feedbackSend.inc.tpl', '', 'accounting');
?> 
