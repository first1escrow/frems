...
$field = addslashes(trim($_REQUEST['f']));
/* Database parameter */
$aColumns       = array('rField',
                        'rName',
                        'rIdentity',
                        'rUnit', 
                        'rNo',
...
( 
	SELECT 
		rField,
        rName,
        case `rIdentity` when 1 then '一般人士' when 2 then '地政士' when 3 then '仲介' else '' END as rIdentity,
        rUnit, 
        rNo,
        rId
	FROM 
 ...
 */
$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS rId, " . str_replace(" , ", " ", implode(", ", $aColumns)) . "
		FROM   $sTable
		$sWhere 
		$sOrder
		$sLimit
