...
while (!$rs->EOF) {
	...
	if ($rs->fields['sUndertaker1'] != $rs->fields['OwnerId'] && $SESSION['member_test'] != 1 ) {//($rs->fields['tBankLoansDate'] >= $rs->fields['cat_etime'] && $rs->fields['cat_etime'] !='0000-00-00') || $rs->fields['cat_etime']=='0000-00-00' && ($rs->fields['tOwner'] !=33 ||$rs->fields['tOwner'] !=32)
...
##
$sql = "SELECT * FROM tPeopleBanktransError WHERE pDate >='".$date_start."' AND pDate <='".$date_end."'";
...
###
######結案數量統計表######
$sql = "SELECT * FROM tPeopleCaseEndError WHERE pDate >='".$date_start."' AND pDate <='".$date_end."'";
...
$sql = "SELECT * FROM tPeopleCaseEnd WHERE pDate >='".$date_start."' AND pDate <='".$date_end."'";
$rs = $conn->Execute($sql);
...
$objPHPExcel->getActiveSheet()->getStyle("A1:BO24")->getBorders()->getAllborders()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);

//寫入表頭資料
$objPHPExcel->getActiveSheet()->mergeCells("A1:BO3");
...
$col = 66;
$row = 4;
$objPHPExcel->getActiveSheet()->setCellValue('AC4','錯誤筆數小計/種類');
...
// $objPHPExcel->getActiveSheet()->getStyle('AA1:AA24')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
$objPHPExcel->getActiveSheet()->getStyle('A13:BO13')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
...
function checkCol2(){
	...
	$name = ($colTC == 0)? chr($col2): chr($colT).chr($col2);

	// echo $col2."_".$colTC."_".$name."<br>";
	return $name;
}
...
for ($i=0; $i < count($data_people); $i++) {
	...
	$objPHPExcel->getActiveSheet()->getStyle($colname.$row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
	$objPHPExcel->getActiveSheet()->getStyle($colname.$row)->getFill()->getStartColor()->setARGB('E4BEB1'); 
	$objPHPExcel->getActiveSheet()->getStyle($colname.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data_people[$i]['pName']);
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pBasic']);

	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pBanktran']);
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pMoney']);
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pBankBranch']);
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pTxt']);
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pAccount']);
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pAccountName']);
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pOther']);
	$row++;


	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pEnd']);
	$row++;
	//錯誤筆數合計
	...
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$total);
	$row++;

	##出款筆數##
	$row = 16;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,(int)$list[$data_people[$i]['pId']]['total']);
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,(int)$list[$data_people[$i]['pId']]['extra']);
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,(int)$list[$data_people[$i]['pId']]['unextra']);
	$row++;
	//出款筆數小計
	...
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$real_total);
	$row++;
	//結案筆數小計
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,(int)$list[$data_people[$i]['pId']]['pEndTotal']);
	$row++;
	...
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$real_total2);
	...
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$error."%");
	$row++;
	...
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,'錯誤說明');
	$row++;

	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(';',$data[$data_people[$i]['pId']]['pBasicMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(';',$data[$data_people[$i]['pId']]['pBanktranMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(';',$data[$data_people[$i]['pId']]['pMoneyMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(';',$data[$data_people[$i]['pId']]['pBankBranchMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(';',$data[$data_people[$i]['pId']]['pTxtMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(';',$data[$data_people[$i]['pId']]['pAccountMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(';',$data[$data_people[$i]['pId']]['pAccountNameMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(';',$data[$data_people[$i]['pId']]['pOtherMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data[$data_people[$i]['pId']]['pEndMsg']);
	$row++;
	//14  16-23  mergeCell
	// echo $row;
	// die;
	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row); //14
	$row++;$row++;
	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);//16
	$row++;
	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);//17
	$row++;
	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);//18
	$row++;
	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);//19
	$row++;
	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);//20
	$row++;
	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);//21
	$row++;
	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);//22
	$row++;
	$objPHPExcel->getActiveSheet()->mergeCells($colname.$row.":".$colname2.$row);//23
	$col++;$col++;
...
$tmp_msg = @implode("\r\n", $msg);
$objPHPExcel->getActiveSheet()->getStyle($colname.$row)->getAlignment()->setWrapText(true);
$objPHPExcel->getActiveSheet()->getStyle($colname.$row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle($colname.$row)->getFill()->getStartColor()->setARGB('F8EDEB'); 
$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$tmp_msg);
$row++;
...
if (is_array($error_count)) {
	foreach ($error_count as $k => $v) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,(int)$v);
		$row++;
	}
}
$objPHPExcel->getActiveSheet()->mergeCells("A15:BO15");
$objPHPExcel->getActiveSheet()->getStyle('A15:BO15')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle('A15:BO15')->getFill()->getStartColor()->setARGB('F8EDEB'); 
...
$objPHPExcel->getActiveSheet()->getStyle('A21:BO21')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle('A21:BO21')->getFill()->getStartColor()->setARGB('F8EDEB'); 
//加權後總筆數
$row = 22;
...
$objPHPExcel->getActiveSheet()->mergeCells("A24:BO24");
$objPHPExcel->getActiveSheet()->getStyle('A24:BO24')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle('A24:BO24')->getFill()->getStartColor()->setARGB('F8EDEB'); 
unset($error_count);unset($error_count2);unset($error);
// ########################################結案錯誤率##########################################################3
// //寫入表頭資料
$objPHPExcel->getActiveSheet()->mergeCells("A26:BO28");
$objPHPExcel->getActiveSheet()->getStyle("A26:BO37")->getBorders()->getAllborders()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
...
$objPHPExcel->getActiveSheet()->getStyle('B37:BO37')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
...
$colTC = 0;
$colname = checkCol();
$objPHPExcel->getActiveSheet()->setCellValue($colname.($row++),'買賣方資訊不完整');
$objPHPExcel->getActiveSheet()->setCellValue($colname.($row++),'履保費分配有異');
$objPHPExcel->getActiveSheet()->setCellValue($colname.($row++),'利息扣憑分配有異');
$objPHPExcel->getActiveSheet()->setCellValue($colname.($row++),'紙本發票應勾選而未勾選');
$objPHPExcel->getActiveSheet()->setCellValue($colname.($row++),'錯誤筆數合計');
$objPHPExcel->getActiveSheet()->mergeCells("A".$row.":BO".$row);
$objPHPExcel->getActiveSheet()->getStyle("A".$row.":BO".$row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle("A".$row.":BO".$row)->getFill()->getStartColor()->setARGB('F8EDEB'); 
$row++;//34
$objPHPExcel->getActiveSheet()->setCellValue($colname.($row++),'結案筆數');
$objPHPExcel->getActiveSheet()->setCellValue($colname.($row++),'錯誤率');
$objPHPExcel->getActiveSheet()->mergeCells("A".$row.":BO".$row);
$objPHPExcel->getActiveSheet()->getStyle("A".$row.":BO".$row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle("A".$row.":BO".$row)->getFill()->getStartColor()->setARGB('F8EDEB'); 
$row++;
...
for ($i=0; $i < count($data_people); $i++) { 
	...
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data2[$data_people[$i]['pId']]['pInfo']);
	$row++;

	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data2[$data_people[$i]['pId']]['pCMoney']);
	$row++;

	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data2[$data_people[$i]['pId']]['pInt']);
	$row++;

	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$data2[$data_people[$i]['pId']]['pInv']);
	$row++;
	//錯誤筆數
	...
	$row++;$row++;
	//結案筆數
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$list2[$data_people[$i]['pId']]);
	$row++;
	//錯誤率(錯誤筆數/結案筆數)
	...
	$colname = checkCol();
	//履保號碼
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,'履保號碼');
	$row++;
	
	// $objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode("\r\n", $data2[$data_people[$i]['pId']]['pInfoCertifiedId']));
	$objPHPExcel->getActiveSheet()->setCellValueExplicit($colname.$row, @implode(";", $data2[$data_people[$i]['pId']]['pInfoCertifiedId']),PHPExcel_Cell_DataType::TYPE_STRING); 
	
	$row++;

	// $objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode("\r\n", $data2[$data_people[$i]['pId']]['pCMoneyCertifiedId']));
	$objPHPExcel->getActiveSheet()->setCellValueExplicit($colname.$row, @implode(";", $data2[$data_people[$i]['pId']]['pCMoneyCertifiedId']),PHPExcel_Cell_DataType::TYPE_STRING); 
	
	$row++;

	// $objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode("\r\n", $data2[$data_people[$i]['pId']]['pIntCertifiedId']));
	$objPHPExcel->getActiveSheet()->setCellValueExplicit($colname.$row, @implode(";", $data2[$data_people[$i]['pId']]['pIntCertifiedId']),PHPExcel_Cell_DataType::TYPE_STRING); 
	
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValueExplicit($colname.$row, @implode(";", $data2[$data_people[$i]['pId']]['pInvCertifiedId']),PHPExcel_Cell_DataType::TYPE_STRING); 
	...
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(";", $data2[$data_people[$i]['pId']]['pInfoMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(";", $data2[$data_people[$i]['pId']]['pCMoneyMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(";", $data2[$data_people[$i]['pId']]['pIntMsg']));
	$row++;

	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,@implode(";", $data2[$data_people[$i]['pId']]['pInvMsg']));
	$row = 29;
	$col++;
...
//錯誤筆數小計/種類
$objPHPExcel->getActiveSheet()->setCellValue('AC'.$row,'錯誤筆數小計/種類');
$row++;
$col = 90;//
...
if (is_array($error_count)) {
	foreach ($error_count as $k => $v) {
		// if ($v== 0) {$v = "0";}
		$objPHPExcel->getActiveSheet()->setCellValue('AC'.$row,$v);
		$row++;
	}
}
$row++;
...
$objPHPExcel->getActiveSheet()->setCellValue('AC'.$row,$tmp."%");
unset($tmp);
#############################################################################################################
//季
...
//寫入表頭資料
$objPHPExcel->getActiveSheet()->getStyle("A1:E11")->getBorders()->getAllborders()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);

$objPHPExcel->getActiveSheet()->mergeCells("A1:E1");
$objPHPExcel->getActiveSheet()->getStyle("B1:B11")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
$objPHPExcel->getActiveSheet()->getStyle("D1:D11")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
...
for ($i=0; $i < count($data_people); $i++) {
	...
	// 	$error = 0;
	// }
	$error = round($error_part[$data_people[$i]['pId']]['total'] / $month_count,2);
	...
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,(int)$list[$data_people[$i]['pId']]['total_plus']);
	$col++;
	$colname = checkCol();
	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,(int)$list[$data_people[$i]['pId']]['extra_plus']);
	$col++;
	$colname = checkCol();

	$objPHPExcel->getActiveSheet()->setCellValue($colname.$row,$error_part[$data_people[$i]['pId']]['total'].'%');
	$col++;
	$colname = checkCol();
	//$error_part
	$row++;
...
//寫入表頭資料
$objPHPExcel->getActiveSheet()->getStyle("A1:D11")->getBorders()->getAllborders()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
...
for ($i=0; $i < count($data_people); $i++) {
	...
	$error = round($error_total2[$data_people[$i]['pId']]['total'] / $month_count,2);
