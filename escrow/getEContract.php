...
require_once dirname(dirname(__FILE__)) . '/class/advance.class.php';
...
include_once dirname(dirname(__FILE__)) . '/bank/report/calTax.php';
...
$vr_code = $_POST['code'];
// $vr_code = '60001101152515';
// $vr_code = '96988100088173';
$msg = array();
##
//已使用
...
$rs                  = $conn->Execute($sql);
$bankCode            = $rs->fields;
$data['certifiedid'] = substr($vr_code, -9); //保證號碼9碼

$prefix = substr($vr_code, 0, 5);
if (in_array($prefix, ['99985', '99986'])) { //永豐需多加1碼
    $prefix .= '0';
}

$sql = 'SELECT cBankCode FROM tContractBank WHERE cBankVR = "' . $prefix . '";';
$rs  = $conn->Execute($sql);
if ($rs->EOF) {
    $msg = [
        'code' => 400,
    ...
    $data['case_bank'] = $rs->fields['cBankCode'];
}
...
if ($bankCode['bApplication'] == 1) { //土地
    ...
    //土地標示
    $sql = "SELECT * FROM _web_land_subject_a WHERE deleted_at is null AND contract_id = '" . $eContractData['contract_id'] . "'";
    ...
    if (!$rs->EOF) {
        while (!$rs->EOF) {
            ...
            $land['land_area']             = ''; //使用分區
            $land['lpower1']               = $rs->fields['right1'];
            $land['lpower2']               = $rs->fields['right2'];
            ...
            $landOwnerData = array();
            $landData      = array();
            $landOwnerData = getLandOwner($rs->fields['land_owner_id']);
            $landData      = getLand($rs->fields['land_id']);
            $json          = json_decode($landOwnerData['lPreviousTransferValue'], true);

            $land['land_money'] = $landOwnerData['lLandPrice']; //土地現值
            $land['land_area']  = getLandCategory($landData['lUseDistricts'], 'cId'); //使用分區

            $contract->AddLand($land, $i); //新增土地
            // lPreviousTransferValue

            //$value['previousTransferValue']
            foreach ($json as $key => $value) {
                $landPriceData                 = array();
                $power                         = explode('/', $value['ownerBatchesScope']);
                $price                         = explode('元', $value['previousTransferValue']);
                $landPriceData['cCertifiedId'] = $data['certifiedid'];
                $landPriceData['cLandItem']    = $i;
                $landPriceData['cItem']        = $key;
                $landPriceData['cMoveDate']    = $value['transferDate'];
                $landPriceData['cLandPrice']   = $price[0];
                $landPriceData['cPower1']      = $power[0];
                $landPriceData['cPower2']      = $power[1];

                unset($power);unset($price);
                $contract->addLandPrice($landPriceData);
            ...
            if(null == $json) {
                for ($j = 0; $j <= 1; $j++) {
                    ...
                    $dataPrice['cLandItem']    = 0;
                    $dataPrice['cItem']        = $j;
                    $dataPrice['cMoveDate']    = '0000-00-00';
                    $dataPrice['cLandPrice']   = 0;
                    $dataPrice['cPower1']      = 0;
                    $dataPrice['cPower2']      = 0;

                    $contract->addLandPrice($dataPrice);
                    unset($dataPrice);
                }
            }

            unset($landOwnerData);unset($json);
    ...
    $sql = "SELECT * FROM _web_land_subject_b WHERE deleted_at is null AND contract_id = '" . $eContractData['contract_id'] . "'";
    $rs  = $conn22->Execute($sql);
    $i   = 0;
    while (!$rs->EOF) {
        $build                                                        = array();
    ...
    $build                = array();
    $build['certifiedid'] = $data['certifiedid'];
    $saleDate = explode('-', $eContractData['sale_date']);
    $saleDate[0] += 1911; //換算西元年
    $build['property_closingday'] = implode('-', $saleDate);

    $contract->AddProperty($build);
    ...
    //房地標的-土地標示
    $sql = "SELECT * FROM _web_subject_a WHERE deleted_at is null AND contract_id = '" . $eContractData['contract_id'] . "'";
    ...
    //房地標的-建物
    $sql = "SELECT * FROM _web_subject_b WHERE deleted_at is null AND contract_id = '" . $eContractData['contract_id'] . "' ORDER BY id ASC";
    // echo $sql;
    ...
    while (!$rs->EOF) {
        ...
        $build['new_property_Item'][$i]          = $i;
        $build['new_property_levelnow' . $i]     = $rs->fields['floors']; //樓層$rs->fields['level_allarea']
        ...
        $build['new_property_builddate' . $i]    = ''; //建築完成日期
        $build['new_property_buildage' . $i]     = ''; //屋齡

        //樓層
        $level      = json_decode($rs->fields['level']);
        $level_area = json_decode($rs->fields['level_area']);
        $main_right1 = json_decode($rs->fields['main_right1']);
        $main_right2 = json_decode($rs->fields['main_right2']);
        ...
        for ($j = 0; $j < count($level); $j++) {
            ...
            $build['new_property_measuretotal' . $i] += $new_cMeasureMain;
            ...
            $sql = "INSERT INTO
					...
					SET
						...
						cPower1 = '" . $main_right1[$j] . "',
						cPower2 = '" . $main_right2[$j] . "',
						cBuildItem = '" . $i . "'
						";
            $conn->Execute($sql);
            $propertyObjectCount++;
        ...
        $build['new_property_measuretotal' . $i] += $new_cMeasureMain;
        if (!empty($rs->fields['level_arcade'])) {
            $sql = "INSERT INTO
						tContractPropertyObject
					SET
						cCertifiedId = '" . $data['certifiedid'] . "',
        ...
        $attach_right1 = json_decode($rs->fields['attach_right1']);
        $attach_right2 = json_decode($rs->fields['attach_right2']);
        ...
        for ($j = 0; $j < count($attach_use); $j++) {
            ...
            $new_cMeasureMain = round($attach_area[$j] * ($rs->fields['level_right1'] / $rs->fields['level_right2']), 2);
            $build['new_property_measuretotal' . $i] += $new_cMeasureMain;
            $sql = "INSERT INTO
						tContractPropertyObject
					SET
						...
						cMeasureMain = '" . $new_cMeasureMain . "',
						cMeasureTotal = '" . $attach_area[$j] . "',
						cPower1 = '" . $attach_right1[$j] . "',
						cPower2 = '" . $attach_right2[$j] . "',
						cBuildItem = '" . $i . "'
						";
            // echo $sql."<br>";
            $conn->Execute($sql);
        ...
        $right_area   = json_decode($rs->fields['right_area']);
        $right_share1 = json_decode($rs->fields['right_share1']);
        $right_share2 = json_decode($rs->fields['right_share2']);
        $right_num    = json_decode($rs->fields['right_num']);

        for ($j = 0; $j < count($right_area); $j++) {
            if ($right_share2[$j] == '' || $right_share2[$j] == 0) {
                $right_share2[$j] = 1;
            }

            if ($right_share1[$j] == '' || $right_share1[$j] == 0) {
                $right_share1[$j] = 1;
            }

            $right_area[$j] = str_replace(',', '', $right_area[$j]);

            $new_cMeasureMain = round($right_area[$j] * ($right_share1[$j] / $right_share2[$j]), 2);
            $build['new_property_measuretotal' . $i] += $new_cMeasureMain;
            $sql = "INSERT INTO
						tContractPropertyObject
					SET
						...
						cItem = '" . $propertyObjectCount . "',
						cCategory = '3',
						cLevelUse = '" . $right_num[$j] . "',
						cMeasureMain = '" . $new_cMeasureMain . "',
						cMeasureTotal = '" . $right_area[$j] . "',
						cPower1 = '" . $right_share1[$j] . "',
						cPower2 = '" . $right_share2[$j] . "',
						cBuildItem = '" . $i . "'
						";
            // echo $sql."<br>";
            $conn->Execute($sql);
        ...
        $build['property_Item']         = $i;
        $build['property_budmaterial']  = '';
        $build['property_builddate']    = ''; //建築完成日期
        $build['property_levelnow']     = $build['new_property_levelnow' . $i];
        $build['property_levelhighter'] = $build['new_property_levelhighter' . $i];
        $build['property_zip']          = $build['new_property_zip' . $i];
        $build['property_addr']         = $build['new_property_addr' . $i];
        $build['property_measuretotal'] = $build['new_property_measuretotal' . $i];
        $build['property_buildno']      = $build['new_property_buildno' . $i];
        $build['property_power10']      = $build['new_property_power1' . $i];
        $build['property_power20']      = $build['new_property_power2' . $i];
        $build['property_builddate']    = $build['new_property_builddate' . $i]; //建築完成日期
        $build['property_buildage']     = $build['new_property_buildage' . $i]; //屋齡
        $build['property_closingday']   = $build['new_property_buildage' . $i];
        $saleDate = explode('-', $eContractData['sale_date']);
        $saleDate[0] += 1911; //換算西元年
        $build['property_closingday'] = implode('-', $saleDate);
        if($rs->fields['use'] == 999) {
            $build['property_cIsOther']  = 1; //主要用途其他
            $build['property_cOther']    = $rs->fields['use_other']; //主要用途其他
        } else {
            $objuse[] = $rs->fields['use'];
            $build['property_objuse']    = $objuse; //主要用途
        }

        $build['property_budmaterial'] = $rs->fields['bud_material']; //主要建材

        $contract->AddProperty($build); //建物
...
// 停車位
$sql = "SELECT * FROM _web_subject_c WHERE deleted_at is null AND contract_id = '" . $eContractData['contract_id'] . "'";
$rs  = $conn22->Execute($sql);
if (!$rs->EOF) {
...
if($bankCode['bApplication'] == 2) {
    $data['income_land']        = $eContractData['land_money']; //土地
    $data['income_building']    = $eContractData['building_money']; //建物
}
...
//買賣方
$sql        = "SELECT * FROM _web_trader WHERE deleted_at is null AND contract_id = '" . $eContractData['contract_id'] . "'";
$rs         = $conn22->Execute($sql);
$buyerCount = 0; //買方數量
$ownerCount = 0; //賣方數量
...
if (!$rs->EOF) {
    while (!$rs->EOF) {
        ...
        $rs->fields['birthday'] = str_replace('/', '-', $rs->fields['birthday']);
        $customer['cBirthdayDay'] = $advance->ConvertDateToAD($rs->fields['birthday'], 2);
...
##
//土增稅
//特定農業區、一般農業區、工業區、鄉村區、森林區、山坡地保育區、風景區、國家公園區、河川區、海域區
$cal = calCase($data['certifiedid']);
// echo $cal;
$sql = "UPDATE tContractIncome SET cAddedTaxMoney = '" . $cal . "' WHERE cCertifiedId = '" . $data['certifiedid'] . "'";
// echo $cal;
$conn->Execute($sql);
...
//土地所有權部
function getLandOwner($land_owner_id)
{
    global $conn22;

    $data = array();
    $sql  = "SELECT * FROM tLand_Owner WHERE lId = '" . $land_owner_id . "'";
    $rs   = $conn22->Execute($sql);
    $data = $rs->fields;
...
//土地標示部
function getLand($land_id)
{
    global $conn22;

    $data = array();
    $sql  = "SELECT * FROM tLand WHERE lId = '" . $land_id . "'";
    $rs   = $conn22->Execute($sql);
    $data = $rs->fields;
    return $data;
}
//使用分區
function getLandCategory($name, $field)
{
    global $conn;

    $value = '';
    $sql   = "SELECT * FROM tCategoryArea WHERE cName = '" . $name . "'";
    // echo $sql;
    $rs    = $conn->Execute($sql);
