...
$oldCertifiedId = $_POST['oCertifiedId']; //舊保證號碼
$newCertifiedId = substr($_POST['scrivener_bankaccount'], -9); //新保證號碼
...
for ($i = 0; $i < count($_POST['cat']); $i++) {
    switch ($_POST['cat'][$i]) {
        case '1': //案件明細表(只複製客服內容)[作廢]
            // ContractService($oldCertifiedId,$newCertifiedId);
            break;
        case '2': //合約書
        ...
        case '3': //土地
            ...
            break;
        case '4': //建物
            ContractProperty($oldCertifiedId, $newCertifiedId);
            ContractPropertyObject($oldCertifiedId, $newCertifiedId);
            break;
        case '5': //地政士 [作廢]
            # code...
            break;
...
function ContractService($old, $new)
{
    global $conn;

    $sql = "SELECT * FROM tContractService WHERE cCertifiedId = '" . $old . "'";
    $rs  = $conn->Execute($sql);
    ...
    for ($i = 0; $i < count($data); $i++) {
        $sql = "INSERT INTO
                ...
                SELECT
                    (SELECT max(cId) FROM tContractService)+1,
                        '" . $new . "',
                        '" . date('Y-m-d H:i:s') . "',
                        cName,
                        cNote,
                        ...
                        '0'
                    FROM
                        tContractService WHERE cId='" . $data[$i]['cId'] . "' AND cDel = 0";
        $conn->Execute($sql);
    }
}
...
function ContractRent($old, $new)
{
    global $conn;

    $sql = "SELECT * FROM tContractRent WHERE cCertifiedId = '" . $old . "'";
    $rs  = $conn->Execute($sql);

    $sql = "UPDATE
                tContractRent
            SET
                cPropertyRendType ='" . $rs->fields['cPropertyRendType'] . "',
                cRentDate = '" . $rs->fields['cRentDate'] . "',
                cRent ='" . $rs->fields['cRent'] . "',
                cFinish = '" . $rs->fields['cFinish'] . "',
                cOther = '" . $rs->fields['cOther'] . "'
            WHERE
                cCertifiedId = '" . $new . "'";
    $conn->Execute($sql);
}

function ContractParking($old, $new)
{
    global $conn;

    $sql = "SELECT * FROM tContractParking WHERE cCertifiedId = '" . $old . "'";
    $rs  = $conn->Execute($sql);
    ...
    if ($total > 0) {
        $sql = "INSERT INTO
            ...
            SELECT (
                    SELECT max(cId) FROM tContractParking)+1,
                        '" . $new . "',
                        cTotal,
                        cGround,
                        cFloor,
                        ...
                        cOther
                    FROM
                        tContractParking WHERE cId='" . $data['cId'] . "'";
        $conn->Execute($sql);
    }
}

function ContractProperty($old, $new)
{
    global $conn;
...
function ContractPropertyObject($old, $new)
{
    global $conn;

    $sql = "SELECT * FROM tContractPropertyObject WHERE cCertifiedId = '" . $old . "'";
    $rs  = $conn->Execute($sql);
...
function ContractAscription($old, $new)
{
    global $conn;

    $sql = "SELECT * FROM tContractAscription WHERE cCertifiedId = '" . $old . "'";
    $rs  = $conn->Execute($sql);
    ...
    $sql = "UPDATE
                tContractAscription
            SET
                cContribute = '" . $data['cContribute'] . "',
                cBuyer = '" . $data['cBuyer'] . "',
                cBuyerOther = '" . $data['cBuyerOther'] . "',
                cOwner = '" . $data['cOwner'] . "',
                cOwnerOther = '" . $data['cOwnerOther'] . "',
                cBuyerCertifiedmoney = '" . $data['cBuyerCertifiedmoney'] . "',
                cOwnerCertifiedmoney = '" . $data['cOwnerCertifiedmoney'] . "'
            WHERE
                cCertifiedId = '" . $new . "'";
    $conn->Execute($sql);
}
...
function ContractNote($old, $new)
{
    global $conn;

    $sql = "SELECT * FROM tContractNote WHERE cCertifiedId = '" . $old . "'";
    $rs  = $conn->Execute($sql);
    ...
    for ($i = 0; $i < count($data); $i++) {
        $sql = "INSERT INTO
                ...
                SELECT
                        (SELECT max(cId) FROM tContractNote)+1,
                        '" . $new . "',
                         cCategory,
                        cNote,
                        '" . date('Y-m-d H:i:s') . "',
                        cDel
                    FROM
                        tContractNote WHERE cId='" . $data[$i]['cId'] . "' AND cDel = 0";
        $conn->Execute($sql);
    }
}
