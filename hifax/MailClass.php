...
//                    mb_convert_encoding($subSubject, 'utf-8' ,'gb2312,ISO-2022-JP');
     ...
     * @return array
     */
    public function getAttach($msgCount,$path,$fname='') {
    ...
    public function mailRead($msgCount) {
        $status = imap_setflag_full($this->_connect, $msgCount , "\\Seen \\Flagged");
        return $status;
    }
    public function mailMove($msgCount){
        $imapresult=imap_mail_move($this->_connect,$msgCount,'INBOX/Saved');
        return $imapresult;
    }
    /**
