...
//取得代書基本資料
$sql = 'SELECT * FROM tScrivener ORDER BY sName ASC;' ;
$rs = $conn->Execute($sql) ;
$i = 0 ;
while (!$rs->EOF) {
	$scr[$i++] = $rs->fields ;
...
<div id="scrvener" style="padding-bottom:10px;">
	地政士姓名：
	<select name="sId" onchange="chgScr()">
	<?php
	for ($i = 0 ; $i < count($scr) ; $i ++) {
		echo '<option value="'.$scr[$i]['sId'].'"' ;
		if ($scr[$i]['sId'] == $sId) {
			echo ' selected="selected"' ;
		}
		echo '>'.$scr[$i]['sName']."</option>\n" ;
	}
	?>
	</select>　
</div>
