...
//刪除
    ...
    $delno = $_POST['delno'];
    $sql   = 'DELETE FROM tIndividualFeedSms WHERE iId="' . $delno . '" AND iIndividual="' . $iId . '";';
    $conn->Execute($sql);
}
##
...
<table border="0" width="100%">
	<tr>
		<th colspan="6"><center>回饋金出款簡訊對象資料</center></th>
	</tr>
	<tr>
...
foreach ($list as $k => $v) {
    ?>
		<tr>
			<td><input type="hidden" name="smsId[]" value="<?=$v['iId']?>">
				<select name="bNID[]">
                    <option></option>
				<?php
for ($i = 0; $i < count($pos); $i++) {
    ...
    ?>
			...
			<td><input type="text" class="input-text-per" name="bName[]" value="<?=$v['iName']?>"></td>
			<td><input type="text" class="input-text-per" name="bMobile[]" maxlength="10" value="<?=$v['iMobile']?>"></td>
			<td><input type="button" value="刪除" onclick="del('<?=$v['iId']?>','<?=$v['iMobile']?>')"></td>
		</tr>
	<?php
}
...
<form name="form_back" id="form_back" method="POST"  action="individualfeedbackedit.php">
