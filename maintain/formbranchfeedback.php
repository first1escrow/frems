...
//刪除
    ...
    $delno = $_POST['delno'];
    $sql   = 'DELETE FROM tBranchFeedback WHERE bId="' . $delno . '" AND bBranch="' . $bId . '";';
    $conn->Execute($sql);
}
##
...
foreach ($list as $k => $v) {
    ?>
		<tr>
			...
			<td><input type="hidden" name="smsId[]" value="<?=$v['bId']?>">
				<select name="bNID[]">
                    <option></option>
				<?php
for ($i = 0; $i < count($pos); $i++) {
    ...
    ?>
			...
			<td><input type="button" value="同步" onclick="copyToFeedBackSms(<?=$v['bId']?>,4)"></td>
			<td><input type="button" value="刪除" onclick="del('<?=$v['bId']?>','<?=$v['bMobile']?>')"></td>
		</tr>
	<?php
}
