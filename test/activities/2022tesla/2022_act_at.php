...
$fh = fopen('AT' . date("Ymd") . '.csv', 'w');
fwrite($fh, "\xEF\xBB\xBF");
fwrite($fh, '保證號碼,簽約日期,建檔日期,案件狀態,狀態時間' . "\r\n");
for ($i = 0; $i < count($list); $i++) {
    if (preg_match("/直營/", $list[$i]['cate'])) {
        fwrite($fh, $list[$i]['cCertifiedId'] . '_,' . $list[$i]['cSignDate'] . ',' . $list[$i]['cApplyDate'] . ',' . $list[$i]['caseStatus'] . ',' . $list[$i]['cFinishDate3'] . "\r\n");
    }
}
fclose($fh);
