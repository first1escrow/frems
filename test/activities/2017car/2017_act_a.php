...
##########
$fromDate = '2017-11-01 00:00:00' ;


$toDate = '2018-02-28 23:59:59' ;
	...
	if ($fg > 0) {
		...
		if ($rs->fields['cBranchNum1'] > 0 ) {
			...
			if ($rs->fields['cServiceTarget1'] == 3) {
				...
				$list[$i]['buyStore'] = ($list[$i]['buyStore'] != '' )? $list[$i]['buyStore']:$list[$i]['brand1'].$list[$i]['branch1']; //1.買賣方、2.賣方、3.買方
				$list[$i]['ownStore'] = ($list[$i]['ownStore'] != '')? $list[$i]['ownStore']:$list[$i]['brand1'].$list[$i]['branch1'];
			}
		}
		// if ($rs->fields['cBranchNum2'] > 0 && $rs->fields['cBrand2'] == '1') {
		// 	if ($rs->fields['cBrand2'] == '1') {
		// 		if ($rs->fields['bCategory2'] == 1) {
		// 			$tmp[] = '加盟';
		// 		}elseif ($rs->fields['bCategory2'] == 2) {
		// 			$tmp[] = '直營';
		// 		}
		// 	}
		// 	if ($rs->fileds['cServiceTarget2'] == 3) {
		// 		$list[$i]['buyStore'] = $list[$i]['brand2'].$list[$i]['branch2']; //1.買賣方、2.賣方、3.買方
		// 	}elseif($rs->fileds['cServiceTarget2'] == 2){
		// 		$list[$i]['ownStore'] = $list[$i]['brand2'].$list[$i]['branch2'];
		// 	}else{
		// 		$list[$i]['buyStore'] = $list[$i]['brand2'].$list[$i]['branch2']; //1.買賣方、2.賣方、3.買方
		// 		$list[$i]['ownStore'] = $list[$i]['brand2'].$list[$i]['branch2'];
		// 	}
...
//print_r($list) ; exit ;
$fh = fopen('A'.date("Ymd").'.csv','w') ;
fwrite($fh,'保證號碼,簽約日期,買方身份證字號,賣方身份證字號,物件地址,類型,買方姓名,賣方姓名,買方電話,賣方電話,買方仲介店,賣方仲介店'."\r\n") ;
for ($i = 0 ; $i < count($list) ; $i ++) {

	if (preg_match("/直營/", $list[$i]['cate'])) {
		fwrite($fh,$list[$i]['cCertifiedId'].'_,'.$list[$i]['cSignDate'].','.$list[$i]['buyerId'].','.$list[$i]['ownerId'].','.$list[$i]['cAddr'].','.$list[$i]['cate'].','.$list[$i]['buyerName'].','.$list[$i]['ownerName'].','.$list[$i]['buymobile'].','.$list[$i]['ownmobile'].','.$list[$i]['buyStore'].','.$list[$i]['ownStore']."\r\n") ;
	}
