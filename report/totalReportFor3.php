	...
	while (!$rs->EOF) {
		...
		if ($sales) {
			...
			if ($CaseCount['countPart'] > 0) { //有分配到該業務
				...
				// echo $rs->fields['cCertifiedId']."_".$count."<br>";
...
function getContractSales($data){ //業務歸屬該計算業務
	...
	// }
		...
		// if ($sales == 34 && $salesCountData['countPart'] > 0) {
		// 	$salesCountData['countPart'] = 1;
		// 	$salesCountData['count'] =1;
		// }

	// if ($data['cCertifiedId'] == '060373477') {
	// 	print_r($salesCountData);
