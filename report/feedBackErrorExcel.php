...
if ($cat == 1 || $cat == 2 || $cat == 3) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('B' . $row, '保證號碼');
    $objPHPExcel->getActiveSheet()->setCellValue('C' . $row, '仲介店名');
    $objPHPExcel->getActiveSheet()->setCellValue('D' . $row, '地政士');
    $objPHPExcel->getActiveSheet()->setCellValue('E' . $row, '總價金');
    $objPHPExcel->getActiveSheet()->setCellValue('F' . $row, '合約保證費');
    $objPHPExcel->getActiveSheet()->setCellValue('G' . $row, '仲介回饋金額');
    $objPHPExcel->getActiveSheet()->setCellValue('H' . $row, '地政士回饋金額');
    $objPHPExcel->getActiveSheet()->setCellValue('I' . $row, '地政士特殊回饋金額');
    $objPHPExcel->getActiveSheet()->setCellValue('J' . $row, '其他回饋金');
    $objPHPExcel->getActiveSheet()->setCellValue('K' . $row, '業績歸屬');
    $objPHPExcel->getActiveSheet()->setCellValue('L' . $row, '收件人(地政士)');
    $objPHPExcel->getActiveSheet()->setCellValue('M' . $row, '寄送地址(地政士)');
    $objPHPExcel->getActiveSheet()->setCellValue('N' . $row, '收件人(仲介)');
    $objPHPExcel->getActiveSheet()->setCellValue('O' . $row, '寄送地址(仲介)');
    $objPHPExcel->getActiveSheet()->setCellValue('P' . $row, '其他回饋對象');
    $objPHPExcel->getActiveSheet()->setCellValue('Q' . $row, '寄送地址(其他)');
    $objPHPExcel->getActiveSheet()->setCellValue('R' . $row, '總回饋比例');
    $objPHPExcel->getActiveSheet()->setCellValue('S' . $row, '履保費出款日');
    $objPHPExcel->getActiveSheet()->setCellValue('T' . $row, '回饋金備註');
    $objPHPExcel->getActiveSheet()->setCellValue('U' . $row, '績效業務');
    $row++;

    $no = 1;
    for ($i = 0; $i < $max; $i++) {
        $color = ($arr[$i]['cBranchNum1'] > 0 || $arr[$i]['cBranchNum2'] > 0 || $arr[$i]['cBranchNum3'] > 0 || $arr[$i]['ScrivenerSPFeedMoney'] > 0 || count($arr[$i]['otherFeed']) > 0) ? "FFB630" : "FFFFFF";
        ...
        if ($arr[$i]['cBranchNum'] > 0) {
            ...
            $feedData['bRenote']   = $arr[$i]['bRenote'];
        ...
        if ($arr[$i]['cBranchNum1'] > 0) {
            ...
            $feedData['bRenote']   = $arr[$i]['bRenote1'];
        ...
        if ($arr[$i]['cBranchNum2'] > 0) {
            ...
            $feedData['bRenote']   = $arr[$i]['bRenote2'];
        ...
        if ($arr[$i]['cBranchNum3'] > 0) {
            $feedData['storeName'] = $arr[$i]['bCode3'] . str_pad($arr[$i]['cBranchNum3'], 5, 0, STR_PAD_LEFT) . $arr[$i]['BrandName3'] . $arr[$i]['BranchName3'];
            $feedData['storeId']   = ($arr[$i]['cBranchNum3'] == 505 || $arr[$i]['cFeedbackTarget3'] == 2) ? $arr[$i]['cScrivener'] : $arr[$i]['cBranchNum3'];
            $feedData['sales']     = getSales($arr[$i]['cCertifiedId'], $arr[$i]['cBranchNum3'], $arr[$i]['cFeedbackTarget3'], $arr[$i]['cScrivener']);
            $feedData['bRenote']   = $arr[$i]['bRenote3'];

            if ($arr[$i]['cCaseFeedback3'] == 0) {
                if ($arr[$i]['cFeedbackTarget3'] == 1) {
                    $feedData['BranchFeedMoney'] = $arr[$i]['cCaseFeedBackMoney3'];
                } else {
                    $feedData['ScrivenerFeedMoney'] = $arr[$i]['cCaseFeedBackMoney3'];
                }

                $feedData['FeedbackTargetMark'] = ($arr[$i]['cFeedbackTarget3'] == 2 && $arr[$i]['cBranchNum3'] != 505) ? 2 : 1;
            }
        ...
        if ($arr[$i]['ScrivenerSPFeedMoney'] > 0) {
            ...
            $feedData['bRenote']              = $arr[$i]['sRenote'];
...
if (is_array($sp)) {
    foreach ($sp as $k => $v) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('A' . $row, '序號');
        $objPHPExcel->getActiveSheet()->setCellValue('B' . $row, '保證號碼');
        $objPHPExcel->getActiveSheet()->setCellValue('C' . $row, '仲介店編號');
        $objPHPExcel->getActiveSheet()->setCellValue('D' . $row, '仲介店名');
        $objPHPExcel->getActiveSheet()->setCellValue('E' . $row, '賣方');
        $objPHPExcel->getActiveSheet()->setCellValue('F' . $row, '買方');
        $objPHPExcel->getActiveSheet()->setCellValue('G' . $row, '總價金');
        $objPHPExcel->getActiveSheet()->setCellValue('H' . $row, '合約保證費');
        $objPHPExcel->getActiveSheet()->setCellValue('I' . $row, '簽約日期');
        $objPHPExcel->getActiveSheet()->setCellValue('J' . $row, '案件狀態日期');
        $objPHPExcel->getActiveSheet()->setCellValue('K' . $row, '標的物座落');
        $objPHPExcel->getActiveSheet()->setCellValue('L' . $row, '回饋' . $arrayCategory2[$v]['bRecall'] . '%');
        $objPHPExcel->getActiveSheet()->setCellValue('M' . $row, '回饋金備註');
        $row++;
        ...
        if (is_array($dataSp[$FeedBackId])) {
            foreach ($dataSp[$FeedBackId] as $key => $value) {
                ...
                if ($value['cBranchNum3'] > 0) {
                    $branchCode .= ',' . $value['bCode3'] . str_pad($value['cBranchNum3'], 5, '0', STR_PAD_LEFT);
                    $branchName .= ',' . $value['BranchName3'];
                    if ($FeedBackType == 'b') {
                        if ($value['cBrand3'] != $FeedBackId) {
                            $checkCount++;
                        }
                    } elseif ($FeedBackType == 'g') {
                        if ($value['BranchGroup3'] != $FeedBackId) {
                            $checkCount++;
                        }
                    }
                }
                ...
                $objPHPExcel->getActiveSheet()->setCellValueExplicit('B' . $row, $value['cCertifiedId'], PHPExcel_Cell_DataType::TYPE_STRING);

                $objPHPExcel->getActiveSheet()->setCellValue('C' . $row, $branchCode);
                $objPHPExcel->getActiveSheet()->setCellValue('D' . $row, $branchName);
                $objPHPExcel->getActiveSheet()->setCellValue('E' . $row, $value['owner']);
                $objPHPExcel->getActiveSheet()->setCellValue('F' . $row, $value['buyer']);
                $objPHPExcel->getActiveSheet()->setCellValue('G' . $row, $value['cTotalMoney']);
                $objPHPExcel->getActiveSheet()->setCellValue('H' . $row, $value['cCertifiedMoney']);
                $objPHPExcel->getActiveSheet()->setCellValue('I' . $row, substr($value['cSignDate'], 0, 10));
                $objPHPExcel->getActiveSheet()->setCellValue('J' . $row, $date);
                $objPHPExcel->getActiveSheet()->setCellValue('K' . $row, $addr);
                $objPHPExcel->getActiveSheet()->setCellValue('L' . $row, $money);
...
function SameRowAdd($data, $row, $no, $color, $feedData)
{
    ...
    $objPHPExcel->getActiveSheet()->setCellValueExplicit('B' . $row, $data['cCertifiedId'], PHPExcel_Cell_DataType::TYPE_STRING);
    $objPHPExcel->getActiveSheet()->setCellValue('C' . $row, $feedData['storeName']);
    $objPHPExcel->getActiveSheet()->setCellValue('D' . $row, $data['sName'] . "(" . $data['sOffice'] . ")");
    $objPHPExcel->getActiveSheet()->setCellValue('E' . $row, $data['cTotalMoney']);
    ...
    if (($tmp - $tmp2) > $data['cCertifiedMoney']) { //合約保證費 如果未達6/10000的合約保證費  在合約保證費的金額位置 加註星星
        $objPHPExcel->getActiveSheet()->setCellValue('F' . $row, '*' . $data['cCertifiedMoney']);
    } else {
        $objPHPExcel->getActiveSheet()->setCellValue('F' . $row, $data['cCertifiedMoney']);
    }

    $objPHPExcel->getActiveSheet()->setCellValue('G' . $row, $feedData['BranchFeedMoney']);
    $objPHPExcel->getActiveSheet()->setCellValue('H' . $row, $feedData['ScrivenerFeedMoney']);
    ...
    if ($feedData['ScrivenerSPFeedMoney'] == 0 && $data['cSpCaseFeedBackMoneyMark'] == 'x') {
        ...
        $objPHPExcel->getActiveSheet()->getStyle('I' . $row)->getFill()->getStartColor()->setARGB("FFBFBF");
    }

    $objPHPExcel->getActiveSheet()->setCellValue('I' . $row, $feedData['ScrivenerSPFeedMoney']);
    $objPHPExcel->getActiveSheet()->setCellValue('J' . $row, $feedData['otherFeed']);
    $objPHPExcel->getActiveSheet()->setCellValue('K' . $row, $feedData['sales']);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('T' . $row, $feedData['bRenote']);
    $objPHPExcel->getActiveSheet()->setCellValue('U' . $row, $data['performanceSales']);

    $tmp = $tmp2 = $tmp3 = $recall = $scrivenerMark = null;
    unset($tmp, $tmp2, $tmp3, $recall, $scrivenerMark);
}
