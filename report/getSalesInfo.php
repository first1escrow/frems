	...
	// if ($query) {
		$sql ='
			SELECT 
				...
				(SELECT sSalesDate FROM tScrivener WHERE sId = cScrivener) AS sPreSales
	...
	if ($query) {
		$sql ='
			SELECT 
				...
				cas.cCaseFeedBackMoney2,
				cas.cSpCaseFeedBackMoney,
				(SELECT bSales FROM tBranchSales WHERE bBranch = rea.cBranchNum) AS bSales1,
				(SELECT bSales FROM tBranchSales WHERE bBranch = rea.cBranchNum1) AS bSales2,
				(SELECT bSales FROM tBranchSales WHERE bBranch = rea.cBranchNum2) AS bSales3,
				(SELECT sSales FROM tScrivenerSales WHERE sScrivener = cScrivener) AS sSales,
				(SELECT bSales FROM tBranch WHERE bId = rea.cBranchNum) AS bPreSales1,
				(SELECT bSales FROM tBranch WHERE bId = rea.cBranchNum1) AS bPreSales2,
				(SELECT bSales FROM tBranch WHERE bId = rea.cBranchNum2) AS bPreSales3,
				(SELECT sSales FROM tScrivener WHERE sId = cScrivener) AS sPreSales,
...
function getOwnStoreTarget($total,$yr='',$m='') { 
...
##
//取得新進地政士
function getOwnScrivener($sales,$date_start,$date_end,$all='',$status='') {
	global $conn ;
	...
	$sql = "SELECT
			...
			WHERE
				".$str."
				ss.sType = 1 AND ss.sSales ='".$sales."'
				GROUP BY s.sId
			...
			if ($status =='') {
				if (!@in_array($rs->fields['bId'], $close)) {
					...
					if ($rs->fields['bOldStoreID'] > 0) {
						//算給舊店
						// $Branch[$i]['oldStore'] = '轉至'.$rs->fields['bCode'].$rs->fields['brand'].$rs->fields['bStore']."(".$rs->fields['bName'].')';
					
						// $tmp = getCloseStore($rs->fields['bOldStoreID']);
						// $Branch[$i]['bId'] = $tmp['bId'];
						// $Branch[$i]['brand'] = $tmp['brand'];
						// $Branch[$i]['bStore'] = $tmp['bStore'];
						// $Branch[$i]['bName'] = $tmp['bName'];
						//算給新店
						$tmp = getCloseStore($rs->fields['bOldStoreID']);
...
function getCloseStore($id){
	global $conn;
	$sql = "SELECT *,(SELECT bName FROM tBrand AS b WHERE b.bId = bBrand) AS brand,(SELECT bCode FROM tBrand AS b WHERE b.bId = bBrand) AS code FROM tBranch WHERE bId = '".$id."'";
	// echo $sql;
	$rs = $conn->Execute($sql);
	$data = $rs->fields;
...
function checkSales($arr,$pId)
{
	...
	$sql = "SELECT sSales AS sales FROM tScrivenerSales WHERE sScrivener ='".$arr['cScrivener']."'";
	$rs = $conn->Execute($sql);

	while (!$rs->EOF) {
		$tmp[] = $rs->fields['sSales'];
	...
	$sql = "SELECT bSales FROM tBranchSales WHERE bBranch ='".$arr['branch']."'";
	$rs = $conn->Execute($sql);
	...
	$sql = "SELECT bSales FROM tBranchSales WHERE bBranch ='".$arr['branch1']."'";
	$rs = $conn->Execute($sql);
	...
	$sql = "SELECT bSales FROM tBranchSales WHERE bBranch ='".$arr['branch2']."'";
	$rs = $conn->Execute($sql);
