    ...
    foreach ($cCertifiedId as $k => $v) {
        $sql = '
			SELECT
				...
				csales.cCreator,
			    cas.cBankList,
			    (SELECT GROUP_CONCAT(i.pName) FROM tScrivenerSales AS s JOIN tPeopleInfo AS i ON s.sSales =i.pId WHERE sScrivener = csc.cScrivener GROUP BY sScrivener) AS scrSales
			FROM
        ...
        //正常回饋
        while (! $rs->EOF) {
            $noFeedBackStore       = []; //不回饋店家，判斷總部回饋用[不回饋要算公司包含總部回饋]
            $Arr                   = $rs->fields;
            $Arr['tBankLoansDate'] = $v['tBankLoansDate'] ? $v['tBankLoansDate'] : $rs->fields['cBankList'];
...
$objPHPExcel->getActiveSheet()->setCellValue(chr($c++) . $row, '履保費出款日');
$objPHPExcel->getActiveSheet()->setCellValue(chr($c++) . $row, '業務');
...
foreach ($sortArray as $value) {
    foreach ($value as $v) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue(chr($c++) . $row, $v['feedbackDate']);      //P
        ...
        $objPHPExcel->getActiveSheet()->setCellValue(chr($c++) . $row, $v['bankLoansDate']);     //W
        $objPHPExcel->getActiveSheet()->setCellValue(chr($c++) . $row, $v['scrSales']);          //X
...
function setData($Arr, $bId, $brandId, $brandName, $feedbackTarget, $branchCode, $branchstore, $branch, $feedback, $feedBackMoney, $category, $cost = '')
{
    ...
    //統計資料-設定預設
    if (empty($dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['brand'])) {$dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['brand'] = [];}
    if (empty($dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['code'])) {$dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['code'] = [];}
    if (empty($dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['store'])) {$dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['store'] = [];}
    if (empty($dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['branch'])) {$dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['branch'] = [];}
    if (empty($dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['branchCategory'])) {$dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['branchCategory'] = [];}
    ...
    $Arr['scrivenerSales'] = getRealScrivenerSales($Arr['cScrivener'], $Arr['cApplyDate']);
    $_scrivener_sales      = $PeopleInfo[$Arr['scrivenerSales']]; //地政士負責業務

    $_sales = ($bId == 505) ? getSalesBy($Arr['scrivenerSales']) : ['id' => $Arr['caseSalesID']];

    //PO02574特例 桃園 和 北北基業務 各分一半
    $TaoyuanSales = [];
    $Taipei       = [];

    if ($Arr['bid'] == '2574' and ($Arr['cEndDate'] >= '2024-10-01' or $Arr['tBankLoansDate'] == '0000-00-00 00:00:00')) {
        //取得北北基 && 桃園市龜山區負責業務
        $sql = 'SELECT zZip, zSales FROM tZipArea WHERE zZip in ("333", "100") ;';
        $res = $conn->Execute($sql);
        while (! $res->EOF) {

            if ($res->fields['zZip'] == '333') {
                $TaoyuanSales = explode(",", $res->fields['zSales']);
            }
            if ($res->fields['zZip'] == '100') {
                $Taipei = explode(",", $res->fields['zSales']);
            }
            $res->MoveNext();
        }
    }
    if ($cost == '' && $feedbackTarget != 3) {                                                                                      //有選回饋才算收入(保證費) [他回饋保證費不能計算]  $feedback == 0 && //不回饋給公司2022-03-22
        ...
        if ($Arr['bid'] == '2574' and ($Arr['cEndDate'] >= '2024-10-01' or $Arr['tBankLoansDate'] == '0000-00-00 00:00:00')) {
            if (in_array($Arr['caseSalesID'], $TaoyuanSales)) { //特例 PO02574 桃園區業務分一半
                $dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['avgCertifiedMoney'] += round($Arr['cCertifiedMoney'] / $Arr['branchCount'] / 2 / (count($TaoyuanSales) + $_scrivener_sales_count));
            } else { //特例 PO02574 北北基業務分一半
                $dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['avgCertifiedMoney'] += round($Arr['cCertifiedMoney'] / $Arr['branchCount'] / 2 / (count($Taipei) + $_scrivener_sales_count));
            }
        } else {
    ...
    if ($dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['checkTwOne'] == 1) {
        ...
        if ($feedBackMoney == 0) {
            ...
            if ($Arr['bid'] == '2574' and ($Arr['cEndDate'] >= '2024-10-01' or $Arr['tBankLoansDate'] == '0000-00-00 00:00:00')) {
                if (in_array($Arr['caseSalesID'], $TaoyuanSales)) {
                    $dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['feedMoney'] += $feedBackMoney / 2 / (count($TaoyuanSales) + $_scrivener_sales_count);
                } else {
                    $dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['feedMoney'] += $feedBackMoney / 2 / (count($Taipei) + $_scrivener_sales_count);
                }
            } else {
    ...
    $dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['feedbackDate'] = dateformate($Arr['cFeedbackDate']);

    $dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['bankLoansDate'] = dateformate($Arr['tBankLoansDate']); //保證費出款時間
    $dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['scrSales']      = $Arr['scrSales'];                    //地政士業務
    ...
    if (! empty($_shadow)) {
        ...
        $dataCaseFeed[$_shadow['cCertifiedId']][$_shadow['sales']]['feedbackDate']   = $dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['feedbackDate'];
        ...
        $dataCaseFeed[$_shadow['cCertifiedId']][$_shadow['sales']]['scrSales']       = $dataCaseFeed[$Arr['cCertifiedId']][$Arr['caseSalesID']]['scrSales']; //地政士業務
...
//2025-07-03 取得地政士負責業務
function getRealScrivenerSales($cScrivener, $cApplyDate)
{
    global $conn;

    //取得地政士負責業務
    $sql = 'SELECT sSales FROM tSalesRegionalAttribution WHERE sType = 1 AND sDate <= "' . $cApplyDate . '" AND sStoreId = "' . $cScrivener . '" ORDER BY sDate DESC LIMIT 1;';
    $rs  = $conn->Execute($sql);

    if ($rs->RecordCount() > 0) {
        return $rs->fields['sSales'];
    }
