...
if ($_POST) {
	...
	if ($identity == 1) {
		...
		foreach ($list as $k => $v) {
			$sql = "SELECT * FROM tFeedBackStoreSms WHERE fType = 1 AND fStoreId = '".$v['sId']."' AND  fDelete = 0";
			$rs = $conn->Execute($sql);
	...
	if ($identity == 2) {
		...
		foreach ($list as $k => $v) {
			...
			// echo $sql."\r\n";
			// $rs = $conn->Execute($sql);
			$sql = "SELECT * FROM tFeedBackStoreSms WHERE fType = 2 AND fStoreId = '".$v['bId']."' AND  fDelete = 0";
			$rs = $conn->Execute($sql);
...
function getFeedBackMoney($sn,$yr,$se,$cat){
	...
	$sql = "SELECT * FROM tFeedBack WHERE fYear = '".$yr."' AND fType = '".$cat."' AND fNum = '".$sn."'";
