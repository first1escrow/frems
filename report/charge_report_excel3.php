...
if (!is_array($sales_arr)) {
	echo "<script>loaction.href='charge_report3.php';</script>";
}
//時間
...
if(!empty($sales_arr)){
    foreach($sales_arr as $k => $v){
        $sql = 'SELECT zZip FROM tZipArea WHERE zPerformanceSales="' . $v . '" ORDER BY zCity,zZip ASC;';
        $rs  = $conn->Execute($sql);

        while (!$rs->EOF) {
            $zipArrAll[$rs->fields['zZip']] = $rs->fields['zZip'];
            $zipArr[] = $rs->fields['zZip'];
            $salesArr['1'][$rs->fields['zZip']] = $v;
            $rs->MoveNext();
        }

        $sql = 'SELECT zZip FROM tZipArea WHERE zPerformanceScrivenerSales="' . $v . '" ORDER BY zCity,zZip ASC;';
        $rs  = $conn->Execute($sql);

        while (!$rs->EOF) {
            $zipArrAll[$rs->fields['zZip']] = $rs->fields['zZip'];
            $zipArr2[] = $rs->fields['zZip'];
            $salesArr['2'][$rs->fields['zZip']] = $v;
            $rs->MoveNext();
        }


        $area_condition = ' AND (rea.cZip IN ("' . implode('","', $zipArr) . '") OR rea.cZip1 IN ("' . implode('","', $zipArr) . '") OR rea.cZip2 IN ("' . implode('","', $zipArr) . '") OR rea.cZip3 IN ("' . implode('","', $zipArr) . '") OR scr.sZip1 IN ("' . implode('","', $zipArr2) . '"))';
    }
}

##
if ($date_type == 2) {
    $_sql = 'SELECT cc.cCertifiedId,cc.cBankList,(SELECT cCertifiedMoney FROM tContractIncome AS ci WHERE ci.cCertifiedId=cc.cCertifiedId) AS cCertifiedMoney FROM tContractCase AS cc 
                 LEFT JOIN tContractRealestate AS rea ON rea.cCertifyId=cc.cCertifiedId
                 LEFT JOIN tContractScrivener AS csc ON csc.cCertifiedId=cc.cCertifiedId
                 LEFT JOIN tScrivener AS scr ON scr.sId = csc.cScrivener';

    // 搜尋條件-履保費出款日
    if ($from_date2 or $to_date2) {

//        $sbankLoansDate = ($from_date2);
//        $ebankLoansDate = ($to_date2);
//        $cId = getBankLoansDateCid($conn, $sbankLoansDate, $ebankLoansDate);

        if(!empty($cId)) {
            $_sql .= ' WHERE cc.cCertifiedId IN (' . implode(",",$cId) .')';
        } else {
            $_sql .= ' WHERE cc.cBankList >= "' . $from_date2 . '" AND cc.cBankList<= "' . $to_date2 . '"';
        }
    }
    ...
    while (!$rs->EOF) {
        if ($rs->fields['cCertifiedId'] != '007106507' && !isset($cCertifiedId[$rs->fields['cCertifiedId']])) {
...
if($date_type == 1){
	...
	$sql = "SELECT cas.cCertifiedId FROM tContractCase AS cas LEFT JOIN tContractRealestate AS rea ON rea.cCertifyId=cas.cCertifiedId
                 LEFT JOIN tContractScrivener AS csc ON csc.cCertifiedId=cas.cCertifiedId
                 LEFT JOIN tScrivener AS scr ON scr.sId = csc.cScrivener WHERE ".$query;
    $sql .= $area_condition;
...
require_once 'charge_report_excel3_2024.php';
...
if ($report_type==2) { //N個業務比較
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A2','多個績效業務比較');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A2','單一績效業務比較');
	$c=66;
