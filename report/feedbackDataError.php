...
if(count($errorDetail) > 0) {
    foreach ($errorDetail as $k=>$v) {
        ...
        $tbl .= "<td>" . ($x+1) . "</td>";
        $tbl .= "<td><a href='#' onclick='contract(\"" . $k . "\")'>" . $k . "</a></td>";
        $tbl .= "<td>" . implode('<br>', $v) . "</td>";
        $tbl .= "</tr>";
...
$smarty->display('feedbackDataError.tpl', '', 'report') ;
exit;
...
//    $str .= " AND (cc.cCaseStatus = 2)";
    $str .= " AND (cc.cCaseStatus = 2 OR (cc.cCaseStatus > 2 AND cc.cFinishDate3 >= '".date("Y-m-d 00:00:00")."' AND cc.cFinishDate3 <= '".date("Y-m-d 23:59:59")."'))";
...
//        ORDER BY cc.cEndDate ASC";cc.cCertifiedId='100008386' AND
    $rs = $conn->Execute($sql);
        ...
        for ($i = 0; $i < count($list); $i++) {
            ...
            if ($list[$i]['branch'] > 0) {
                if ($list[$i]['cFeedbackTarget'] == 2) { //scrivener
                    ...
                    if($brecall[0] > 0){ $cScrivenerSpRecallFlag = false; }
                } else {
                    $brecall[0] = $list[$i]['bRecall'] / 100; //計算用
                    if($cScrivenerSpRecallFlag && in_array($list[$i]['brand'],array(1,2,49))){ $cScrivenerSpRecallFlag = false; }
                }
            ...
            if ($list[$i]['branch1'] > 0) {
                if ($list[$i]['cFeedbackTarget1'] == 2) { //scrivener
                    ...
                    if($brecall[1] > 0){ $cScrivenerSpRecallFlag = false; }
                } else {
                    $brecall[1] = $list[$i]['bRecall1'] / 100; //計算用
                    if($cScrivenerSpRecallFlag && in_array($list[$i]['brand1'],array(1,2,49))){ $cScrivenerSpRecallFlag = false; }
                }
            ...
            if ($list[$i]['branch2'] > 0) {
                if ($list[$i]['cFeedbackTarget2'] == 2) { //scrivener
                    ...
                    if($brecall[2] > 0){ $cScrivenerSpRecallFlag = false; }
                } else {
                    $brecall[2] = $list[$i]['bRecall2'] / 100; //計算用
                    if($cScrivenerSpRecallFlag && in_array($list[$i]['brand2'],array(1,2,49))){ $cScrivenerSpRecallFlag = false; }
                }
            ...
            if ($list[$i]['branch3'] > 0) {
                ...
                if ($list[$i]['cFeedbackTarget3'] == 2) { //scrivener
                    ...
                    if($brecall[3] > 0){ $cScrivenerSpRecallFlag = false; }
                } else {
                    $brecall[3] = $list[$i]['bRecall3'] / 100; //計算用
                    if($cScrivenerSpRecallFlag && in_array($list[$i]['brand3'],array(1,2,49))){ $cScrivenerSpRecallFlag = false; }
                }
