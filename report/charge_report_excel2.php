...
include_once dirname(__DIR__) . '/bank/Classes/PHPExcel.php' ;
include_once dirname(__DIR__) . '/bank/Classes/PHPExcel/Writer/Excel2007.php' ;
...
if($date_type == 1) { //簽約時間
    ...
    $sql = 'SELECT DISTINCT tMemo, tMoney ,tBankLoansDate FROM tBankTrans WHERE  tPayOk="1" AND tKind = "保證費" AND tId != "451978" AND tId != "664562" AND tMemo IN ('.implode(",", $cId).') AND tMemo != "000000000" ';
    $rs = $conn->Execute($sql);
...
##
} elseif ($date_type == 3) { //結案日期
    if ($query) $query .= ' AND ';
    $query .= " DATE(cas.cFeedbackDate) >='".$from_date."' AND DATE(cas.cFeedbackDate)<='".$to_date."'";
...
function convertToLetter($c)
{
    if($c <= 90) {
        return chr($c);
    }
    $leftLetter = chr(($c - 90)/26 + 65);
    $rightLetter = chr(($c - 90)%26 + 65);
