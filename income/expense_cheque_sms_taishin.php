...
$sql='
	...
	$check_data=$rs->fields["eId"];
	$sms = new SMS_Gateway();
 //判斷有沒有合約書
if(empty($check_data)){
	  die("<script>alert('合約書尚未建立!!請先建立合約書後再發送簡訊');location.href='expense_cheque_taishin.php'</script>");
...
if(isset($_POST['id'])){
	...
	$sql='UPDATE tExpense_cheque_taishin SET eSms=1 WHERE eId='.$id;
	...
	$sms->send( $rs->fields["CertifiedId"] , $sid, $rs->fields["cBranchNum"], 'chequetaisin', $id, 'y', 0,$_POST['mobile'],$_POST['txt']);


	die("<script>alert('已發送簡訊');location.href='expense_cheque_taishin.php'</script>");
}
###
$array = $sms->send( $rs->fields["CertifiedId"] , $sid, $rs->fields["cBranchNum"], 'chequetaisin', $id, 'n', 0);
