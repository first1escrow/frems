...
if (preg_match("/^09[0-9]{8}$/",$Q_mobile) && (preg_match("/^[0-9]{13}$/",$Q_msgid))) {
	// Query 參數設定
...
<body>
<form method="POST" name="apiQ">
<div style="width:1044px; margin-bottom:5px;padding-top:10px;padding-left:10px; height:30px; background-color: #CCC">
<strong style="color:#002060;">遠傳簡訊狀態查詢反饋列表</strong>　　
<input type="hidden" name="ng" value="1">
<span style="font-size:12px;color:#000080;">門號：<input type="text" name="qMobile" style="width:150px;" value=""></span>　
<span style="font-size:12px;color:#000080;">Message Id：<input type="text" name="qId" style="width:150px;" value=""></span>
<input type="submit" value="搜尋" onsubmit="">&nbsp;
<input type="button" value="清除" onclick="clearAll()">
</div>
</form>

<div id="returnCode" style="width:1044px;margin-top:20px;display:<?=$show?>;">
回覆字串：<?=$resA?>
<hr style="width:1044px;">
<?php
