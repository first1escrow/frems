...
class SMS_Gateway extends PDO
{
    ...
    public function sendIncome($pid, $sid, $bid, $target, $tid, $ok = "n", $sendList = '')
    {
        ...
        switch ($target) {
            ...
            case 'income2':
                ...
                // print_r($memo);
                //正常版
                $sms_txt = '第一建經信託履約保證專戶已於' . $M . '月' . $D . '日收到保證編號' . substr($pid, 5, 9) . '（買方' . $buyer . $bCount . '賣方' . $seller . $oCount . '）存入' . $money . '元（認列:' . $memo['normal'] . '元)';
    ...
    private function getShortUrl($url, $key, $code = null)
    {
