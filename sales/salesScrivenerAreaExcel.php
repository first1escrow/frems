...
$objPHPExcel->getActiveSheet()->setCellValue(chr($col++).'1','地政士業務歸屬');
$col =65;
$objPHPExcel->getActiveSheet()->setCellValue(chr($col++).'2','地區');
$objPHPExcel->getActiveSheet()->setCellValue(chr($col++).'2','地政士編號');
$objPHPExcel->getActiveSheet()->setCellValue(chr($col++).'2','地政士');
...
foreach ($list as $k => $v) {
	...
	$objPHPExcel->getActiveSheet()->getStyle("A".$row.":G".$row."")->getFill()->getStartColor()->setARGB('FFDEDE');
	$row++;
	...
	foreach ($list[$k] as $key => $value) {
		...
		if (is_array($list[$k][$key])) {
			foreach ($list[$k][$key] as $ke => $scrivener) {
				$col =65;
				$objPHPExcel->getActiveSheet()->setCellValue(chr($col++).$row,$key);
				$objPHPExcel->getActiveSheet()->setCellValue(chr($col++).$row,$scrivener['sCode']);
				$objPHPExcel->getActiveSheet()->setCellValue(chr($col++).$row,$scrivener['sName']."(".$scrivener['sOffice'].")");
				$objPHPExcel->getActiveSheet()->setCellValue(chr($col++).$row,$scrivener['salesname']);
				$objPHPExcel->getActiveSheet()->setCellValue(chr($col++).$row,$scrivener['sTelArea'].'-'.$scrivener['sTelMain']);
				$objPHPExcel->getActiveSheet()->setCellValueExplicit(chr($col++).$row, $scrivener['sMobileNum'],PHPExcel_Cell_DataType::TYPE_STRING); 
				$objPHPExcel->getActiveSheet()->setCellValueExplicit(chr($col++).$row, $scrivener['city'].$scrivener['area'].$scrivener['sAddress'],PHPExcel_Cell_DataType::TYPE_STRING); 
...
$_file = 'salesAreaS'.date('Y-m-d').'.xlsx' ;
