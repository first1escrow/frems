...
if (!empty($_POST)) {
	...
	if ($cat == '1') {
		...
		$sql = "DELETE FROM tCategoryLandGoverment WHERE cId = '".$id."'" ;
		// echo $sql;
		$msgCode = 3;
		if ($conn->Execute($sql)) {
			$msg = '刪除成功';
...
##
$sql = "SELECT *,(SELECT zCity FROM tZipArea WHERE zZip=cZip) AS city FROM tCategoryLandGoverment WHERE cId = '".$id."'";
$rs = $conn->Execute($sql);
$data = $rs->fields;
...
$smarty->display('LandGovermentEdit.inc.tpl', '', 'line');
?>
