...
class Contract extends Advance
{
    ...
    public function AddProperty2($data)
    {
        ...
        if (isset($data['new_property_Item']) && is_array($data['new_property_Item'])) {
    ...
    public function SaveContract($data)
    {
        ...
        // 初始化可能缺少的鍵
        $keys = [
            'cCaseFeedBackMoney3', 'cFeedbackTarget3', 'case_dealid', 'cCaseFeedback3',
            'case_exceptionreason', 'case_property', 'cSpCaseFeedBackMoneyMark',
            'cNoIncome', 'cNoClosing', 'contract_sale', 'case_show', 'case_cancellingClauseNote',
            'cAffixBranch', 'cAffixBranch1', 'cAffixBranch2', 'cAffixBranch3',
        ];
        ...
        if (isset($data['cCaseFeedBackMoney3']) && $data['cCaseFeedBackMoney3'] != '') {
            if ($data['realestate_branchnum'] == '') {
        ...
        $sql = "UPDATE  `tContractCase` SET
                    ...
                    `cFinishDate` = '" . (isset($data['case_finishdate']) ? $data['case_finishdate'] : '') . "',
                    `cFinishDate2` = '" . (isset($data['case_finishdate2']) ? $data['case_finishdate2'] : '') . "',
        ...
        if ($data['realestate_branchnum'] > 0) {
            $sql = "
                UPDATE  `tContractRealestate` SET
                    ...
                    `cAffixBranch` = '" . (isset($data['cAffixBranch']) ? $data['cAffixBranch'] : '') . "',
                    `cAffixBranch1` = '" . (isset($data['cAffixBranch1']) ? $data['cAffixBranch1'] : '') . "',
                    `cAffixBranch2` = '" . (isset($data['cAffixBranch2']) ? $data['cAffixBranch2'] : '') . "',
                    `cAffixBranch3` = '" . (isset($data['cAffixBranch3']) ? $data['cAffixBranch3'] : '') . "'
    ...
    public function SaveAscription($data)
    {
        ...
        $cOwner = '';
        if (isset($data['ascription_owner']) && is_array($data['ascription_owner'])) {
            $cOwner = implode(',', $data['ascription_owner']);
        }

        $cBuyer = '';
        if (isset($data['ascription_buy']) && is_array($data['ascription_buy'])) {
            $cBuyer = implode(',', $data['ascription_buy']);
        }

        $sql = "
            UPDATE  `tContractAscription` SET
                `cContribute` =  '" . (isset($data['ascription_contribute']) ? $data['ascription_contribute'] : '') . "',
                `cBuyer` =  '" . $cBuyer . "',
    ...
    public function SaveContractRent($data)
    {
        ...
        $sql = "
            UPDATE  `tContractRent` SET
                ...
                `cFinish` =  '" . (isset($data['rent_finish']) ? $data['rent_finish'] : '') . "',
                `cOther` =  '" . (isset($data['rent_cOther']) ? $data['rent_cOther'] : '') . "'
    ...
    public function SaveProperty2($data, $item = 0)
    {
        ...
        $object = '';
        if (isset($data['property_cPropertyObject' . $item]) && is_array($data['property_cPropertyObject' . $item])) {
            $object = implode(',', $data['property_cPropertyObject' . $item]);
        }

        if (isset($data['property_power2' . $item]) && $data['property_power2' . $item] > 0) {
        ...
        $sql = "
            UPDATE `tContractProperty` SET
                    ...
                    `cHasCar` = '" . (isset($data['property_hascar' . $item]) ? $data['property_hascar' . $item] : '') . "',
                    `cBuildNo` = '" . $data['property_buildno' . $item] . "',
                    `cTownHouse` = '" . $data['property_housetown' . $item] . "',
                    `cRent` = '" . (isset($data['property_rent' . $item]) ? $data['property_rent' . $item] : '') . "',
                    `cRentDate` = '" . (isset($data['property_rentdate' . $item]) ? $data['property_rentdate' . $item] : '') . "',
                    `cFinish` = '" . (isset($data['property_finish' . $item]) ? $data['property_finish' . $item] : '') . "',
                    `cMeasureTotal` = '" . $data['property_measuretotal' . $item] . "',
                    `cActualArea` = '" . $data['property_actualArea' . $item] . "',
                    `cPropertyObject` = '" . $object . "',
                    `cObjectOther` = '" . (isset($data['property_cObjectOther' . $item]) ? $data['property_cObjectOther' . $item] : '') . "',
    ...
    {
        // 檢查變數類型，避免陣列到字串的自動轉換
        $land_landprice = is_array($data['land_landprice']) ? implode(',', $data['land_landprice']) : $data['land_landprice'];
        $land_money     = is_array($data['land_money']) ? implode(',', $data['land_money']) : $data['land_money'];
        $land_movedate  = is_array($data['land_movedate']) ? implode('-', $data['land_movedate']) : $data['land_movedate'];

        $data['land_landprice'] = str_replace(',', '', $land_landprice);
        $data['land_money']     = str_replace(',', '', $land_money);
        $data['land_movedate']  = $land_movedate . "-00";
        $sql                    = " UPDATE `tContractLand` SET
                    `cZip` = '" . $data['land_zip'] . "',
                    `cAddr` = '" . (isset($data['land_addr']) ? $data['land_addr'] : '') . "',
                    `cLand1` = '" . $data['land_land1'] . "',
                    `cLand2` = '" . $data['land_land2'] . "',
                    `cLand3` = '" . $data['land_land3'] . "',
                    `cLand4` = '" . (isset($data['land_land4']) ? $data['land_land4'] : '') . "',
                    `cMeasure` = '" . $data['land_measure'] . "',
                    `cCategory` = '" . $data['land_category'] . "',
                    `cMoney` = '" . $data['land_money'] . "',
                    `cPower1` = '" . (is_array($data['lpower1']) ? implode(',', $data['lpower1']) : $data['lpower1']) . "',
                    `cPower2` = '" . (is_array($data['lpower2']) ? implode(',', $data['lpower2']) : $data['lpower2']) . "',
                    `cMoveDate` = '" . $data['land_movedate'] . "',
                    `cFarmLand` = '" . (is_array($data['land_farmland']) ? implode(',', $data['land_farmland']) : $data['land_farmland']) . "',
    ...
    {
        $data['income_loanmoney']          = isset($data['income_loanmoney']) ? str_replace(',', '', $data['income_loanmoney']) : '';
        $data['income_signmoney']          = str_replace(',', '', $data['income_signmoney']);
        $data['income_affixmoney']         = str_replace(',', '', $data['income_affixmoney']);
        $data['income_dutymoney']          = str_replace(',', '', $data['income_dutymoney']);
        $data['income_estimatedmoney']     = str_replace(',', '', $data['income_estimatedmoney']);
        ...
        $sql = "UPDATE  `tContractIncome` SET
            `cBankLoan` =  '" . (isset($data['income_bankloan']) ? $data['income_bankloan'] : '') . "',
            ...
            `cSrivenerMoney` = '" . (isset($data['income_scrivenermoney']) ? $data['income_scrivenermoney'] : '') . "',
    ...
    public function SaveInvoice($data)
    {
        ...
        $data['invoice_invoiceother']      = isset($data['invoice_invoiceother']) ? str_replace(",", "", $data['invoice_invoiceother']) : '';

        $sql = "
            UPDATE  `tContractInvoice` SET
                    `cCertifiedBankAcc` =  '" . (isset($data['invoice_certifiedbankacc']) ? $data['invoice_certifiedbankacc'] : '') . "',
                    `cTaxReceiptTarget` =  '" . (isset($data['cTaxReceiptTarget']) ? $data['cTaxReceiptTarget'] : '') . "',
    ...
    public function SaveOwner($data)
    {
        ...
        $data['owner_money2'] = isset($data['owner_money2']) ? str_replace(",", "", $data['owner_money2']) : '';
        ...
        if ($data['owner_name']) {
            $sql = " UPDATE `tContractOwner` SET
                ...
                `cResidentLimit` = '" . (isset($data['owner_resident_limit']) ? $data['owner_resident_limit'] : '') . "',
                `cPaymentDate` = '" . $data['owner_payment_date'] . "',
                `cNHITax` = '" . $data['owner_NHITax'] . "',
                `cOther` = '" . (isset($data['owner_other']) ? $data['owner_other'] : '') . "',
                `cBirthdayDay` = '" . $data['owner_birthdayday'] . "',
                `cContactName` = '" . (isset($data['owner_contactname']) ? $data['owner_contactname'] : '') . "',
                `cMobileNum` = '" . $data['owner_mobilenum'] . "',
                `sAgentName1` = '" . (isset($data['owner_agentname1']) ? $data['owner_agentname1'] : '') . "',
                `sAgentName2` = '" . (isset($data['owner_agentname2']) ? $data['owner_agentname2'] : '') . "',
                `sAgentName3` = '" . (isset($data['owner_agentname3']) ? $data['owner_agentname3'] : '') . "',
                `sAgentName4` = '" . (isset($data['owner_agentname4']) ? $data['owner_agentname4'] : '') . "',
                `sAgentMobile1` = '" . (isset($data['owner_agentmobile1']) ? $data['owner_agentmobile1'] : '') . "',
                `sAgentMobile2` = '" . (isset($data['owner_agentmobile2']) ? $data['owner_agentmobile2'] : '') . "',
                `sAgentMobile3` = '" . (isset($data['owner_agentmobile3']) ? $data['owner_agentmobile3'] : '') . "',
                `sAgentMobile4` = '" . (isset($data['owner_agentmobile4']) ? $data['owner_agentmobile4'] : '') . "',
    ...
    public function SaveBuyer($data)
    {
        ...
        if ($data['buy_name']) {
            $sql = "UPDATE `tContractBuyer` SET
                ...
                `cTaxtreatyCode` = '" . (isset($data['buyer_taxtreaty']) ? strtoupper($data['buyer_taxtreaty']) : '') . "',
                `cResidentLimit` = '" . (isset($data['buyer_resident_limit']) ? $data['buyer_resident_limit'] : '') . "',
                `cPaymentDate` = '" . $data['buyer_payment_date'] . "',
                `cNHITax` = '" . $data['buyer_NHITax'] . "',
                `cOther` = '" . (isset($data['buyer_other']) ? $data['buyer_other'] : '') . "',
                `cBirthdayDay` = '" . $data['buy_birthdayday'] . "',
                `cContactName` = '" . (isset($data['buy_contactname']) ? $data['buy_contactname'] : '') . "',
                `cMobileNum` = '" . $data['buy_mobilenum'] . "',
                `sAgentName1` = '" . (isset($data['buyer_agentname1']) ? $data['buyer_agentname1'] : '') . "',
                `sAgentName2` = '" . (isset($data['buyer_agentname2']) ? $data['buyer_agentname2'] : '') . "',
                `sAgentName3` = '" . (isset($data['buyer_agentname3']) ? $data['buyer_agentname3'] : '') . "',
                `sAgentName4` = '" . (isset($data['buyer_agentname4']) ? $data['buyer_agentname4'] : '') . "',
                `sAgentMobile1` = '" . (isset($data['buyer_agentmobile1']) ? $data['buyer_agentmobile1'] : '') . "',
                `sAgentMobile2` = '" . (isset($data['buyer_agentmobile2']) ? $data['buyer_agentmobile2'] : '') . "',
                `sAgentMobile3` = '" . (isset($data['buyer_agentmobile3']) ? $data['buyer_agentmobile3'] : '') . "',
                `sAgentMobile4` = '" . (isset($data['buyer_agentmobile4']) ? $data['buyer_agentmobile4'] : '') . "',
    ...
    public function saveOwnerSales($data)
    {
        if (is_array($data['owner_agentmobile'])) {
            for ($i = 0; $i < count($data['owner_agentmobile']); $i++) {
                ...
                if (isset($data['owner_agenId']) && isset($data['owner_agenId'][$i])) {
                    $sql = "UPDATE
                                tContractPhone
                            SET
    ...
    public function saveBuyerSales($data)
    {
        if (is_array($data['buyer_agentname'])) {
            for ($i = 0; $i < count($data['buyer_agentname']); $i++) {
                ...
                if (isset($data['buyer_agenId']) && isset($data['buyer_agenId'][$i])) {
                    $sql = "UPDATE
                                tContractPhone
                            SET
