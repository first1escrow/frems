...
class ExcelPu extends ExcelBase
{
    ...
    public function GenerateField($certifiedId = null)
    {
        ...
        foreach ($list_case as $k => $v) {
            ...
            $address = $this->getAddress($v['cCertifiedId']); //2022-07-06 增加自訂一欄位五(AX欄)顯示部分建物地址資訊
            ...
            if ($data_buyer['cInvoiceMoney'] > 0) {
                ...
                $arrItem[$index]['AW'] = '****' . substr($v['cCertifiedId'], -5);
                $arrItem[$index]['AX'] = $address; //2022-07-06 增加自訂一欄位五(AX欄)顯示部分建物地址資訊
            ...
            for ($i = 0; $i < $_arr_max; $i++) {
                if ($_arr[$i]['cInvoiceMoney'] > 0) {
                    ...
                    $_num      = $pu_roc_date . str_pad($flowNo++, 6, '0', STR_PAD_LEFT);
                    $AQ        = '1';
                    if (preg_match("/^\w{10}$/", $_arr[$i]['cIdentifyId']) or preg_match("/^\w{7}$/", $_arr[$i]['cIdentifyId'])) {
                        $invoice_kind = '2';
            ...
            if ($data_res['cInvoiceMoney'] > 0) {
                ...
                if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                    ...
                    $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractRealestate_R', $info_branch['bId'], $info_branch['bName'], $info_branch['bSerialnum'], $info_branch['bSerialnum'], $info_branch['bPassword'], $data_res['cInvoiceMoney']);
                }
                ##
            ...
            foreach ($_arr1 as $ka => $va) {
                if ($va['cInvoiceMoney'] > 0) {
                    ...
                    if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                        ...
                        $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractInvoiceExt_R', $va['cId'], $va['cName'], $va['cIdentifyId'], $va['cIdentifyId'], $info_branch['bPassword'], $va['cInvoiceMoney']);
                    }
                    ##
            ...
            if ($data_res['cInvoiceMoney1'] > 0) {
                ...
                if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                    ...
                    $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractRealestate_R1', $info_branch1['bId'], $info_branch1['bName'], $info_branch1['bSerialnum'], $info_branch1['bSerialnum'], $info_branch1['bPassword'], $data_res['cInvoiceMoney1']);
                }
                ##
            ...
            foreach ($_arr1 as $ka => $va) {
                if ($va['cInvoiceMoney'] > 0) {
                    ...
                    if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                        ...
                        $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractInvoiceExt_R1', $va['cId'], $va['cName'], $va['cIdentifyId'], $va['cIdentifyId'], $info_branch1['bPassword'], $va['cInvoiceMoney']);
                    }
                    ##
            ...
            if ($data_res['cInvoiceMoney2'] > 0) {
                ...
                if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                    ...
                    $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractRealestate_R2', $info_branch2['bId'], $info_branch2['bName'], $info_branch2['bSerialnum'], $info_branch2['bSerialnum'], $info_branch2['bPassword'], $data_res['cInvoiceMoney2']);
                }
                ##
            ...
            foreach ($_arr1 as $ka => $va) {
                if ($va['cInvoiceMoney'] > 0) {
                    ...
                    if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                        ...
                        $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractInvoiceExt_R2', $va['cId'], $va['cName'], $va['cIdentifyId'], $va['cIdentifyId'], $info_branch2['bPassword'], $va['cInvoiceMoney']);
                    }
                    ##
            ...
            if ($data_res['cInvoiceMoney3'] > 0) {
                $giver_num                  = $v['cCertifiedId'] . '3' . str_pad((++$total_index), 2, '0', STR_PAD_LEFT);
                $_num                       = $pu_roc_date . str_pad($flowNo++, 6, '0', STR_PAD_LEFT);
                $org_cInvoice               = $data_res['cInvoiceMoney3'];
                $data_res['cInvoiceMoney3'] = round($data_res['cInvoiceMoney3'] / 1.05);
                $invoice_kind               = '3';
                $AQ                         = '0'; //0:三聯式 1:二聯式
                ...
                $arrItem[$index]['AH'] = '0';
                $arrItem[$index]['AI'] = '1';
                $arrItem[$index]['AJ'] = $data_res['cInvoiceMoney3'];
                $arrItem[$index]['AK'] = $data_res['cInvoiceMoney3'];
                $arrItem[$index]['AQ'] = $AQ;
                $arrItem[$index]['AR'] = $info_branch3['bSerialnum'];
                $arrItem[$index]['AS'] = $pu_roc_date . $giver_num . '0'; //年月日7碼+履保編號9碼+對象1碼(買1賣2仲介3地政士4)+人數2碼+其他/捐贈1碼(不捐贈0捐贈1其他2)、預設不捐贈0
                $arrItem[$index]['AT'] = '保證號碼:' . $v['cCertifiedId'];
                $arrItem[$index]['AU'] = '出貨單號:' . $pu_roc_date . $giver_num . '0';
                $arrItem[$index]['AV'] = $info_branch3['bName'];
                $arrItem[$index]['AW'] = '****' . substr($v['cCertifiedId'], -5);
                $arrItem[$index]['AX'] = $address; //2022-07-06 增加自訂一欄位五(AX欄)顯示部分建物地址資訊
                ...
                $loadNo1 = $info_branch3['bSerialnum'];
                $loadNo2 = $loadNo1; //依據關網林先生 2015-06-30 的說法、顯碼與隱碼應該一致
                ...
                if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                    if ($arrItem[$index]['AQ'] == 0) //法人(不含事務所)要*1.05
                    {
                        $data_res['cInvoiceMoney3'] = $org_cInvoice;
                        // $data_res['cInvoiceMoney3'] = Round($data_res['cInvoiceMoney3']*1.05);
                    }

                    $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractRealestate_R2', $info_branch3['bId'], $info_branch3['bName'], $info_branch3['bSerialnum'], $info_branch3['bSerialnum'], $info_branch3['bPassword'], $data_res['cInvoiceMoney3']);
                }
                ##
            ...
            foreach ($_arr1 as $ka => $va) {
                if ($va['cInvoiceMoney'] > 0) {
                    ...
                    if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                        ...
                        $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractInvoiceExt_R3', $va['cId'], $va['cName'], $va['cIdentifyId'], $va['cIdentifyId'], $info_branch3['bPassword'], $va['cInvoiceMoney']);
                    }
                    ##
            ...
            if ($data_invoice['cInvoiceScrivener'] > 0 and $data_sc['cInvoiceMoney'] > 0) {
                ...
                if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                    ...
                    $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractScrivener', $info_sc['sId'], $_name, $info_sc['sIdentifyId'], $loadNo1, $info_sc['sPassword'], $data_invoice['cInvoiceScrivener']);
                }
                ##

                $index++;
            }
            unset($org_cInvoice, $_name);
            ##
            ...
            foreach ($_arr as $ka => $va) {
                if ($va['cInvoiceMoney'] > 0) {
                    ...
                    if ($this->checkInvoice($v['cCertifiedId'], $arrItem[$index]['AS'])) {
                        ...
                        $this->AddInvoiceNo($v['cCertifiedId'], $_num, $arrItem[$index]['AS'], '', 'tContractInvoiceExt_S', $va['cId'], $va['cName'], $va['cIdentifyId'], $loadNo1, $info_sc['sPassword'], $va['cInvoiceMoney']);
                    }
                    ##
    ...
    //20220706 取得建物門牌且隱現部分 (縣市 + 區域 + 3 文字 + ***)
