...
namespace First1\V1\CheckFeedbackMoney;
...
class CheckFeedbackMoney
{
    public function check($id)
    {
        ...
        if (! empty($id)) {
            $errorDetail2 = $this->checkFeedMoneyReview($id);
            $errorDetail  = $this->getFeedMoney($type, $id, $id2 = '', $FeedDateCat = '');
...
//        return json_encode($output, JSON_UNESCAPED_UNICODE);
    ...
    ###############重新計算回饋金###################
    public function getFeedMoney($type, $id, $id2 = '', $FeedDateCat = '')
    {
        global $conn;
            ...
            for ($i = 0; $i < count($list); $i++) {
                ...
                if ($list[$i]['branch'] > 0) {
                    if ($list[$i]['cFeedbackTarget'] == 2) {  //scrivener
                        ...
                        if ($cScrivenerSpRecallFlag && in_array($list[$i]['brand'], [1, 2, 49])) {
                            $cScrivenerSpRecallFlag = false;
                        }
                    }
                ...
                if ($list[$i]['branch1'] > 0) {
                    if ($list[$i]['cFeedbackTarget1'] == 2) { //scrivener
                        ...
                        if ($cScrivenerSpRecallFlag && in_array($list[$i]['brand1'], [1, 2, 49])) {
                            $cScrivenerSpRecallFlag = false;
                        }
                    }
                ...
                if ($list[$i]['branch2'] > 0) {
                    if ($list[$i]['cFeedbackTarget2'] == 2) { //scrivener
                        ...
                        if ($cScrivenerSpRecallFlag && in_array($list[$i]['brand2'], [1, 2, 49])) {
                            $cScrivenerSpRecallFlag = false;
                        }
                    }
                ...
                if ($list[$i]['branch3'] > 0) {
                    ...
                    if ($list[$i]['cFeedbackTarget3'] == 2) { //scrivener
                        ...
                        if ($cScrivenerSpRecallFlag && in_array($list[$i]['brand3'], [1, 2, 49])) {
                            $cScrivenerSpRecallFlag = false;
                        }
                    }
...
//            $cCertifiedId[] = $list[$i]['cCertifiedId'];
    ...
    public function checkFeedMoneyReview($id)
    {
        global $conn;
        ...
        $sql = "SELECT a.fId,a.fCertifiedId,c.fRId,
            ...
            where a.fCertifiedId = '" . $id . "' AND c.fDelete = 0 AND a.fStatus = 1 AND a.fFail = 0 ORDER BY c.fId DESC";
        $rs = $conn->Execute($sql);
