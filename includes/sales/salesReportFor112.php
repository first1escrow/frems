...
if ($sales) {
    ...
    while (!$rs->EOF) {
        ...
        if ((date("Y", strtotime($date_start)) == '2022') && ($i == 6) && in_array($sales, [68, 90, 97])) { //2022-06-30 副總要求廷蔚(68)、偉哲(90)、孟璋(97) 111年第2季達成率進件數減3件(7件)
        ...
        if ($sales == 68) { //廷蔚只算他牌+非仲介
            $summary1[$i]['groupcount'] = $summary1[$i]['othercount'];
        } else {
        ...
        // $season1[$sess]['targetQuota'] += $quota_case; //簽約數門檻
        if (($sess == 1) || ((date("Y", strtotime($date_start)) == '2022') && ($sess == 2))) { //2022-06-30 副總要求廷蔚(68)、偉哲(90)、孟璋(97) 111年第2季達成率進件數減3件(7件)
    ...
    for ($i = 1; $i <= 4; $i++) {
        if ($sales == 34) { //政棋
            ...
            // $season1[$i]['target'] = round(($season1[$i]['targetcount'] / (7 * 3)) * 100); //政棋固定每個月7間
            // unset($spTarget);unset($normalTarget);unset($target34);
            // }

            $_targetQuota = 21;
            if ($i == 1) { //第一季(農曆新年通常在第一季的月分中)
                $_targetQuota = floor(7 * 0.6); //農曆新年月份，案件額度打六折、小數無條件捨去
                $_targetQuota += 7 * 2; //其他兩個月份
            }

            $season1[$i]['target'] = round(($season1[$i]['targetcount'] / $_targetQuota) * 100); //政棋達成率
        } else {
    ...
    $percentTarget = 0.15; //通路簽約數占比 2022-11-17(15%)

    // $percentGroupTw = Round(($percent['pGroupTW'] / 100), 2); //成長率(台)
    // $percentGroupTw = 0.5; //成長率(台) 2022-11-17(50%)
    $percentGroupTw = 0.4; //成長率(台) 2022-12-16(40%)

    // $percentGroupUnTw = Round(($percent['pGroupUnTW'] / 100), 2); //成長率(非台)
    // $percentGroupUnTw = 0.2; //成長率(非台) 2022-11-17(20%)
    $percentGroupUnTw = 0.3; //成長率(非台) 2022-12-16(30%)

    // $percentGroupALL = Round((($percent['pGroupUnTW'] + $percent['pGroupTW']) / 100), 2); //成長率
    $percentGroupALL = 0.7; //成長率

    // $percentUse = $percent['pPercentUse']; //有效使用率
    $percentUse = 0.15; //有效使用率 2022-11-17(15%)

    // $EffectiveGoal = $percent['pEffectiveGoal']; //基準趴數
    // $EffectiveGoal = 40; //基準趴數 2022-11-17(40%)
    $EffectiveGoal = 30; //基準趴數 2022-12-16(30%)

    // $EffectiveBaseScore = $percent['pEffectiveBaseScore']; //基準趴數的基準分
    $EffectiveBaseScore = 15; //基準趴數的基準分 2022-11-17(15分)

    // $EffectivePlus = $percent['pEffectivePlus']; //變動分數(基準趴數增減多少?趴)
    $EffectivePlus = 1; //變動分數(基準趴數增減多少?趴) 2022-11-17(1%)

    // $EffectivePlus2 = $percent['pEffectivePlus2']; //變動分數(基準趴數增減多少$EffectivePlus趴就扣?分)
    $EffectivePlus2 = 1; //變動分數(基準趴數增減多少$EffectivePlus趴就扣?分) 2022-11-17(1分)
    ...
    if ($sess == 1) { //第一季(農曆新年通常在第一季的月分中)
        ...
        $targetQuota = ($sales == 34) ? 21 : (getRatioCaseQuota(getSalesCaseRatio($sales, $_date)) * 3);
    }
    ...
    if (($showseason['target'] < 80) && ($showseason['target'] >= 70)) {
        $gradeNotice .= empty($gradeNotice) ? '' : '<br>';
        $gradeNotice .= ' [本季開發總組數未達&nbsp;80%]';
    } else if ($showseason['target'] < 70) {
        $gradeNotice .= empty($gradeNotice) ? '' : '<br>';
        $gradeNotice .= ' [本季開發總組數未達&nbsp;70%]';
    }
