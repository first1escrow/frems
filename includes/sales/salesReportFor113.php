...
if ($sales) {
    ...
    if (is_numeric($sales) && in_array($sales, [90, 118, 123])) { //偉哲、俊智、和毅改用 sTaipeiQuantityThisYear(JSON)取代
        ...
        for ($x = 1; $x <= 12; $x++) {
            ...
            $summary2[$i] = empty($summary2[$i]) ? array('twcount' => 0, 'othercount' => 0, 'groupcount' => 0) : $summary2[$i];
    ...
    while (!$rs->EOF) {
        ...
        /****** 月進案件數成長率 ******/
        if (in_array($sales, [57, 68])) { //永鑫(58)、廷蔚(38) 他們台屋跟非台合併
     ...
     * 2023-12-29
     */
    $percentTarget = 0.15; //通路新簽約數 2023-12-29(15%)

    $percentGroupALL  = 0.65; //進案件成長率(全) 2023-12-29(65% = 50% + 15%)
    $percentGroupTw   = 0.5; //進案件成長率(台屋) 2023-12-29(50%)
    $percentGroupUnTw = 0.15; //進案件成長率(非台屋) 2023-12-29(15%)

    $percentUse = 0.20; //有效使用率 2023-12-29(20%)
    ...
    $EffectiveBaseScore = 20; //基準趴數的基準分 2023-12-29(20分)

    $EffectivePlus  = 1; //有效率變動分數(基準趴數增減多少?趴) 2023-12-29(1%)
    $EffectivePlus2 = 1; //有效率變動分數(基準趴數增減多少$EffectivePlus趴就扣?分) 2023-12-29(1分)
