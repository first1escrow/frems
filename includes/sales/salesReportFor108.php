 ...
 //    $seasonLast = array() ;
   ...
   // echo '<pre>' ;print_r($seasonLast) ; //exit ;
	...
	while (!$rs->EOF) {
		...
		if(($sales == 38 || $sales == 42) &&  $i < 4 && $yr == 108){
			 $check = false;
		}
	...
	for ($i=1; $i <= 4; $i++) { 
		...
		if ($i == 1) {
			...
			if ($sales == 47 && $i == 1 && $yr == 108) { //20190319 明哲說26件為滿分  政耀同意讓他改
				$season1[$i]['target'] = 100 + (($season1[$i]['targetcount'] - 26)*10);
		...
		$check = ($pOnBoard > $tmpD)? false:true;
		// echo $pOnBoard.">".$tmpD;
		if(($sales == 38 || $sales == 42) &&   $i == 1 && $yr == 108){
			 $check = false;
		}
