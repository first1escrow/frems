...
function getBranchSales($bId){
  ...
  $sql = "SELECT (SELECT pName FROM tPeopleInfo WHERE pId = bSales) AS SalesName ,bSales FROM tBranchSales WHERE bBranch = '".$bId."'";
...
function setSalesAreaLog($bid,$sales,$zip,$type)
{
	...
	$sql = "INSERT INTO tSalesAreaLog (sType,sZip,sBranch,sSales) VALUES('".$type."','".$zip."','".$bid."','".$sales."')";
...
{
	global $conn;
    $sql = "DELETE FROM tScrivenerSales WHERE sScrivener = '".$sid."'";

    $conn->Execute($sql);

  
    foreach ($sales as  $value) {
      $sql = "INSERT INTO tScrivenerSales(sSales,sScrivener,sStage) VALUES('".$value."','".$sid."','1')";
      // echo $sql."\r\n";
      $conn->Execute($sql);
    }
...
function getScrivenerSales($id){
  ...
  $sql = "SELECT (SELECT pName FROM tPeopleInfo WHERE pId = sSales) AS SalesName,sSales FROM tScrivenerSales WHERE sScrivener = '".$id."'";
...
function checkSales($id,$sales,$b='b'){
  ...
  if ($b == 'b') {
    $sql = "SELECT (SELECT pName FROM tPeopleInfo WHERE pId = bSales) AS SalesName ,bSales FROM tBranchSales WHERE bBranch = '".$id."' AND bSales ='".$sales."'";
...
function getSalesHtml($id,$cat='b'){
  ...
  if ($cat == 'b') {
    ...
    for ($i=0; $i < count($tmp); $i++) { 
      $txt .= "<span class=\"btnC\" title=\"點擊X可刪除\"><span onclick=\"delSales(".$id.",".$tmp[$i]['bSales'].")\" class=\"del\" >X</span>".$tmp[$i]['SalesName']."</span>";
...
function setSingSales($sales,$bid,$type)
{
  ...
  // $sql = "SELECT * FROM tSalesSign WHERE sType  = ".$type." AND sStore = '".$bid."' AND sSales = 0 AND sSignDate != '0000-00-00'";
