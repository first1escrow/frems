   ...
   // print_r($summary2);
	...
	while (!$rs->EOF) {
		...
		if ($sales == 34) {
			...
			$summary1[$i]['target'] = getOwnStoreTarget($summary1[$i]['targetcount'],$yr,$i,$sales,6);
		}else{
		...
		if (($sales == 57 || $sales == 65) && ($yr == 109 && $mn >= 4 ) ) { //他們台屋跟非台合併
			if ($check) { //滿一年(跟去年比)
	...
	for ($i=1; $i <= 4; $i++) { 
		...
		if ($i == 1) {
			...
			if ($sales == 34) {
				...
				$season1[$i]['target'] = round(($season1[$i]['targetcount']/(6*3))*100);//一月簽6間滿分
			}else{
