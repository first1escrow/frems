...
//傳入參數
$cCertifiedId = $_POST['pdfId'];
// $cCertifiedId = '120028845';

if (!preg_match("/^\d{9}$/", $cCertifiedId)) {
    throw new \Exception('Invalid certified id');
}
##
...
$case      = $paybycase->getPayByCase($cCertifiedId, 'S');
...
if ($contractCase['cBankList'] == '') { //結案日
    $sql                       = "SELECT tBankLoansDate FROM tBankTrans WHERE tVR_Code = " . $contractCase['cEscrowBankAccount'] . " AND tKind = '保證費'";
    $rs                        = $conn->Execute($sql);
    $contractCase['cBankList'] = $rs->fields['tBankLoansDate'];
}
##
...
foreach ($fDetail->case as $key => $value) {
    ...
    if($cCaseFeedBackMoney == 0) continue;
    //序號
    ...
    //結案日期
    $pdf->Cell(24, $cell_y5_table, $contractCase['cBankList'], 0, 0, 'C');
