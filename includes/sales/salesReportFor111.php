...
if ($sales) {
    ...
    while (!$rs->EOF) {
        ...
        if ($i == 2) {
            if ($sales == 34) {
                $summary1[$i]['target'] = getOwnStoreTarget($summary1[$i]['targetcount'], $yr, $i, $sales, 4); //達成率
            } else {
                $summary1[$i]['target'] = getOwnStoreTarget($summary1[$i]['targetcount'], $yr, $i, $sales, 6); //達成率
            }
        } else {
            if ($sales == 34) {
    ...
    for ($i = 1; $i <= 4; $i++) {
        ...
        if ($i == 1 && $yr == 111) {
            // echo $season1[$i]['targetPart']."<br>";
            if ($sales == 34) { //111年第一季滿分18
                if ($season1[$i]['targetcount'] <= 18) {
                    $season1[$i]['target'] = round($season1[$i]['targetPart'] / 3);
                    // $season1[$i]['target'] = round(($season1[$i]['targetcount']/18)*100);
                } else if ($season1[$i]['targetcount'] > 18 && $season1[$i]['targetcount'] <= 26) { //維持滿分
                    $season1[$i]['target'] = round((18 / (18)) * 100); //一月簽7間滿分
                } else {
                    $target34 = round((18 / (18)) * 100); //原本部分

                    $normalTarget = 26; //一般業務標準
                    $spTarget     = round((($season1[$i]['targetcount'] - $normalTarget) / 26) * 100); //超過26恢復原標準
        ...
        } else if ($i == 2 && $yr == 111) { //2022-06-30 副總要求廷蔚(68)、偉哲(90)、孟璋(97) 111年第2季達成率進件數減3件(7件)
            $season1[$i]['target'] = round($season1[$i]['targetPart'] / 3);
        } else {
    ...
    if ($sess == 1) {
        if ($sales == 34) {
            ...
            if ($showseason['targetcount'] < 18) {
                $gradecolor  = "#FF0000";
                $gradeNotice = '[本季尚缺簽約數' . (18 - $showseason['targetcount']) . ']';
            }
        ...
        if ($sales == 34) {
            ...
            if ((date("Y", strtotime($date_start)) == '2022') && ($sess == 2) && in_array($sales, [68, 90, 97])) { //2022-06-30 副總要求廷蔚(68)、偉哲(90)、孟璋(97) 111年第2季達成率進件數減3件(7件)
                if ((27 - $showseason['targetcount']) > 0) {
                    $gradecolor  = "#FF0000";
                    $gradeNotice = '[本季尚缺簽約數' . (27 - $showseason['targetcount']) . ']';
                }
            } else {
