...
if ($_POST['save'] == 'ok') {
    ...
    if (empty($originPassword)) {
        exit('<center>請輸入目前密碼</center>');
    }

    if (empty($newPassword)) {
        exit('<center>請輸入新密碼</center>');
    }

    if (empty($retypePassword)) {
        exit('<center>請輸入確認密碼</center>');
    }

    if ($newPassword != $retypePassword) {
        exit('<center>新密碼與確認密碼不一致</center>');
    }

    $sql  = 'SELECT pPassword FROM tPeopleInfo WHERE pId = :id;';
    $bind = [
        'id' => $id,
    ];
    $rs = $conn->one($sql, $bind);

    if (strtoupper($originPassword) != strtoupper($rs['pPassword'])) {
        exit('<center>目前密碼錯誤</center>');
    }

    $sql  = 'UPDATE tPeopleInfo SET pPassword = :password WHERE pId = :id;';
    $bind = [
