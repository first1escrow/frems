...
if ($_POST['sDate']) {
    ...
    while (!$rs->EOF) {
        ...
        if ($storeData[$rs->fields['bId']]['bAct_2021'] == 1) {
            $storeData[$rs->fields['bId']]['Act_2021'] = '辦法一';
        }

        if ($storeData[$rs->fields['bId']]['bAct_2021_gift'] == 1) {
            $storeData[$rs->fields['bId']]['Act_2021_gift'] = '7-11禮券';
        } elseif ($storeData[$rs->fields['bId']]['bAct_2021_gift'] == 2) {
            $storeData[$rs->fields['bId']]['Act_2021_gift'] = '全聯禮券';
        } elseif ($storeData[$rs->fields['bId']]['bAct_2021_gift'] == 3) {
            $storeData[$rs->fields['bId']]['Act_2021_gift'] = '現金';
        } else {
            $storeData[$rs->fields['bId']]['Act_2010_gift'] = '未設定';
        }
    ...
    ##
    //總價500萬以上 履保費有收足萬分之六，解約不計入統計，跨店0.5件計算
    $query = ' cas.cCertifiedId<>"" AND cas.cCertifiedId !="005030342" AND cas.cCaseStatus IN(2,3) AND cTotalMoney >= 5000000'; //005030342 電子合約書測試用沒有刪的樣子
...
$smarty->display('act_202110.inc.tpl', '', 'actives');
