...
$sql = '
		SELECT
			...
			(SELECT bName FROM tBranch WHERE bId = rea.cBranchNum) AS branchCompany,
			(SELECT bName FROM tBranch WHERE bId = rea.cBranchNum1) AS branchCompany1,
			(SELECT bName FROM tBranch WHERE bId = rea.cBranchNum2) AS branchCompany2,
			(SELECT bName FROM tBranch WHERE bId = rea.cBranchNum3) AS branchCompany3,
...
$branchCategory = array(1 => "加盟", 2 => '直營', 3 => '非仲介成交');
foreach ($temp_data as $v) {
    $cStoreBrand    = array();
    $cStoreBranch   = array();
    $cStoreCode     = array();
    $cStoreCompany  = array();
    $cStoreCategory = array();
    ...
    if ($v['branch'] > 0) {
        ...
        array_push($cStoreCategory, $branchCategory[$v['branchCategory']]);
        array_push($cStoreCompany, $v['branchCompany']);
    }
    ...
    if ($v['branch1'] > 0) {
        ...
        array_push($cStoreCategory, $branchCategory[$v['branchCategory1']]);
        array_push($cStoreCompany, $v['branchCompany1']);
    }
    ...
    if ($v['branch2'] > 0) {
        ...
        array_push($cStoreCategory, $branchCategory[$v['branchCategory2']]);
        array_push($cStoreCompany, $v['branchCompany2']);
    }
    ...
    if ($v['branch3'] > 0) {
        ...
        array_push($cStoreCategory, $branchCategory[$v['branchCategory3']]);
        array_push($cStoreCompany, $v['branchCompany3']);
    }
    ...
    if (!$rs->EOF) {
        $sql = "UPDATE tContractCaseReport
				...
				cStoreCompany = '" . @implode(',', $cStoreCompany) . "',
				cStoreCategory = '" . @implode(',', $cStoreCategory) . "'
				WHERE
