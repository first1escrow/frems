...
class Ku
{
    ...
    public function getCPI($date)
    {
        ...
        $tmp  = explode('/', $date);
        $date = ((int) $tmp[0] + 1911) . '-' . $tmp[1] . '-01';
        $tmp  = null;unset($tmp);

        $sql = 'SELECT cCPI FROM tCPI WHERE cDate = :date ORDER BY cId DESC LIMIT 1;';
    ...
    public function getBuildings($cId)
    {
        ...
        $buildings[$k]['parking'] = $this->getContractParking($cId);
    ...
    private function tContractTransferAreaBefore($cId, $items)
    {
        $sql = 'SELECT
                    ...
                    b.cLandPrice
                FROM
                    tContractTransferAreaBefore AS a
                JOIN tContractLandPrice AS b ON a.cCertifiedId = b.cCertifiedId
                    AND a.cLandItem = b.cLandItem
                    AND a.cItem = b.cItem
                WHERE
                    a.cCertifiedId = :cId
                    AND a.cLandItem IN (' . implode(',', $items) . ')
                ORDER BY
        ...
        foreach ($extra as $k => $v) {
            if (preg_match("/^\d{4}\-\d{2}\-\d{2}$/", $v['cMoveDate'])) {
    ...
    {
        $buyers = $this->getRegisterBuyer($cId);

        return empty($buyers) ? $this->getBuyerOwners($cId, 1) : $buyers;
    }

    private function getRegisterBuyer($cId)
    {
        //買方名義代理人 tContractOthers -> cIdentity = 5
        $sql = 'SELECT
                    a.*,
        ...
        $buyers = [];
        foreach ($rs as $v) {
            $land     = $this->getTransferArea($cId, 'L', $v['cIdentifyId']);
            $building = $this->getTransferArea($cId, 'B', $v['cIdentifyId']);
    ...
    private function getBuyerOwners($cId, $target)
    {
        ...
        $land     = $this->getTransferArea($cId, 'L', $rs['cIdentifyId']);
        $building = $this->getTransferArea($cId, 'B', $rs['cIdentifyId']);
        $item     = $this->getObjectItems($land, $building);
        $tel1     = $this->formatTelephoneNo($rs['cTelArea1'], $rs['cTelMain1']);
        $tel2     = $this->formatTelephoneNo($rs['cTelArea2'], $rs['cTelMain2']);
