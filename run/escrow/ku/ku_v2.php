...
$fh = $dir . '/kumain_' . date("YmdHis") . '.csv';
...
$csv .= '前次.p地號,前次.p權利範圍2,前次.p前移轉日,前次.p前移轉價,前次.noidno,前次.noname';
$csv = iconv("UTF-8","big5", $csv);
header("Content-type: text/csv");
file_put_contents($fh, $csv . "\n", FILE_APPEND);

$buyer_first = $owner_first = true; //買賣方第一次讀取的 flag
$land_first  = true; //土地第一次讀取的 flag
...
for ($i = 0; $i < $max_row; $i++) {
    ...
    if (!empty($buyers[$i])) {
        ...
        $data[$i][3]  = ''; //bidno.no地持分 ?
        $data[$i][4]  = ''; //bidno.no建持分 ?
    ...
    if (!empty($owners[$i])) {
        ...
        $data[$i][14] = ''; //sidno.no地持分
        $data[$i][15] = ''; //sidno.no建持分
    ...
    if (!empty($lands[$i])) {
        ...
        if (!empty($land_first)) {
            ...
            $cpi = $ku->getCPI($lands_before[$i]['cMoveDate']);
        }
    ...
    if (!empty($buildings[$i])) {
        ...
        $data[$i][83] = ''; //kutpa.a共建面1
        $data[$i][84] = ''; //kutpa.a共建面2
        $data[$i][85] = ''; //kutpa.a共建面3
        $data[$i][86] = ''; //kutpa.a共建面4
        $data[$i][87] = ''; //kutpa.a共建面5
    ...
    if (!empty($lands_before[$i])) {
        ...
        $data[$i][104] = $v['cMoveDate']; //前次.p前移轉日
        $data[$i][105] = $v['cLandPrice']; //前次.p前移轉價
