...
function post_data(url,cp) {
	var url = 'applycase2019_result.php' ;
	...
	$.post(url,
		...
		'scrivener':sc,'zip':zp,'citys':ct,'brand':bd,'undertaker':ut,'status':st,'realestate':es,'cCertifiedId':cid,
		'buyer':byr,'owner':owr,'total_page':tp,'record_limit':rl,'current_page':cp,'sales':sales},
		function(txt) {
			$('#container').html(txt) ;
	}) ;
}
...
function first() {
	...
	post_data('applycase2019_result.php',current_page) ;
}
function back() {
...
function xls_ep() {	
	$('form[name="myform"]').attr('action','applycase2019_result.php') ;
	$('[name="xls"]').val('ok') ;
	var max = "<{$max}>";
	max = max.replace(/,/, "");
...
<form method="post" name="myform">
	...
	<tr style="background-color:#E4BEB1;line-height:24px;text-align:center;width:840px;">
		...
		<td nowrap style="width:150px;border:0px;">合約保證費金額</td>
		<td nowrap style="width:100px;border:0px;">回饋金額</td>
	...
	<tr style="line-height:24px;text-align:center;background-color:#F8ECE9;">
		<td style="width:100px;border:0px;"><{$caseAna[$realKey].count}>&nbsp;</td>
		<td nowrap style="width:150px;border:0px;"><{$caseAna[$realKey].total}>&nbsp;</td>
		<td nowrap style="width:150px;border:0px;"><{$caseAna[$realKey].certifiedMoney}>&nbsp;
		</td>
		<td nowrap style="width:100px;border:0px;"><{$caseAna[$realKey].feedbackmoney}></td>
		<td nowrap style="width:100px;border:0px;"><{$caseAna[$realKey].money}></td>
		
		<!-- <td nowrap style="width:180px;border:0px;"><{$transMoney}>&nbsp;</td> -->
		<td style="width:100px;border:0px;"><span id="a_tag"><a href=# onclick="list()">檢視明細</a></span></td>
	</tr>
	<tr>
		<td colspan="5" style="height:40px;">
			<input type="button" class="bt4" value="回上一頁" onclick="go_back('applycase2019.php')">
	...
	<tr style="text-align:center;background-color:<{$item.color}>" >
		<td style="width:30px;line-height:30px;"><{$i++}></td>
		<td style="line-height:30px;"><{$item.cCertifiedId}></td>
		<td style="width:90px;line-height:30px;"><{$item.showBrandname}></td>
		<td style="width:130px;line-height:30px;"><{$item.showBranchname}></td>
		<td style="width:80px;line-height:30px;"><{$item.owner}></td>
		<td style="width:80px;line-height:30px;"><{$item.buyer}></td>
		<td style="line-height:30px;"><{$item.cTotalMoney}></td>
		<td style="line-height:30px;"><{$item.cCertifiedMoney}></td>
		<td style="width:90px;line-height:30px;">
		<{if $t_day == '結案日期'}>
			<{$item.cEndDate}>
		<{else}>
			<{$item.cSignDate}>
		...
		<td style="width:90px;line-height:30px;"><{$item.cApplyDate}></td>
		<!-- <td style="width:90px;line-height:30px;">結案日期</td> -->
		<td style="width:90px;line-height:30px;"><{$item.scrivener}></td>
		<td style="width:60px;line-height:30px;"><{$item.status}></td>
	</tr>
	<{/foreach}>
	</table>
	...
	顯示第&nbsp;<{$i_begin}>&nbsp;筆到第&nbsp;<{$i_end}>&nbsp;筆的紀錄，共&nbsp;<{$caseAna[$realKey].count}>&nbsp;筆紀錄
	...
	<input type="hidden" name="realKey" value="<{$realKey}>">
