...
function saveBook(id)
{
  ...
  var cat = $('[name="Category"]').val();
   ...
   $.ajax({
      ...
      type: 'POST',
      dataType: 'html',
      data: {'Date': date,'BookId':BookId,'money':money,'count':count,'cat':cat,'bId':id,'ContractId':$('[name="ContractId"]').val()},
   })
   .done(function(txt) {
      if (txt) {
         alert('儲存成功');
   ...
   <form  method="POST" name="form_save" id="form_save">
      ...
      <table width="80%" border="0" cellpadding="1" cellspacing="1" class="tb2">
         ...
         <tr>
            ...
            <{if $data.bMoney ==0 }>
               <{$data.expMoney}>
               <input type="hidden" name="money" value="<{$data.expMoney}>" />
            <{else}>
               <{$data.bMoney}>
               <input type="hidden" name="money" value="<{$data.bMoney}>" />
            <{/if}>
