        ...
        if (obj.msg == 'ok') {
            if (checkUID(val)) {
                if ($("[name='"+cat+"Name_"+id+"']").val() != '') {
                    if (confirm("已有資料存在，是否要取代?") === false) {
                        return;
                    }
                }

                replace_data(cat, id, obj); //身分證號查詢到的資料填入對應欄位
            }
        }
    });
}
...
//身分證號查詢到的資料填入對應欄位
function replace_data(cat, id, obj) {
    ...
    $("[name='"+cat+"Checklist_"+id+"']").click();

    for (let i = 0; i < obj.bank.length; i++) {
        if (i > 0) {
            addBank(cat,id);    
        }

        clickChecklist(cat,id);
        $("#"+cat+"BankMain_"+id+"_"+i).combobox("destroy");
        $("#"+cat+"BankMain_"+id+"_"+i).val(obj.bank[i].bankcode);

        GetBankBranchList($("#"+cat+"BankMain_"+id+"_"+i),$("#"+cat+"cBankBranch_"+id+"_"+i),obj.bank[i].bankbranch);
        $("#"+cat+"BankAccNum_"+id+"_"+i).val(obj.bank[i].bankaccnumber); //cBankMain-16021
        $("#"+cat+"BankAccName_"+id+"_"+i).val(obj.bank[i].bankaccname);//cBankAccName-16021

        setComboboxNormal(cat+"BankMain_"+id+"_"+i,'id');//再把combobox加回來
    }
}
...
function checkForeign(cat,id) {
    let pat = /[a-zA-Z]{2}/ ;
    let pat2 = /[a-zA-z]{1}[8|9]{1}[0-6]{8}/;//2021新證號
    let _val = $('#'+cat+"IdentifyId_"+id).val();//newIdentifyId_0
   
    if (pat.test(_val) || pat2.test(_val)) { //newForeign_0
...
function addBank(cat,id) {
    let count = parseInt($("#"+cat+'Index_'+id).val());
    count++;
...
<form method="POST" name="myform" id="myform">
    ...
    <table cellpadding="0" cellspacing="0" class="tb" width="100%" id="old<{$item.cId}>">
        <tr>
            <td class="Title" colspan="3">
                [<span id="no"><{$item.no}></span>]<{$_ide}>資料　(<{$cCertifiedId}>) 
                <{if $checkSave == 1}>
        ...
        <tr>
            ...
            <td>
                <input type="text" maxlength="10" style="width:120px;" class="idc invoice" id="oldIdentifyId_<{$key}>" name="oldIdentifyId_<{$key}>" value="<{$item.cIdentifyId}>" onkeyup="checkID('old','<{$key}>')"/>
        ...
        <tr>
            ...
            <td><input type="text" maxlength="10" style="width:120px;" name="oldMobileNum_<{$key}>" id="oldMobileNum_<{$key}>" value="<{$item.cMobileNum}>" class="invoice" /></td>
        </tr>
        <tr>
            <th>&nbsp;&nbsp;出生日期︰</th>
    ...
    <table cellpadding="0" cellspacing="0" class="tb row" width="100%" >
        ...
        <tr>
            <th><span class="sign-red">*</span>身分證號/統編︰</th>
            <td>
                <input type="text" maxlength="10" style="width:120px;" class="idc invoice" id="newIdentifyId_0" name="newIdentifyId_0" value="" onkeyup="checkID('new','0')"/>
                <span id="newIdc_0_img" style="padding-left:5px;" ></span>
            </td>
        </tr>
        <tr>
            <th><span class="sign-red">*</span><{$_ide}>姓名︰</th>
    ...
    <{if $checkSave == 1}>
    <div style="float:center;text-align:center"><input type="submit" value="儲存" class="xxx-button" /></div>
    <{/if}>
</form>
