...
if ($data['bCategory'] == 4) {
    $data_detail[0]['bMoney'] = ($data_detail[0]['bMoney']) ? $data_detail[0]['bMoney'] : 0;
    $txt                      = "■繳交稅款金額：新台幣" . number_format($data_detail[0]['bMoney']) . "元。";
    $pdf->MultiCell('', '1', $txt, 0, 'L', 0, 1, '', '', true, 0, false, true, 1, 'M');
} elseif ($data['bCategory'] == 3) {
    for ($i = 0; $i < count($data_detail); $i++) {
        ...
        if ($data_detail[$i]['bName'] != '' && $data_detail[$i]['bMoney'] != 0) {
            ...
            $txt                       = "■ 開立本行支票，抬頭：" . $data_detail[$i]['bName'] . "，" . $data_detail[$i]['bStop'] . "背書轉讓，金額：" . number_format($data_detail[$i]['bMoney']) . "元。";
...
// $txt = '轉入分行：台新銀行'.$data['bReBank'].'分行臨時存欠過度會計科目帳戶';
// $pdf->MultiCell('', "1", $txt, 0, 'L', 0, 1, '', '', true, 0, false, true, 1, 'M');

// $txt = '700-6600000-1002';
// $pdf->MultiCell('', "1", $txt, 0, 'L', 0, 1, '', '', true, 0, false, true, 1, 'M');
...
$tmp_y = $pdf->getY();
$txt   = '請依上列指示事項通知  貴行' . $data['bReBank'] . '分行';
$pdf->MultiCell('', "1", $txt, 0, 'L', 0, 1, '', '', true, 0, false, true, 1, 'M');

$tmp_y = $pdf->getY();
$txt   = '※開立本行支票取款交易後，請協助回傳簽收單';
$pdf->MultiCell('', "1", $txt, 0, 'L', 0, 1, '', '', true, 0, false, true, 1, 'M');
