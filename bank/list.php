    ...
    if ($_GET['cat'] == 1) { //以出款項目為主
        ...
        if (is_array($data[$conBank[$i]['cBankVR']])) {
            ...
            foreach ($data[$conBank[$i]['cBankVR']] as $v) {
                ...
                echo "<td width=\"258\">";
                echo "<input type=\"checkbox\" name=\"a" . $v["tVR_Code"] . "\"  onclick=\"checkALL('" . $v["tVR_Code"] . "')\" " . $legalAllow . ">
										專屬帳號 <font color=\"blue\"><strong>" . $v["tVR_Code"] . "</strong></font>" . $legalNotice;
                ...
                $applyBankRelay = '';
                if(in_array($v["tVR_Code"], $VRCodes)) {
                    $applyBankRelay = '<span style="color:red;font-weight:bold;">(申請代墊利息)</span>';
                }

                echo "<td width=\"213\">筆數 " . $v["count"] . "{$_payOkBalance}{$applyBankRelay}</td>";
        ...
        while (!$rs->EOF) {
            ...
            $data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['tVR_Code']    = $rs->fields["tVR_Code"];
            $data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['tLegalAllow'] = $rs->fields["tLegalAllow"];
            $data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['data'][]      = $rs->fields;
            if($rs->fields['tInvoice'] != null) {
                $data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['isInvoice']++;
            }
            if(in_array($rs->fields['tObjKind'], ['點交(結案)', '解除契約', '建經發函終止', '預售屋'])) {
                $data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['close']++;
            }
            if($rs->fields["tPayOk"] == '2') {
                $data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['payOkTotal'] += $rs->fields["tMoney"];
            }
            if (empty($data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['legalAllow'])) {
                $data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['legalAllow'] = $rs->fields["tLegalAllow"];
            }
            $data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['legalAllow'] = ($data[$conBank[$i]['cBankVR']][$rs->fields['tOwner']][$rs->fields["tVR_Code"]]['legalAllow'] == '1') ? '1' : $rs->fields["tLegalAllow"];
