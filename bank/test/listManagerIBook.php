...
$aResultFilterTotal = array();
while (!$rResultFilterTotal->EOF) {
    array_push($aResultFilterTotal, $rResultFilterTotal->fields);

    $rResultFilterTotal->MoveNext();
}
...
while (!$rResultTotal->EOF) {
    array_push($aResultTotal, $rResultTotal->fields);
        ...
        if ($aColumns[$i] == "version") {
            ...
            if($aColumns[$i] == 'bBank2'){
               ...
               if($_SESSION['pBankBook'] == 2 || $_SESSION['pBankBook'] == 3){
                    $link .= '&nbsp;&nbsp;|&nbsp;&nbsp; <a href="javascript:void(0)" onclick="Audit('.$aRow['bId'].')">已審核</a>';
                }
