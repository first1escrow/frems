 ...
 //    $seasonLast = array() ;
   ...
   // echo '<pre>' ;print_r($seasonLast) ; //exit ;
	...
	while (!$rs->EOF) {
		...
		if ($sales == 34) {
			$summary1[$i]['target'] = getOwnStoreTargetFor34($summary1[$i]['targetcount'],$yr,$i,$sales);   //達成率
		}else{
			$summary1[$i]['target'] = getOwnStoreTarget($summary1[$i]['targetcount'],$yr,$i,$sales);   //達成率
		}
		...
		if ($sales == 57 || $sales == 65) { //他們台屋跟非台合併
			if ($check) { //滿一年(跟去年比)
				...
				// echo 'GO_';
				// $summary1[$i]['groupTWshow'] = getPercentMonth107($sales,($yr+1911),$i,$summary2[$i]['twcount'],$summary1[$i]['twcount'],'gTw',$check,'show');  //成長率
				// $summary1[$i]['groupUnTWshow'] = getPercentMonth107($sales,($yr+1911),$i,$summary2[$i]['othercount'],$summary1[$i]['othercount'],'gUnTw',$check,'show');  //成長率
				// $summary1[$i]['groupTW'] = getPercentMonth107($sales,($yr+1911),$i,$summary2[$i]['twcount'],$summary1[$i]['twcount'],'gTw',$check);  //成長率
				// $summary1[$i]['groupUnTW'] = getPercentMonth107($sales,($yr+1911),$i,$summary2[$i]['othercount'],$summary1[$i]['othercount'],'gUnTw',$check);  //成長率
