...
include_once '../includes/escrow/contractbank.php' ;
//預載log物件
$logs = new Intolog() ;
##
...
//$advance->ConvertOption($data, $value, $option);
...
// $data_property=$contract->GetProperty($id);
...
while (!$rs->EOF) {
	...
	if (preg_match("/0000\-00\-00/",$data_property[$i]['cClosingDay'])) { $data_property[$i]['cClosingDay'] = '' ; }
	else { $data_property[$i]['cClosingDay'] = $advance->ConvertDateToRoc($data_property[$i]['cClosingDay'], base::DATE_FORMAT_NUM_DATE); }
...
//第一組仲介店
$branch_opt = $brand->GetBranchList($data_realstate['cBrand'],$data_realstate['bCategory']) ;
...
unset($max) ; unset($branch_opt) ;
//本票同意書換成服務費先行撥付同意書
if ($data_realstate['bServiceOrderHas1']) { $promissory1 = '有' ; }
else { $promissory1 = '無' ; }
##

//第二組仲介店
$branch_opt1 = $brand->GetBranchList($data_realstate['cBrand1'],$data_realstate['bCategory1']) ;
...
if ($data_realstate['bServiceOrderHas2']) { $promissory2 = '有' ; }
else { $promissory2 = '無' ; }
##

//第三組仲介店
$branch_opt2 = $brand->GetBranchList($data_realstate['cBrand2'],$data_realstate['bCategory2']) ;
...
if ($data_realstate['bServiceOrderHas3']) { $promissory3 = '有' ; }
else { $promissory3 = '無' ; }
##

//第四組仲介店
$branch_opt3 = $brand->GetBranchList($data_realstate['cBrand3'],$data_realstate['bCategory3']) ;
...
unset($max) ; unset($branch_opt3) ;
// echo $data_realstate['cBranchNum1']."_".$data_realstate['cBranchNum2']."_".$data_realstate['cBranchNum3'];
...
//取得其他買賣方利息金額(增加查詢姓名及發票金額)
	$sql = 'SELECT cInterestMoney,cInvoiceMoney,cName,cIdentity,cInvoiceDonate,cId,cInvoicePrint FROM tContractOthers WHERE cCertifiedId="'.$id.'" ORDER BY cId ASC;' ;
...
$list_categorybank_base = $contract->GetCategoryBank();
$menu_categorybank_base = $contract->ConvertOption($list_categorybank_base, 'cId', 'cBankName');
//$list_categorybank_twhg = $contract->GetCategoryBank(array(8, 77, 61));
$list_categorybank_twhg = $contract->GetCategoryBank(array(8, 77, 68, 80));
...
// $menu_scrivener = $scrivener->ConvertOption($list_scrivener, 'sId', 'sName');
...
$menu_categoryarea = $contract->GetCategoryAreaMenu();
...
//取得總行(2)選單
//$buyer_menu_main = getBankMain($conn,$data_buyer['cBankKey2']) ;
##
...
if (preg_match("/0000\-00\-00/",$data_land['cMoveDate'])) { $data_land['cMoveDate'] = '' ; }
else { $data_land['cMoveDate'] = $advance->ConvertDateToRoc($data_land['cMoveDate'], base::DATE_FORMAT_NUM_MONTH); }

$data_buyer['cBirthdayDay'] = $advance->ConvertDateToRoc($data_buyer['cBirthdayDay'], base::DATE_FORMAT_NUM_DATE);

if (preg_match("/0000-00-00/",$data_buyer['cPaymentDate'])) { $data_buyer['cPaymentDate'] = '' ; }
else { $data_buyer['cPaymentDate'] = $advance->ConvertDateToRoc($data_buyer['cPaymentDate'], base::DATE_FORMAT_NUM_DATE); }

$data_owner['cBirthdayDay'] = $advance->ConvertDateToRoc($data_owner['cBirthdayDay'], base::DATE_FORMAT_NUM_DATE);

if (preg_match("/0000-00-00/",$data_owner['cPaymentDate'])) { $data_owner['cPaymentDate'] = '' ; }
else { $data_owner['cPaymentDate'] = $advance->ConvertDateToRoc($data_owner['cPaymentDate'], base::DATE_FORMAT_NUM_DATE); }
...
if ($data_realstate['cBranchNum2'] > 0) {
	...
	$rel3 = $rs->fields ;
	//回饋對象資料
	// $data_feedData3 = FeedBackData($data_realstate['cBranchNum2'],2);
...
//修正地址縣市區域重複
// $data_property['cAddr'] = filterCityAreaName($conn,$data_property['cZip'],$data_property['cAddr']) ;
...
}elseif ($data_realstate['cBrand1']!=1&&$data_realstate['cBrand1']!='0') {
...
}elseif ($data_realstate['cBrand2']!=1&&$data_realstate['cBrand2']!='0') {
...
//如果有仲介代書回饋比率，就顯示
if($data_case['cSpCaseFeedBackMoney'] > 0){
	$sSpRecall ='';
}else{
	$sSpRecall = 'none';
}
//cBrandRecall
// if ($data_case['cBranchScrRecall'] == 0 && $data_case['cBranchScrRecall1'] == 0 && $data_case['cBranchScrRecall2'] == 0 && 
// 	$data_case['cBrandScrRecall'] == 0 && $data_case['cBrandScrRecall1'] == 0 && $data_case['cBrandScrRecall2'] == 0 && 
// 	$data_case['cBrandRecall'] == 0 && $data_case['cBrandRecall1'] == 0 && $data_case['cBrandRecall2'] == 0) {
// 	if($check==0 || ($scrivener_sSpRecall == 0 && $data_scrivener['sSpRecall2'] == '' && $data_case['cScrivenerRecall'] == '')){
// 		$sSpRecall ='none';
// 	}else{
// 		$sSpRecall ='';
// 	}
// }else if($data_case['cSpCaseFeedBackMoney'] > 0){
// 	$sSpRecall ='';
// }
##
...
for ($j = 1 ; $j < 7 ; $j ++) {
	if ($data_case['cCaseStatus']=='3' || ($data_case['cLastTime'] >= '107-01-24 00:00:00' && ($data_case['cCaseStatus']==4 || $data_case['cCaseStatus']==9))) {	$_index = "" ; }
	else { $index = ' onclick="processing('.$j.')"' ; }
...
##
//($data_case['cLastTime'] >= '2018-01-24' && ($data_case['cCaseStatus']==4 || $data_case['cCaseStatus']==9))
...
while (!$rs->EOF) {
	...
	if (($rs->fields['tObjKind'] == '點交(結案)' || $rs->fields['tObjKind'] == '解除契約') && $rs->fields['tKind'] == '賣方') {
		$checkCaseEnd = true;//有出結案
		$ownerExportAccountName[] = $rs->fields['tAccountName']; //賣方出款帳戶
		if (!in_array($rs->fields['tAccountName'], $ownerArr)) { //非賣方帳戶要顯示填寫
			$checkOwnerNote = false;
	...
	// if (($rs->fields['tObjKind'] == '點交(結案)' || $rs->fields['tObjKind'] == '解除契約') ) {
	// 	$checkCaseEnd = true;//有出結案
	// 	if ($rs->fields['tKind'] == '賣方') {
	// 		if (in_array($rs->fields['tAccountName'], $ownerArr)) { //出款裡面有賣方
	// 			$checkOwnerNote = true;
...
//還未出結案
if ($checkCaseEnd == false || $data_case['cBank'] == 68) {
	$checkOwnerNote = true;
}
...
// $total -= $minus_money;//減掉未兌現
// $total -= $minus_money + 1 - 1 ;
if ($tbl == '') {
	$tbl = '
...
$imgStampEdit = '' ;
$imgStamp = '' ;
$sql = 'SELECT bId FROM tContractBranchStamp WHERE bCertifiedId = "'.$id.'" AND bBranchId = "'.$data_realstate['cBranchNum'].'";' ;
$rs = $conn->Execute($sql) ;
if ($rs->RecordCount() > 0) {
	$imgStampEdit = '1' ;
	if (!$rs->EOF) $imgStamp = '<div onclick="newImg('.$data_realstate['cBranchNum'].', \''.$id.'\')" style="cursor:pointer;"><img src="showcIdStamp.php?bId='.$data_realstate['cBranchNum'].'&cId='.$id.'" style="width:236px;height:135px;"></div>' ;
}
##
...
$imgStampEdit1 = '' ;
$imgStamp1 = '' ;
$sql = 'SELECT bId FROM tContractBranchStamp WHERE bCertifiedId = "'.$id.'" AND bBranchId = "'.$data_realstate['cBranchNum1'].'";' ;
$rs = $conn->Execute($sql) ;
if ($rs->RecordCount() > 0) {
	$imgStampEdit1 = '1' ;
	if (!$rs->EOF) $imgStamp1 = '<div onclick="newImg('.$data_realstate['cBranchNum1'].', \''.$id.'\')" style="cursor:pointer;"><img src="showcIdStamp.php?bId='.$data_realstate['cBranchNum1'].'&cId='.$id.'" style="width:236px;height:135px;"></div>' ;
}
##
...
$imgStampEdit2 = '' ;
$imgStamp2 = '' ;
$sql = 'SELECT bId FROM tContractBranchStamp WHERE bCertifiedId = "'.$id.'" AND bBranchId = "'.$data_realstate['cBranchNum2'].'";' ;
$rs = $conn->Execute($sql) ;
if ($rs->RecordCount() > 0) {
	$imgStampEdit2 = '1' ;
	if (!$rs->EOF) $imgStamp2 = '<div onclick="newImg('.$data_realstate['cBranchNum2'].', \''.$id.'\')" style="cursor:pointer;"><img src="showcIdStamp.php?bId='.$data_realstate['cBranchNum2'].'&cId='.$id.'" style="width:236px;height:135px;"></div>' ;
}
##
// 回饋金修改者
...
while (!$rs->EOF) {
	...
	$sql = "SELECT * FROM tFeedBackMoneyReviewList WHERE fCertifiedId = '".$id."' AND fRId = '".$SalesReview[$i]['fId']."' AND fDelete = 0 ORDER BY fCategory ASC";
	...
	while (!$rs2->EOF) {
		if ($rs2->fields['fCategory'] == 1) {
			...
			$SalesReview[$i]['data'][$j] = getStoreData($SalesReview[$i]['data'][$j]['fFeedbackTarget'],$rs2->fields['fFeedbackStoreId']);
...
//如果已經結案次月後
//就不行在改回進行中 做內容修改了
// $tmp = substr($data_case['cFinishDate3'],6,2);
$disabled_caseStatus = '';
if ($data_case['cFinishDate3'] != '0000-00-00 00:00:00' ) { //已更改時間去鎖
	...
	if ($month_check <= date('Ym')) {
		...
		// if ($month_check <= date('Ym') && $disabled_caseStatus == '' && $data_case['cEndDate'] != '0000-00-00 00:00:00') {
		// 	// $disabled_caseStatus = 'disabled=disabled';
		// 	// echo 'B';
		// }
	}
	
}elseif (($data_case['cFinishDate3'] == '0000-00-00 00:00:00' && $data_case['cEndDate'] != '0000-00-00 00:00:00' ) ) {
	$month_check = date("Ym",mktime(0,0,0,(substr($data_case['cEndDate'],4,2)+1),substr($data_case['cEndDate'],8,2),(substr($data_case['cEndDate'],0,4)+1911))) ;
	if ($month_check <= date('Ym')) {
		$disabled_caseStatus = 'disabled=disabled';
		// echo 'C';
...
// echo $checkCertifiedFee;


##
$undate = (substr($data_case['cEndDate'], 0,4)+1911)."-".substr($data_case['cEndDate'], 4);
// echo $undate."_".strtotime($undate)."_".strtotime("2018-12-13 00:00:00");
if ($data_income['cInspetor2'] > 0 && $data_case['cCaseStatus'] == 2) {
	$unf = 1;
	// echo 'C';
}elseif (strtotime($undate) <= strtotime("2018-12-13 00:00:00") && $data_case['cCaseStatus'] == 3) {
	$unf = 1;
	// echo 'A';
}elseif (strtotime($undate) > strtotime("2018-12-13 00:00:00") && $data_case['cCaseStatus'] == 3) {
...
//賣方備註
$sql = "SELECT tAnother,tAnotherNote,eSend FROM tBankTransSellerNote WHERE tCertifiedId = '".$id."'" ;
...
unset($BankCount);unset($BankCountCk);
##
if (is_array($caseSales)) {
	# code...
}
##
//檢查票據是否兌現(日期檢查)
...
$smarty->assign('menu_categorybank_base', $menu_categorybank_base);
...
// $smarty->assign('data_feedDataCount1',count($data_feedData1)); //回饋金資料1
// $smarty->assign('data_feedDataCount2',count($data_feedData2)); //回饋金資料2
// $smarty->assign('data_feedDataCount3',count($data_feedData3)); //回饋金資料3
