...
while( !$rs->EOF ) {    
	$checked = '';
	if (is_array($_POST['CertifiedId'])) {
		if (in_array($rs->fields['tMemo'],$_POST['CertifiedId'] )) {
   ...
   $kind = ($rs->fields['tCode2'] == '大額繳稅' || $rs->fields['tCode2'] == '臨櫃開票')? '【'.$rs->fields['tCode2'].'】':'';
    ...
    echo "<div style=\"padding: 5px; text-align: left;margin-left: 30%;\">
                <span class=\"\"><input type=\"checkbox\" name=\"CertifiedId[]\" value=\"".$rs->fields['tMemo'].'_'.$rs->fields['tObjKind']."\" id=\"CertifiedId".$rs->fields['tMemo']."\" ".$checked."><label for=\"CertifiedId".$rs->fields['tMemo']."\"><span></span>".$rs->fields['tMemo']."_".$rs->fields['tObjKind'].$kind."</label></span>
            </div>";
