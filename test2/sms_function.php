...
class SMS_Gateway extends PDO{
  ...
  public function __construct() {
	...
	public function send($pid,$sid,$bid,$target,$tid,$ok="n",$realty=0,$arr=null,$stxt=''){
		...
		switch($target) {
			...
			case "income" :
				...
				//
				$_data = array(); $_data1 = array(); $_data4 = array() ;  $_special = array(); $ownerBranch = array();
				$_data5 = array() ;
				...
				if ($checkService == 2) {
					$ownerBranch = array_merge($ownerBranch,$_data1,$_data4,$_data5) ;
					// echo 'A';
				}else{

					$_data = array_merge($_data,$_data1,$_data4,$_data5) ;
					// echo 'B';
				...
				unset($_data1) ; unset($_data4) ;unset($checkService);unset($_data5);
				...
				if ($bid2 > 0) {
					$_data1 = array(); $_data4 = array() ; $_data5 = array();
				...
				if ($bid3 > 0) {
					$_data1 = $_data4 = array() ;  $_data5 = array();
				...
				if (((($money - $fetchValue[0]["eBuyerMoney"]) > 0) && ($fetchValue[0]["eBuyerMoney"] > 0))||$fetchValue[0]['eExtraMoney'] >0) {
					...
					foreach ($ownerBranch as $k => $v) {
						$_special[$_owner_no]["mName"] =  $v['mName'] ;			 
						$_special[$_owner_no]["mMobile"] =  $v['mMobile'] ;	
						$_special[$_owner_no]["iden"] = 'oBranch';
						$_special[$_owner_no]['boss'] = $v['boss'];
						$_special[$_owner_no]['smsText'] = $v['smsText'];
						$_special[$_owner_no]["tTitle"] = '賣方仲介';
						$_owner_no ++ ;
					}
					...
					for ($i=0; $i < count($other_phone); $i++) { 
						$_special[$_owner_no]["mName"] = $other_phone[$i]["cName"]	; 					
						$_special[$_owner_no]["mMobile"] = $other_phone[$i]['cMobileNum']	;
						$_special[$_owner_no]["tTitle"] = '賣方經紀人';
						$_owner_no ++ ;
					}
					...
					foreach ($ownerBranch as $k => $v) {
						...
						$_all[$_start+$_t]["tTitle"] = '賣方仲介';
						$_all[$_start+$_t]['smsText'] = $v['smsText'];
						$_t ++ ;
					}
				...
				$_special = $this->filter_array($_special,$boss) ;
				$_total = count($_all);
				...
				if ($ok == 'y'){
					...
					if (count($boss) > 0) {
						...
						$_boss = $this->sendBossSms($boss,$sms_txt_b,$target,$pid,$tid,$sys,$ok); //******
						...
						for ($i = 0 ; $i < count($_special) ; $i ++) {
							...
							if (preg_match('/^09[0-9]{8}$/',$mobile_tel)) {
								...
								if ($_special[$i]['boss'] == 1) { //賣方BOSS
									...
									$sms_id = $this->sms_send($mobile_tel,$mobile_name,$sms_txt_b,$target,$pid,$tid,$sys) ;
									unset($sms_txt_b);
								}else{
									$sms_id = $this->sms_send($mobile_tel,$mobile_name,$sms_txt,$target,$pid,$tid,$sys) ;
								}
			...
			case 'income2':
					...
					//
					$_data = array(); $_data1 = array(); $_data4 = array() ; $_data5 = array() ; $ownerBranch = array(); $_all = array();
					...
					if ($bid2 > 0) {
						...
						unset($_data1) ; unset($_data4) ; unset($_data5);
					...
					$sms_txtC = '第一建經信託履約保證專戶已於'.$M.'月'.$D.'日收到保證編號' .substr($pid,5,9).'（買方'.$buyer.$bCount.'賣方'.$seller.$oCount.'）存入'.$changeMoney.'元' ;
					...
					if ($ok == 'y'){
						...
						if ($memo['owner'] != '' ) {
							...
							$owner_sptxtC = '第一建經信託履約保證專戶已於'.$M.'月'.$D.'日收到保證編號' .substr($pid,5,9).'（買方'.$buyer.$bCount.'賣方'.$seller.$oCount.'）存入'.($changeMoney).'元' ;
							...
							for ($i = 0 ; $i < count($_special) ; $i ++) {
								...
								if (preg_match('/^09[0-9]{8}$/',$mobile_tel)) {
									...
									//買賣方不看調帳
								if ($changeMoney > 0) {
									if ($_special[$i]['iden'] == 'buyer' || $_special[$i]['iden'] == 'owner') {
			...
			case "cheque" :
				...
				//增加地政士與第一組仲介簡訊發送
				$_data = array();$_data1 = array();$_data4 = array();$_data5 = array();$manager = array() ;
				...
				unset($_data,$_data1,$_data4,$_data5) ;
			...
			case "chequetaisin" :
				...
				if ($bid4 > 0) {
					$_data1 = $this->getsBranchMobile($pid,$bid4,'店長'); // 取得店發送簡訊對象
					$_data4 = $this->getsBranchMobile($pid,$bid4,'店東'); // 取得店發送簡訊對象
					$_data5 = $this->getsBranchMobile($pid,$bid4,'經紀人');
					$manager = array_merge($manager,$_data1,$_data4,$_data5);
				...
				$_total = count($_all);
				// $arr = array(0=>'0935615538',1=>'0910613800');
				...
				if ($ok == 'y') {
					for ($i = 0 ; $i < $_total ; $i ++) {
						...
						if (trim($_all[$i]["mMobile"]) != "") {
							...
							//開始發送簡訊
							if (preg_match('/^09[0-9]{8}$/',$mobile_tel)) {
								if (($fetchValue[0]['tCode2'] == '大額繳稅' || $fetchValue[0]['tCode2'] == '臨櫃開票') && $_all[$i]['tTitle'] == '地政士' ) {
									if ($fetchValue[0]['tCode2'] == '大額繳稅') {
										$sms_txt_scrivener = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount.",賣方".$seller.$oCount."之稅款已完成作業，請於".$M.'/'.$D."前往指定分行辦理";
										$sms_id = $this->sms_send($mobile_tel,$_all[$i]["mName"],$sms_txt_scrivener,$target,$pid,$tid,$sys) ;
									}elseif ($fetchValue[0]['tCode2'] == '臨櫃開票') {
										$sms_txt_scrivener = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount.",賣方".$seller.$oCount."之臨櫃開票已完成作業，請於".$M.'/'.$D."前往指定分行辦理";
										$sms_id = $this->sms_send($mobile_tel,$_all[$i]["mName"],$sms_txt_scrivener,$target,$pid,$tid,$sys) ;
									}
					...
					if ($manager) {
						...
						// $_boss = $this->sendBossSms($manager,$sms_txt,$target,$pid,$tid,$sys,$ok);
						// $tmp = substr($pid,5,9);
			...
			case "點交(結案)":
				...
				if (is_array($manager)) {
					...
					if ($realty > 0) {
						$bb = $this->sendCaseEndRealty($pid,$fetchValue[0]["tExport_nu"],"n",$noSend,$target,$tid,$sys,($buyer.$bCount),($seller.$oCount),$M.'/'.$D);
				...
				if ($ok == 'y') {
					...
					if ($realty > 0)  {//服務費 另外發送
						...
						$this->sendCaseEndRealty($pid,$fetchValue[0]["tExport_nu"],$ok,$noSend,$target,$tid,$sys,($buyer.$bCount),($seller.$oCount),$M.'/'.$D);
					}

					//店長、店東
					$addr = $this->getProperty(substr($pid,5,9),'all');
					$sms_txt_b = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount.",賣方".$seller.$oCount.";".$addr.",已完成點交作業,各應收款項已於".$M.'/'.$D."匯入指定帳戶" ;
					$_boss = $this->sendBossSms($manager,$sms_txt_b,$target,$pid,$tid,$sys,$ok); //******
					unset($sms_txt_b);
			...
			case "解除契約":
				...
				if ($bid3) {
					$_data1 = $this->getsBranchMobile($pid,$bid3,'店長'); // 取得店發送簡訊對象
					$_data4 = $this->getsBranchMobile($pid,$bid3,'店東'); // 取得店發送簡訊對象
					$_data5 = $this->getsBranchMobile($pid,$bid3,'經紀人'); 
					$manager = array_merge($manager,$_data1,$_data4);
					$_all = array_merge($_all,$_data5);
				}
				unset($_data1) ; unset($_data4) ;unset($_data5);
				...
				$sms_txt = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount." ,賣方".$seller.$oCount."已完成 `解約/終止履保作業` ,各應收款項已於".$M.'/'.$D."匯入指定帳戶";
				if ($ok == 'n') {
					echo $sms_txt;
				}
				...
				if ($ok == 'y') {
					...
					$addr = $this->getProperty(substr($pid,5,9),'all');
					$sms_txt_b = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount." ,賣方".$seller.$oCount.";".$addr."已完成 `解約/終止履保作業` ,各應收款項已於".$M.'/'.$D."匯入指定帳戶";
			...
			case "仲介服務費" :
				...
				$this->execSQL = "select bt.*,cc.cSignDate from tBankTrans AS bt LEFT JOIN tContractCase AS cc ON cc.cCertifiedId=bt.tMemo where bt.tVR_Code='".$pid."' and bt.tId=$tid";
				$getMD = $this->DB_link->prepare($this->execSQL);
				$getMD->execute();
				$fetchValue = $getMD->fetchALL(PDO::FETCH_ASSOC);
				...
				if ($ok == 'y') {
					for ($i = 0 ; $i < $_total ; $i ++) {
						...
						if (trim($_all[$i]["mMobile"]) != "") {
							...
							if ($_all[$i]['smsText'] != '') {
								$replace = $addr.'('.$_all[$i]['smsText'].")";
								if ($_all[$i]['smsTextStyle'] == 1) {
									//簽約日+買方姓名+賣方姓名+門牌+(京)+服務費內容。(不要有履保編號)
									$sms_txt = '第一建經信託履約保證專戶通知：簽約日'.substr($fetchValue[0]["cSignDate"], 5,2)."月".substr($fetchValue[0]["cSignDate"], 8,2).'日,買方'.$buyer.',賣方'.$seller.';'.$addr.','.$_money.'元已於'.$date.'匯入仲介指定帳戶';
								}
							...
							if (preg_match('/^09[0-9]{8}$/',$mobile_tel)) {
								...
								$sms_id = $this->sms_send($mobile_tel,$_all[$i]["mName"],$sms_txt_b,$target,$pid,$tid,$sys) ;
								unset($sms_txt_b);
					...
					// 		echo $mobile_tel."_".$_all[$i]["mName"]."_".$sms_txt_b."_".$target."_".$pid."_".$tid."_".$sys."<br>";
					// 	}
					// }
					//回覆簡訊發送結果
					return $_all ;
			...
			case "代清償" :
				...
				if ($bid2) {
					$_data1 = $this->getsBranchMobile($pid,$bid2,'店長'); // 取得店發送簡訊對象
					$_data4 = $this->getsBranchMobile($pid,$bid2,'店東'); // 取得店發送簡訊對象
					$_data5 = $this->getsBranchMobile($pid,$bid2,'經紀人');
					$manager = array_merge($manager, $_data1, $_data4) ;
					$_all = array_merge($_all,$_data5);
				}
				...
				if ($ok == 'y'){
							...
							if (preg_match('/^09[0-9]{8}$/',$mobile_tel)) {
								if (($fetchValue[0]['tCode2'] == '大額繳稅' || $fetchValue[0]['tCode2'] == '臨櫃開票') && $_all[$i]['tTitle'] == '地政士') {
									if ($fetchValue[0]['tCode2'] == '大額繳稅') {
										$sms_txt_scrivener = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount.",賣方".$seller.$oCount."之稅款已完成作業，請於".$M.'/'.$D."前往指定分行辦理";
										$sms_id = $this->sms_send($mobile_tel,$_all[$i]["mName"],$sms_txt_scrivener,$target,$pid,$tid,$sys) ;
									}else if($fetchValue[0]['tCode2'] == '臨櫃開票'){
										$sms_txt_scrivener = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount.",賣方".$seller.$oCount."之臨櫃開票已完成作業，請於".$M.'/'.$D."前往指定分行辦理";
										$sms_id = $this->sms_send($mobile_tel,$_all[$i]["mName"],$sms_txt_scrivener,$target,$pid,$tid,$sys) ;
									}
					...
					##
					$addr = $this->getProperty(substr($pid,5,9),'all');
					$sms_txt_b = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：".$_seller_txt.";".$addr."代償金額新台幣".$_money."元已於".$M.'/'.$D."匯入指定帳戶" ;
					$_boss = $this->sendBossSms($manager,$sms_txt_b,$target,$pid,$tid,$sys,$ok); //******
					unset($sms_txt_b);
			...
			case "賣方先動撥" : //20150917買賣方不收簡訊
				...
				$_data1 = $this->getsBranchMobile($pid,$bid,'店長'); // 取得店發送簡訊對象
				$_data4 = $this->getsBranchMobile($pid,$bid,'店東'); // 取得店發送簡訊對象
				$_data5 = $this->getsBranchMobile($pid,$bid,'經紀人');
				$manager = array_merge($manager, $_data1, $_data4) ;
				$_all = array_merge($_all,$_data5);
				...
				if ($ok == 'y') {
					...
					////////
					$addr = $this->getProperty(substr($pid,5,9),'all');
					$sms_txt_b = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）賣方".$seller.$oCount.";".$addr.",動用買賣價金新台幣".$money."元已於".$M.'/'.$D."存入賣方指定帳戶" ;
			...
			case "保留款撥付":
				...
				//
				$_data = array() ; $_data1 = array() ; $_data4 = array() ; $manager = array() ; $_data5 = array() ;
				...
				if ($ok == 'y') {
					...
					$addr = $this->getProperty(substr($pid,5,9),'all');
					$sms_txt_b = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount."賣方".$seller.$oCount.";".$addr.",已完成保留款撥付作業,應收款項已於".$M.'/'.$D."匯入指定帳戶";
			...
			case '預售屋':
				...
				//有服務費
					...
					$_data1 = array() ; $_data4 = array() ; $_data5 = array(); $manager = array() ;
					$_data1 = $this->getsBranchMobile($pid,$bid,'店長'); // 取得店發送簡訊對象
					$_data4 = $this->getsBranchMobile($pid,$bid,'店東'); // 取得店發送簡訊對象
					$_data5 = $this->getsBranchMobile($pid,$bid,'經紀人'); // 取得店發送簡訊對象
					$manager = array_merge($manager,$_data1,$_data4);
				...
				$sms_txt = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount.",賣方".$seller.$oCount."已完成 `預售屋作業` ,各應收款項已於".$M.'/'.$D."匯入指定帳戶";
				if ($ok == 'n') {
					echo $sms_txt;
				}
				...
				if ($ok == 'y') {
					...
					// $sms_txt = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount.",賣方".$seller.$oCount.";".$addr.",已完成點交作業,各應收款項已於".$M.'/'.$D."匯入指定帳戶" ;
					$sms_txt = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCount.",賣方".$seller.$oCount.";".$addr."已完成 `預售屋作業` ,各應收款項已於".$M.'/'.$D."匯入指定帳戶";
					$_boss = $this->sendBossSms($manager,$sms_txt,$target,$pid,$tid,$sys,$ok); //******
	...
	public function send2($pid,$target,$tid,$nu,$ok="n",$realty=0,$storeId=0){
		...
		for ($i=0; $i < count($BankTranSms); $i++) { 
			if ($BankTranSms[$i]['bStoreId'] > 0) {
				if ($BankTranSms[$i]['bIden'] == '店東' || $BankTranSms[$i]['bIden'] == '店長') {
					...
					$branchData = $this->getBranchData($BankTranSms[$i]['bStoreId']);
					$BankTranSms[$i]['smsText'] = $branchData['bSmsText'];
					$BankTranSms[$i]['smsTextStyle'] = $branchData['bSmsTextStyle'];
					unset($branchData);
				}
		...
		$bCount += count($_others);
		unset($_others);
		//計算賣方人數
		$oCount = 1;
		$_others = $this->get_others($pid,'2') ;
		$oCount += count($_others);
		...
		switch ($target) {
			...
			case '解除契約':
				...
				$sms_txt = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCountTxt." ,賣方".$seller.$oCountTxt."已完成 `解約/終止履保作業` ,各應收款項已於".$M.'/'.$D."匯入指定帳戶";
				//仲介簡訊
				// $sms_txt_b = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCountTxt.",賣方".$seller.$oCountTxt.";".$addr.",已完成解除契約作業,應收款項已於".$M.'/'.$D."匯入指定帳戶" ;
				$sms_txt_b = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCountTxt." ,賣方".$seller.$oCountTxt.";".$addr."已完成 `解約/終止履保作業` ,各應收款項已於".$M.'/'.$D."匯入指定帳戶";
			...
			case '保留款撥付':
				...
				break;
			case '預售屋':
				$sms_txt = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCountTxt.",賣方".$seller.$oCountTxt.";已完成 `預售屋作業` ,各應收款項已於".$M.'/'.$D."匯入指定帳戶";
				$sms_txt_b = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCountTxt.",賣方".$seller.$oCountTxt.";".$addr."已完成 `預售屋作業` ,各應收款項已於".$M.'/'.$D."匯入指定帳戶";
				break;
			default:
				return false;
				break;
				...
				if (trim($_all[$i]["mMobile"]) != "") {
						...
						$mobile_tel = str_replace($check_word,"",$_all[$i]["mMobile"]) ;	//濾除分隔字元
							...
							if (preg_match('/^09[0-9]{8}$/',$mobile_tel)) {
                                ...
                                if (($fetchValue[0]['tCode2'] == '大額繳稅' || $fetchValue[0]['tCode2'] == '臨櫃開票') && $_all[$i]['tTitle'] == '地政士') {
                                	if ($fetchValue[0]['tCode2'] == '大額繳稅') {
                                		 $sms_txt_scrivener = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCountTxt.",賣方".$seller.$oCountTxt."之稅款已完成作業，請於".$M.'/'.$D."前往指定分行辦理";
                                		 $sms_id = $this->sms_send($mobile_tel,$_all[$i]["mName"],$sms_txt_scrivener,$target,$pid,$tid,$sys) ;
                                	}else if($fetchValue[0]['tCode2'] == '臨櫃開票'){
                                		 $sms_txt_scrivener = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.$bCountTxt.",賣方".$seller.$oCountTxt."之臨櫃開票已完成作業，請於".$M.'/'.$D."前往指定分行辦理";
                                		 $sms_id = $this->sms_send($mobile_tel,$_all[$i]["mName"],$sms_txt_scrivener,$target,$pid,$tid,$sys) ;
                                	}
	...
	public function sendIncome($pid,$sid,$bid,$target,$tid,$ok="n",$sendList=''){
		$sys = $this->getSmsSystem() ;
		$_contract_data = $this->getContractData($pid);
		...
		if ($checkService == 2) {
			$ownerList = array_merge($ownerList,$_data1,$_data4,$_data5) ;
			// echo 'A';
		}else{
		...
		for ($i=0; $i < count($other_phone); $i++) { 
			$_data[$_start]["mName"] = $_contract_data[0]["b_name"]	; 					
			$_data[$_start]["mMobile"] = $other_phone[$i]['cMobileNum']	;
			$_data[$_start]["iden"] = 'buy';
			$_data[$_start]["tTitle"] = '買方';
			$_start++;
		...
		for ($i=0; $i < count($other_phone); $i++) { 
			$_data[$_start]["mName"] = $other_phone[$i]["cName"]	; 					
			$_data[$_start]["mMobile"] = $other_phone[$i]['cMobileNum']	;
			$_data[$_start]["tTitle"] = '買方經紀人';
			$_start++;
		}
		...
		for ($i=0; $i < count($other_phone); $i++) { 
			$ownerList[$_owner_no]["mName"] = $_contract_data[0]["o_name"]	; 					
			$ownerList[$_owner_no]["mMobile"] = $other_phone[$i]['cMobileNum']	;
			$ownerList[$_owner_no]["iden"] = 'owner';
			$ownerList[$_owner_no]["tTitle"] = '賣方';
			$_owner_no ++ ;
		}
		...
		//賣方經紀人
		$_other_owners = $this->get_phone(4,$pid);
		for ($i=0; $i < count($_other_owners); $i++) { 
			$ownerList[$_owner_no]["mName"] = $_other_owners[$i]["cName"]	; 					
			$ownerList[$_owner_no]["mMobile"] = $_other_owners[$i]['cMobileNum']	;
			$ownerList[$_owner_no]["tTitle"] = '賣方經紀人';
			$_owner_no ++ ;
		}
		...
		switch ($target) {
			case 'income':
				...
				if (((($money - $fetchValue[0]["eBuyerMoney"]) > 0) && ($fetchValue[0]["eBuyerMoney"] > 0))||$fetchValue[0]['eExtraMoney']>0) {
					...
					//$sms_txt = "第一建經通知:買方".$buyer.$bCount.",賣方".$seller.$oCount."(保證號碼".substr($pid,5,9).")".$memo.",".$money."元於".$M.'/'.$D."存入履保專戶" ;
					$sms_txt_owner = '第一建經信託履約保證專戶已於'.$M.'月'.$D.'日收到保證編號' .substr($pid,5,9).'（買方'.$buyer.$bCount.'賣方'.$seller.$oCount.'）存入'.$memo.$money.'元' ;
					if ($changeMoney > 0) {
						$changeMoney = $changeMoney - $fetchValue[0]["eBuyerMoney"] - $fetchValue[0]['eExtraMoney'];

						$sms_txt_owner = '第一建經信託履約保證專戶已於'.$M.'月'.$D.'日收到保證編號' .substr($pid,5,9).'（買方'.$buyer.$bCount.'賣方'.$seller.$oCount.'）存入'.$memo.$changeMoney.'元' ;
					...
					$sms_txt_owner = $sms_txt;
				}

				##簡訊資訊顯示($ok = n)或發送($ok = y)
				
				if ($ok == 'y'){
					// echo 'CCCC';
					//要比對是否為仲介
					for ($i=0; $i < count($_data); $i++) { 
						foreach ($sendList['normal_sms'] as $val) {
							// echo $_data[$i]['mMobile']."_".$val."\r\n";
							if ($_data[$i]['mMobile'] == $val) {
								...
								if ($_data[$i]['boss'] == 1) { //簡訊要有地址
									...
									if (preg_match('/^09[0-9]{8}$/',$_data[$i]['mMobile'])) {
										// echo $_data[$i]['mMobile']."_".$_data[$i]["mName"]."_".$sms_txt."_".$target."_".$pid."_".$tid."_".$sys."\r\n";
					...
					for ($i=0; $i < count($ownerList); $i++) { 
						foreach ($sendList['owner_sms'] as $val) {
							if ($ownerList[$i]['mMobile'] == $val) {
								...
								if ($ownerList[$i]['boss'] == 1) {
									...
									if (preg_match('/^09[0-9]{8}$/',$ownerList[$i]['mMobile'])) {
										// echo $ownerList[$i]['mMobile']."_".$ownerList[$i]["mName"]."_".$sms_txt."_".$target."_".$pid."_".$tid."_".$sys."\r\n";
			...
			case 'income2':
				...
				if ($memo['owner'] != '') { //另外發送
					$sms_txt_owner = '第一建經信託履約保證專戶已於'.$M.'月'.$D.'日收到保證編號' .substr($pid,5,9).'（買方'.$buyer.$bCount.'賣方'.$seller.$oCount.'）存入'.$memo['owner'].'元' ;
					// if ($changeMoney > 0) { //調帳
					// 	$sms_txt_owner_changeMoney = '第一建經信託履約保證專戶已於'.$M.'月'.$D.'日收到保證編號' .substr($pid,5,9).'（買方'.$buyer.$bCount.'賣方'.$seller.$oCount.'）存入'.($changeMoney).'元' ;
					// }		
	...
	private function getBranchData($bId){
		$sql = "SELECT bSmsText,bSmsTextStyle FROM tBranch WHERE bId = '".$bId."'";

		$getMD = $this->DB_link->prepare($sql);
		$getMD->execute();
		$data = $getMD->fetch(PDO::FETCH_ASSOC);
	...
	private function sendCaseEndRealty2($pid,$mediaCode,$ok,$target,$tid,$sys,$buyer,$seller,$addr,$date,$manager){
		...
		$sql = 'SELECT
				...
				FROM
					tBankTrans AS tr 
				LEFT JOIN
					tContractCase AS cc ON cc.cCertifiedId=tr.tMemo
				WHERE
						...
						if (trim($tmp[$i]["mMobile"]) != "") {
								...
								if ($target == '點交(結案)') {
									...
									if ($tmp[$i]['smsTextStyle'] == 1) {
										//簽約日+買方姓名+賣方姓名+門牌+(京)+服務費內容。(不要有履保編號)
										$realty_sms_txt = '第一建經信託履約保證專戶通知：簽約日'.substr($value['signdate'], 5,2)."月".substr($value['signdate'], 8,2).'日,買方'.$buyer.',賣方'.$seller.';'.$addr.$branchTxt.',已完成點交作業服務費'.$value['money'].'元已於'.$date.'匯入仲介指定帳戶';

									}else{
										$realty_sms_txt = '第一建經信託履約保證專戶通知（保證號碼：'.substr($pid,5,9).'）：買方'.$buyer.',賣方'.$seller.';'.$addr.$branchTxt.',已完成點交作業服務費'.$value['money'].'元已於'.$date.'匯入仲介指定帳戶';
									}
									...
									if ($tmp[$i]['smsTextStyle'] == 1) {
										//簽約日+買方姓名+賣方姓名+門牌+(京)+服務費內容。(不要有履保編號)
										$realty_sms_txt = '第一建經信託履約保證專戶通知：簽約日'.substr($value['signdate'], 5,2)."月".substr($value['signdate'], 8,2).'日,買方'.$buyer.',賣方'.$seller.';'.$addr.$branchTxt.'，已完成預售屋作業服務費'.$value['money'].'元已於'.$date.'匯入仲介指定帳戶';

									}else{
										$realty_sms_txt = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.",賣方".$seller.';'.$addr.$branchTxt."，已完成預售屋作業服務費".$value['money']." 元已於".$date."匯入仲介指定帳戶";
									}
	...
	private function sendCaseEndRealty($pid,$mediaCode,$ok,$noSend,$target,$tid,$sys,$buyer,$seller,$date){
				...
				if ($ok == 'y') {
					for ($i=0; $i < count($tmp); $i++) { 
						if ($target == '點交(結案)') {
							...
							if ($tmp[$i]["smsTextStyle"] == 1) {
								//簽約日+買方姓名+賣方姓名+門牌+(京)+服務費內容。(不要有履保編號)
								$realty_sms_txt ='第一建經信託履約保證專戶通知：簽約日'.substr($value['signdate'], 5,2)."月".substr($value['signdate'], 8,2).'日,買方'.$buyer.',賣方'.$seller.';'.$addr.$realty_sms_txt.',已完成點交作業服務費'.$value['money'].'元已於'.$date.'匯入仲介指定帳戶';
							}else{
								$realty_sms_txt = '第一建經信託履約保證專戶通知（保證號碼：'.substr($pid,5,9).'）：買方'.$buyer.',賣方'.$seller.';'.$addr.$realty_sms_txt .',已完成點交作業服務費'.$value['money'].'元已於'.$date.'匯入仲介指定帳戶';
							}
							...
							if ($tmp[$i]["smsTextStyle"] == 1) {
								...
								$realty_sms_txt = "第一建經信託履約保證專戶通知（保證號碼：".substr($pid,5,9)."）：買方".$buyer.",賣方".';'.$addr.$realty_sms_txt.$seller."已完成預售屋作業服務費".$value['money']." 元已於".$date."匯入仲介指定帳戶";
							}
  ...
  ##
	...
	public function getsBranchMobile($pid,$bid,$title) {
		...
		if ($title != '') {
			...
			for ($i = 0 ; $i < count($arr) ; $i ++) {
				foreach ($smsTarget as $k => $v) {
					if ($arr[$i]['mMobile'] == $v) {
						...
						$aryTemp2[$x]['smsTextStyle'] = $arr[$i]['smsTextStyle'];
						if (($title == '店長') || ($title == '店東')) {
							$aryTemp2[$x]['boss'] = 1;//	
							$aryTemp2[$x]['smsText'] = $arr[$i]['smsText'];
			...
			//取得額外職稱
			if($title=='店長'){
				$sql= "SELECT a.bName as mName, a.bMobile as mMobile,b.tTitle,(SELECT CONCAT((Select bName From `tBrand` c Where c.bId = bBrand ),bStore) FROM tBranch WHERE bId = a.bBranch) AS storeName,(SELECT bSmsText FROM tBranch WHERE bId = a.bBranch) AS smsText,
					(SELECT bSmsTextStyle FROM tBranch WHERE bId = a.bBranch) AS smsTextStyle FROM tBranchSms AS a JOIN tTitle_SMS AS b ON b.id=a.bNID WHERE a.bDel = 0 AND a.bBranch=".$bid." AND b.tCheck =1 AND a.bCheck_id =".$pid ;
				...
				for ($i = 0 ; $i < count($arr3) ; $i ++) {
					foreach ($smsTarget as $k => $v) {
						if ($arr3[$i]['mMobile'] == $v) {
							...
							$Temp[$x]['smsText'] = $arr3[$i]['smsText'];
							$Temp[$x]['smsTextStyle'] = $arr3[$i]['smsTextStyle'];
							if (($title == '店長') || ($title == '店東')) {
								$Temp[$x]['smsText'] = $arr3[$i]['smsText'];
								$Temp[$x]['boss'] = 1;//	
							}						
			...
			for ($i = 0 ; $i < count($boss) ; $i ++) {
				...
				if ($boss[$i]['boss'] == 1) { 
					...
					if ($ok == 'y') {
						...
						// echo $mobile_tel.$mobile_name.$sms_txt_b.",".$target.",".$pid.",".$tid.",".$sys."\r\n"; 
					}else{
						// echo $mobile_tel.$mobile_name.$sms_txt_b.",".$target.",".$pid.",".$tid.",".$sys."\r\n";
					}
