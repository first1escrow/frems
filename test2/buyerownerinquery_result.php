...
$query = '' ; 
//if ($_SESSION['member_income'] == '1') $query = '' ;
//else $query = ' scr.sUndertaker1="'.$_SESSION['member_id'].'" ' ; 
...
if ($buyer && !$owner) {
	...
	if ($buyer) { 
		...
		// if ($query) { $query .= " OR " ; }
		// $query .= 'other.cIdentifyId="'.$buyer.'" AND  other.cIdentity = 1 ';
...
if ($branch) {
	...
	$branch_id += 1 - 1 ;
	//$query .= ' rea.cBranchNum="'.$branch_id.'" AND rea.cBrand=(SELECT bId FROM tBrand AS bah WHERE bCode="'.$bcode.'") ' ;
...
if ($_SESSION['member_test'] != 0) { 
	...
	// 		$rs->MoveNext();
	// 	}
	// 	$query .= "pro.cZip IN(".implode(',', $test_tmp).")";
	// }
...
// $max = (is_array($arr))?count($arr):0;

##
//一筆直接進案件內
if ($max==1) {
	// echo $max.",".$arr[0]['cCertifiedId'];
	// echo $cCertifiedId."<br>";
...
if ($max > 0) {
   ...
   // echo $query;
	...
	for ($i = 0 ; $i < count($arr) ; $i ++) {
		...
		// while ($tmp=mysqli_fetch_array($rel)) {
			
		// 	if ($tmp['cIdentity']==1) {
		// 		$tmp_buy[]=$tmp['cName'];
		// 	}elseif ($tmp['cIdentity']==2) {
		// 		$tmp_owner[] = $tmp['cName'];
		// 	}

		// }
		$buyer_count='';$owner_count='';$b=0;$o=0; $buyer='';$owner='';
		if (is_array($tmp_buy)) {
			$b=count($tmp_buy)+1;
			$buyer_count="等".$b."人";
		...
		if ($arr[$i]['cSignDate'] == '0000-00-00 00:00:00') $arr[$i]['cSignDate'] = '' ;
		
		if ($arr[$i]['cSignDate']) {
			$arr[$i]['cSignDate'] = substr($arr[$i]['cSignDate'],0,10) ;
			$tmp = explode('-',$arr[$i]['cSignDate']) ;
			if ($tmp[0]) $arr[$i]['cSignDate'] = ($tmp[0]-1911).'-'.$tmp[1].'-'.$tmp[2] ;
			unset($tmp) ;
		}
		...
		<tr style="text-align:center;background-color:'.$color_index.';">
			<td style="font-size:10pt;">
				<a href="#" onclick=detail("'.$arr[$i]['cCertifiedId'].'")>'.$arr[$i]['cCertifiedId'].'</a>
				<a href="#" onclick=contract("'.$arr[$i]['cCertifiedId'].'")><br>(合約書)</a>
			</td>
			<td style="font-size:10pt;">&nbsp;'.$arr[$i]['cBank'].'&nbsp;</td>
			<td style="font-size:10pt;">&nbsp;'.$arr[$i]['cSignDate'].'&nbsp;</td>
			<td style="font-size:10pt;">&nbsp;'.$arr[$i]['city'].$arr[$i]['area'].'&nbsp;</td>
			<td style="font-size:10pt;">&nbsp;'.$arr[$i]['brand'].'&nbsp;</td>
			<td style="font-size:10pt;">&nbsp;'.$arr[$i]['branch'].'&nbsp;</td>
			<td style="font-size:10pt;">&nbsp;'.$arr[$i]['scrivener'].'&nbsp;</td>
			<td style="font-size:10pt;" title="'.$buyer.'">&nbsp;'.$arr[$i]['buyer'].$buyer_count.'&nbsp;</td>
			<td style="font-size:10pt;" title="'.$owner.'">&nbsp;'.$arr[$i]['owner'].$owner_count.'&nbsp;</td>
			<td style="font-size:10pt;">&nbsp;'.$arr[$i]['undertaker'].'&nbsp;</td>
...
function checkSales($arr,$pId){
    ...
    // if ($arr['scrivenerCategory'] == 2 || $arr['sId'] == 1182 || $arr['sId'] == 632) { //直營代書不可以給業務看
    //     return false;
    // }
    ...
    $sql = "SELECT sSales FROM tScrivenerSales WHERE sScrivener =".$arr['sId']." AND sSales='".$pId."'";
    $rs = $conn->Execute($sql);
    $salesCount +=$rs->RecordCount();
