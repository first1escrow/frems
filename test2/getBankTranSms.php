...
if (is_numeric($_POST['vr'])) {
	$_all = $data  = $scrivener = $branchArr = $branchArr2 = $branch = $branch1= $branch2 = $branch3  = $branch4 = $buyer =$owner = $buyerAgent = $buyerAgent2 = $ownerAgent = $ownerAgent2 =array();

	$branchAccounting = $branchAccounting1 = $branchAccounting2 = $branchAccounting3 = array();
	...
	$branch = array_merge($branchArr,$branchArr2);
	...
	if ($caseData[0]['cBranchNum1'] > 0) {
		...
		$branch1 = array_merge($branchArr,$branchArr2);
	...
	if ($caseData[0]['cBranchNum2'] > 0) {
		...
		$branch2 = array_merge($branchArr,$branchArr2);
		unset($branchArr);unset($branchArr2);
	...
	if ($caseData[0]['cBranchNum3'] > 0) {
		...
		$branch3 = array_merge($branchArr,$branchArr2);
		unset($branchArr);unset($branchArr2);
	...
	//主賣方其他電話
	$other_phone = $sms->get_phone(2,$pid);
	...
	//其他賣方$pid
	$_other_owners = $sms->get_others($pid,'2') ;
	//print_r($_other_owners) ;
	...
	##
	//7賣方代理人
	$_other_owners = $sms->get_others($pid,'7') ;
				//print_r($_other_owners) ;
	for ($i = 0 ; $i < count($_other_owners) ; $i ++) {
