...
foreach ($Data as $key => $value) {
	...
	$sql = "SELECT * FROM tSMS_Log WHERE tKind = '回饋金2' AND sSend_Time >= '".$time."' AND tTo = '".$value['mMobile']."'";
...
if (count($noData2) > 0) {
	...
	foreach ($noData2 as $k => $v) {
		...
		if ($v['mMobile'] != '') {
			...
			$jsonArr['txt'] = '第一建經通知：111年第1季回饋金已結算,請點下列網址至第一建經官網確認,並依辦法請款;新E化回饋金結算操作手冊,可於登入官網後下載,謝謝。'.$url."\r\n";
			$jsonArr['name'] = $v['mName'];
			// print_r($v);

			if ($jsonArr['code'] == 'SC0210') {//|| $jsonArr['code'] == 'YC03664'
