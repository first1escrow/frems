...
//簡訊發送起始時間
$time = '2024-04-12 00:00:00';
//設定檔案存放目錄位置
$xls = __DIR__ . '/excel/113Q1.xlsx';
$sheet = 0; //季結
//讀取 excel 檔案
...
//     $sms = new SMS_Gateway();
    ...
    foreach ($noData2 as $k => $v) {
        if ($v['mMobile'] != '') {
            ...
            //組成發送文案
            $txt = ($sheet == 0) ? '第一建經通知：113年第1季' : '第一建經通知：113年01月';
            ...
            //測試簡訊
//            if(in_array($v['code'], [ 'TH00180', 'SC0166', 'SC0170'])) {
...
function getShortUrl($url, $key, $code = null)
{
    ...
    $sql          = "SELECT * FROM tShortUrl WHERE sCategory = '0' AND storeId = '" . $code . "'";
    $rs           = $conn->Execute($sql);
