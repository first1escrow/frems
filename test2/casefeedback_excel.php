...
$objPHPExcel->getActiveSheet()->setCellValue('AA'.$cell_no,'未收足');
...
for ($i = 0 ; $i < $max ; $i ++) {
	...
	$checkMoney = round(($list[$i]['cTotalMoney']-$list[$i]['cFirstMoney'])*0.0006);
	if ($checkMoney > ($list[$i]['cCertifiedMoney']+10)) {
		$objPHPExcel->getActiveSheet()->setCellValue('AA'.$cell_no,'未收足');
	}
