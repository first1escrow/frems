...
if (count($dataCase) > 0) {
    foreach ($dataCase as $k => $v) {
        ...
        // die;
        // if ($v['sCertifiedId'] =='090113862') {
        //     echo mb_strlen($oName)."_";
        //     echo mb_strlen($bName)."<br>";

        //    if (mb_strlen($oName) > mb_strlen($bName)) {
        //        $hBuyer = $hOwner;
        ...
        $pdf->SetXY($X,$Y) ;
        $pdf->MultiCell(32,$hStore,$storeTxt,1,'L') ;
        ...
        if ($v['sCertifiedId'] =='090113862') {
            $pdf->MultiCell(19,$hBuyer,$v['sBuyer'],0,'L') ;
        }else{
        ...
        $pdf->Cell(20,$h,number_format($v['sTotalMoney']),1,0,'R') ;
...
if (count($dataAccount)) {
    foreach ($dataAccount as $k => $v) {
        ...
        $pdf->Cell(50,6,'銀行',1,0,'L') ;
        $pdf->Cell(50,6,$v['BankMain'],1,0,'L') ;
        $pdf->Cell(50,6,'分行',1,0,'L') ;
        $pdf->Cell(50,6,$v['BankBranch'],1,1,'L') ;

        $pdf->Cell(50,6,'指定帳號',1,0,'L') ;
        $pdf->Cell(150,6,$v['sBankAccountNo'],1,1,'L') ;

        $pdf->Cell(50,6,'戶名',1,0,'L') ;
        $pdf->Cell(150,6,$v['sBankAccountName'],1,1,'L') ;

        if (($data['sMethod'] == 3 || $data['sMethod'] == 2) && count($dataAccount) > 1) {
            $pdf->Cell(50,6,'收款金額',1,0,'L') ;
            $pdf->Cell(150,6,$v['sBankMoney'],1,1,'L') ;
