...
function getTaxForm($cId,$arr,$today){
	global $pdf;
	global $owner;
	global $buyer;
	global $scrivener;
	...
	$pdf->setX($tmpX);
	$pdf->SetFont('msungstdlight', 'B', 8);
	$pdf->MultiCell(1.2, 1, $arr['area'], 1, 'C', 0, 0);//鄉鎮市區 
	$pdf->MultiCell(1.2, 1, $arr['cLand1'], 1, 'C', 0, 0);//段
	$pdf->MultiCell(1, 1, $arr['cLand2'], 1, 'C', 0, 0);//小段
	$pdf->MultiCell(1, 1, $arr['cLand3'], 1, 'C', 0, 0);//地號
	$pdf->MultiCell(0.6, 1, $arr['land4'], 1, 'C', 0, 0);//地目
	// $pdf->Cell(0.6, 1, $arr['land4'], 1,  0,'C', 0);//地目
	...
	$pdf->setX($tmpX);
	if ($arr['cPower1']==1 && $arr['cPower2'] == 1) { //打勾
		$pdf->Cell(0.3, 0.5, "v", 0,  0,'C', 0);
	}
	...
	$tmpX2 = $pdf->getX();
	$pdf->Cell(1, 0.68, "□持分", $border,  0,'L', 0);
	$tmpX = $pdf->getX();

	$pdf->setX($tmpX2);
	if ($arr['cPower1']!=1 && $arr['cPower2'] != 1) { //打勾
		$pdf->Cell(0.3, 0.6, "v", 0,  0,'C', 0);
	}
	...
	$pdf->Cell(1, 0.38, $arr['cPower1'], $border,  0,'C', 0); 
	...
	$pdf->Cell(1, 0.38, $arr['cPower2'], $border ,  0,'C', 0);
	...
	$pdf->Cell(0.8, 0.59, $arr['cMeasure'], 1,  0,'C', 0); //平方公尺
	...
	$pdf->Cell(0.5, 0.3, $arr['cMoveDateYear'], 1,  0,'C', 0); 
	$pdf->Cell(0.5, 0.3, $arr['cMoveDateMonth'], 1,  0,'C', 0); 
	...
	$pdf->Cell(1.5, 0.65, $arr['cMoveCategory'], 1,  0,'C', 0); 
	...
	$arr['cMoney'] = 30500;
	if ($arr['cMoney'] == 0) {
		$arr['cMoney'] = '_______________';
	}
	$txt = "□按土地公告現值每平方公尺\r\n".$arr['cMoney']."元計課";

	$pdf->MultiCell(4,0.6, $txt, $border, 'L', 0, 0); //cMoney
	$pdf->setX($tmpX);
	
	if ($arr['cMoney'] > 0) {
		$pdf->Cell(0.3, 0.2, "v", 0,  0,'C', 0);
	}
	...
	$pdf->SetFont('msungstdlight', 'B', 6);
	$tmpH = 0.3;
	$txt = "(10)茲委託".$scrivener['sName']."君代辦土地現值申報、領取土地增值稅繳款書或免稅證明書及應納未納土地稅繳款書、工程受益費繳款書等事項。";
	...
	$pdf->Cell(1.1, 0.8, '義務人', 1,  0,'C', 0); //
	// $x += 1.1;

	$pdf->Cell(2.5, 0.8, $owner['cName'], 1,  0,'C', 0); //姓名
	// $x += 2.5;

	$pdf->Cell(2, 0.8, $owner['cIdentifyId'], 1,  0,'C', 0);//身分證ID
	// $x += 2;
	$tmpX = $pdf->getX();
	$pdf->Cell(1, 0.4, $owner['cBirthdayDayYear'], 1,  0,'C', 0);//出生年
	// $x += 0.5;
	$tmpY = $pdf->getY()+0.4;
	$pdf->setY($tmpY);
	$pdf->setX($tmpX);
	$pdf->Cell(0.5, 0.4, $owner['cBirthdayDayMonth'], 1,  0,'C', 0);//出生月
	$pdf->Cell(0.5, 0.4, $owner['cBirthdayDayDay'], 1,  0,'C', 0);//出生日

	$tmpX = $pdf->getX();
	$pdf->setY($Y);
	$pdf->setX($tmpX);
	$pdf->Cell(1.2, 0.4, $arr['cPower1'], 1,  0,'C', 0);//權利移轉
	$tmpY = $pdf->getY()+0.4;
	$pdf->setY($tmpY);
	$pdf->setX($tmpX);
	$pdf->Cell(1.2, 0.4, $arr['cPower2'], 1,  0,'C', 0);//權利移轉
	// $x += 1.2;
	$pdf->SetFont('msungstdlight', 'B', 6);
	$tmpX = $pdf->getX();
	$pdf->setY($Y);
	...
	$pdf->Cell(4.8, 0.4, $owner['cRegistZip'].$owner['RegistArea'].$owner['cRegistAddr'], 1,  0,'L', 0); //戶籍地址
		
	$tmpY = $pdf->getY();
	$pdf->setY(($tmpY+0.4));
	$pdf->setX($tmpX);
	$pdf->Cell(1, 0.4, '', 1,  0,'C', 0); //通訊地址
	$pdf->Cell(4.8, 0.4, $owner['cBaseZip'].$owner['BaseArea'].$owner['cBaseAddr'], 1,  0,'L', 0); //通訊地址
	...
	$pdf->Cell(1.1, 0.8, '權益人', 1,  0,'C', 0); //權益人
	// $x += 1.1;

	$pdf->Cell(2.5, 0.8, $buyer['cName'], 1,  0,'C', 0); //姓名
	// $x += 2.5;

	$pdf->Cell(2, 0.8, $buyer['cIdentifyId'], 1,  0,'C', 0);//身分證ID
	// $x += 2;
	$tmpX = $pdf->getX();
	$pdf->Cell(1, 0.4, $buyer['cBirthdayDayYear'], 1,  0,'C', 0);//出生年
	// $x += 0.5;
	$tmpY = $pdf->getY()+0.4;
	$pdf->setY($tmpY);
	$pdf->setX($tmpX);
	$pdf->Cell(0.5, 0.4, $buyer['cBirthdayDayMonth'], 1,  0,'C', 0);//出生月
	$pdf->Cell(0.5, 0.4, $buyer['cBirthdayDayDay'], 1,  0,'C', 0);//出生日
	...
	$pdf->SetFont('msungstdlight', 'B', 6);
	$pdf->Cell(1, 0.4, '', 1,  0,'C', 0); //戶籍地址
	$pdf->Cell(4.8, 0.4, $buyer['cRegistZip'].$buyer['RegistArea'].$buyer['cRegistAddr'], 1,  0,'L', 0); //戶籍地址
		
	$tmpY = $pdf->getY();
	$pdf->setY(($tmpY+0.4));
	$pdf->setX($tmpX);
	$pdf->Cell(1, 0.4, '', 1,  0,'C', 0); //通訊地址
	$pdf->Cell(4.8, 0.4, $buyer['cBaseZip'].$buyer['BaseArea'].$buyer['cBaseAddr'], 1,  0,'L', 0); //通訊地址
	// $pdf->Cell(1, 0.5, '通訊地址', 1,  0,'C', 0); //通訊地址
	...
	for ($i=0; $i < 2 ; $i++) { 
		...
		$tmpX = $pdf->getX();
		$pdf->Cell(1, 0.4, '', 1,  0,'C', 0);//出生年
		// $x += 0.5;

		$pdf->setY(($tmpY+0.4));
		$pdf->setX($tmpX);
		$pdf->Cell(0.5, 0.4, '', 1,  0,'C', 0);//出生月
		$pdf->Cell(0.5, 0.4, '', 1,  0,'C', 0);//出生日

		$tmpX = $pdf->getX();
		$pdf->setY($tmpY);
		$pdf->setX($tmpX);
		$pdf->Cell(1.2, 0.4, '', 1,  0,'C', 0);//權利移轉
		...
		$pdf->setX($tmpX);
		$pdf->Cell(1, 0.4, '', 1,  0,'C', 0); //戶籍地址
		$pdf->Cell(4.8, 0.4, '', 1,  0,'L', 0); //戶籍地址
			
		$tmpY = $pdf->getY();
		$pdf->setY(($tmpY+0.4));
		$pdf->setX($tmpX);
		$pdf->Cell(1, 0.4, '', 1,  0,'C', 0); //通訊地址
		$pdf->Cell(4.8, 0.4, '', 1,  0,'L', 0); //通訊地址
	...
	$txt = '(11)申報人';
	$h = $tmpY-$Y2;
	$pdf->MultiCell(0.6, $h, $txt, 1, 'C', 0, 1,1.5,$Y2);//身分代號
	...
	$pdf->Cell(2.5, 1, $scrivener['sName'], 1,  0,'C', 0); //姓名

	$pdf->Cell(2, 1, $scrivener['sIdentifyId'], 1,  0,'C', 0);//身分證ID 
		
	$tmpX = $pdf->getX();
	$pdf->Cell(1, 0.5, '', 1,  0,'C', 0);//出生年
		// $x += 0.5;

	$pdf->setY(($tmpY+0.5));
	$pdf->setX($tmpX);
	$pdf->Cell(0.5, 0.5, '', 1,  0,'C', 0);//出生月
	$pdf->Cell(0.5, 0.5, '', 1,  0,'C', 0);//出生日
	 
	$tmpX = $pdf->getX();
	$pdf->setY($tmpY);
	$pdf->setX($tmpX);
	$pdf->Cell(1.2, 0.5, '', 1,  0,'C', 0);//權利移轉
	...
	$pdf->setX($tmpX);
	$pdf->SetFont('msungstdlight', 'B', 6);
	$pdf->Cell(1, 0.5, '', 1,  0,'C', 0); //戶籍地址 sCpAddress
	$pdf->Cell(4.8, 0.5, '', 1,  0,'L', 0); //戶籍地址
	
	$tmpY = $pdf->getY();
	$pdf->setY(($tmpY+0.5));
	$pdf->setX($tmpX);
	$pdf->Cell(1, 0.5, '', 1,  0,'C', 0); //通訊地址
	$pdf->Cell(4.8, 0.5, $scrivener['sCpZip1'].$scrivener['Area'].$scrivener['sCpAddress'], 1,  0,'L', 0); //通訊地址
	...
	$pdf->Cell(1.7, 0.5, $scrivener['sMobileNum'], 1,  1,'C', 0); //電話
	###
	$pdf->SetFont('msungstdlight', 'B', 6);
	$tmpY = $pdf->getY();
	$pdf->setY($tmpY);
	...
	$txt = $today['year']."年".$today['month']."月".$today['day']."日";
