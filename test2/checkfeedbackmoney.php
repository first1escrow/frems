...
function getFeedMoney($type,$id,$id2='',$FeedDateCat=''){
	...
	$str = " AND ( cc.cEndDate >= '".$sDate."' AND cc.cEndDate <= '".$eDate."')";
	...
	$sql ="SELECT 
        ...
        // echo $sql;
        // die;
 	// echo $sql; // AND cc.cFeedBackClose = 0
    $rs = $conn->Execute($sql);
    //
    while (!$rs->EOF) {
	...
	if (is_array($list)) {
		...
		for ($i=0; $i < count($list); $i++) { 
			if ($rs->fields['cCertifiedId'] == '100055823') {
	    		echo '********';
			    	die;
	    	}
		    ...
		    //如果有回饋給地政士 特殊回饋不回饋  
		    // echo $scrpart."_".$list[$i]['cFeedbackTarget']."_".$list[$i]['cFeedbackTarget1']."_".$list[$i]['cFeedbackTarget2']."_".$list[$i]['cFeedbackTarget3']."\r\n";
