...
require_once dirname(dirname(dirname(__DIR__))).'/configs/config.class.php';
require_once dirname(dirname(dirname(__DIR__))).'/class/advance.class.php';
require_once dirname(dirname(dirname(__DIR__))).'/openadodb.php' ;
require_once dirname(dirname(dirname(__DIR__))).'/report/getBranchType.php';
require_once dirname(dirname(dirname(__DIR__))).'/includes/maintain/feedBackData.php';
...
while (!$rs->EOF) {
	...
	if (empty($sales)) {
		...
		foreach ($sales_arr as $s) {
			if (checkSales($rs->fields, $s)) {
				$tf = true;
				break;
			}
		}
