		...
		foreach ($cCertifiedId as $k => $v) {
			...
			while (!$rs->EOF) {
				...
				if (($Arr['bid'] == $Arr['cBranchNum'])) {
					...
					$Arr['caseSalesID'] = ($Arr['cBrand'] == 1 || $Arr['cBrand'] == 49)?3:$Arr['caseSalesID'];//台屋跟優美算政耀
					$Arr['SalesName'] = ($Arr['cBrand'] == 1 || $Arr['cBrand'] == 49)?'曾政耀':$Arr['SalesName'];//台屋跟優美算政耀
					...
					$Arr['caseSalesID'] = ($Arr['cBrand1'] == 1 || $Arr['cBrand1'] == 49)?3:$Arr['caseSalesID'];//台屋跟優美算政耀
					$Arr['SalesName'] = ($Arr['cBrand1'] == 1 || $Arr['cBrand1'] == 49)?'曾政耀':$Arr['SalesName'];//台屋跟優美算政耀
					...
					$Arr['caseSalesID'] = ($Arr['cBrand2'] == 1 || $Arr['cBrand2'] == 49)?3:$Arr['caseSalesID'];//台屋跟優美算政耀
					$Arr['SalesName'] = ($Arr['cBrand2'] == 1 || $Arr['cBrand2'] == 49)?'曾政耀':$Arr['SalesName'];//台屋跟優美算政耀
...
function getOtherFeedForReport($data) {//把部分欄位取代
	...
	$sql = "SELECT * FROM tFeedBackMoney WHERE fCertifiedId ='".$data['cCertifiedId']."' ".$str."  AND fDelete = 0 AND (fType = 1 OR fType = 2)";
	// echo $sql."<br>";
	$rs = $conn->Execute($sql);
