...
if ($_POST) {
   ...
   $sql ="SELECT 
    ...
    for ($i=0; $i < count($list); $i++) {  //cBranchRecall cBranchScrRecall
         ...
         if (($list[$i]['cerifiedmoney'] + 10) < $tmpCerifiedMoney) {  //不回饋
            ...
            if ($list[$i]['branch'] > 0) {
                ...
                if ($list[$i]['brandRecall1'] != '' ) {
                    $brecall[0] = $list[$i]['brandRecall1'];
                
                }

                if ($list[$i]['brandScrRecall1'] != '') {
                    $scrRePart = $list[$i]['brandScrRecall1'];
                    $scrRePartsp[] = $list[$i]['brandScrRecall1'];
                }
            ...
            if ($list[$i]['branch1'] > 0) {
                ...
                if ($list[$i]['brandRecall2'] != '' ) {
                    $brecall[1] = $list[$i]['brandRecall2'];
                
                }

                if ($list[$i]['brandScrRecall2'] != '') {
                    $scrRePart = $list[$i]['brandScrRecall2'];
                    $scrRePartsp[] = $list[$i]['brandScrRecall2'];
                }
            ...
            if ($list[$i]['branch2'] > 0) {
                ...
                if ($list[$i]['brandRecall3'] != '' ) {
                    $brecall[2] = $list[$i]['brandRecall3'];
                
                }

                if ($list[$i]['brandScrRecall3'] != '') {
                    $scrRePart = $list[$i]['brandScrRecall3'];
                    $scrRePartsp[] = $list[$i]['brandScrRecall3'];
                }
            ...
            if ($bcount == 1) { //只有一間店
                ...
                if ($scrRePart != '') {
                    ...
                    //品牌回饋代書
                    if (is_array($scrRePartsp)) {
                        rsort($scrRePartsp);
                        $scrFeedMoney = round(($scrRePartsp[0]/100)*$list[$i]['cerifiedmoney']) ;
        ...
        unset($brecall);unset($scrRecall);unset($uSql);unset($type);unset($scrFeedMoney);unset($scrRePartsp);
